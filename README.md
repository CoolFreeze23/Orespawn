...
- **2.0.0-beta.9**: every creature's hit boxes follow its model —
  ...
  streams a creature's pose sends it every second tick.
- **2.0.0-beta.8**: `/kill` kills every OreSpawn creature again,
...
- **2.0.0-beta.7**: creatures you leave behind despawn as they
  ...
  wild adults go when left behind, so tame what you mean to keep. Applies to
  existing worlds as their chunks load.
- **2.0.0-beta.6**: the hotfix for the three player reports
...
- **Next**: the Queen Coherence pass (flight-state and animation
  ...
  — bone-synced boss hitboxes for King/Godzilla, config toggles
  for the original's roughest edges, the archived kyanite branch as
  optional content.
...
- **MultiHitboxLib** by DerToaster — vendored into
  ...
  (upstream credits: DerToaster, Meme Man, 19__). The upstream LICENSE text
  governs the vendored portion: the GNU Lesser General Public License,
  version 3 (29 June 2007), shipped verbatim as
  `src/main/resources/META-INF/LICENSE-MultiHitboxLib.txt` (inside the
  jar) and
...
source, with original file:line citations behind every change, and the
checks ship in the repository: a 192-test GameTest suite, and
a static asset audit that runs on every build. The commit
history is the audit trail — each change went in on a green build,
asset audit, and test suite.

...
[![Status](https://img.shields.io/badge/release-1.0.0--beta.1-blue)](https://github.com/CoolFreeze23/Orespawn/releases)
...
- [ ] **Nightmare** (5 size variants: 125 HP / 250 HP / 500 HP / 750 HP / 1000 HP)
      ...
      flagship Danger Dimension boss. Currently only `pitch_black` exists as a
      partial stand-in. **[v1.1]**
- [ ] **Leonopteryx** — Avatar-inspired flying boss, drops Battle Axe + Kraken
      Repellent. Should spawn in its own Mine Dimension nest dungeon, tameable
      with raw beef. Entirely absent from the registry. **[v1.1]**
- [ ] **Cave Fisher** is registered but its proper trip-wire / pulley hostile
      cave-ambush AI is unfinished — currently a static placeholder. **[v1.1]**
...
- [ ] **Robo Pounder**: ground-tearing attack (digs holes when arms swing into
      terrain) — currently only does melee damage. **[v1.1]**
- [ ] **Triffid**: invincibility-while-shell-closed reaction window — the open /
      closed defense state machine matching the 1.7.10 timing. **[v1.1]**
...
- [ ] **Dinosaur AI Polish** — Pointysaurus eye-contact aggression trigger
      ...
      T-Rex's Big-Bertha-tooth drop weight tuning. **[v1.1]**
- [ ] **Robo-Army Polish** — separate `Robo Pounder` / `Robo Gunner` /
      `Robo Spinner` / `Robo Warrior` AI files instead of `robot_1..5`
      generic shells (currently they share a base class with stat overrides
      only). **[v1.1]**
- [ ] **CaterKiller**: tree-eating health-regen behaviour + scheduled
      transformation into Brutalfly + butterflies on death. Entity exists,
      transformation logic does not. **[v1.1]**
...
- [ ] **Caterkiller cobweb-on-chase** — uses vanilla path slowdown, should
      drop actual `minecraft:cobweb` blocks under the player. **[v1.1]**
- [ ] **Nightmare Lair** spawner-block mob (separate from the Pitch Black)
      with 5-size lottery on spawn. **[v1.1]**

...
| filter order | (a) :216-226: nearest → `!isCreativeMode` (:217) → sight (:218) → strafe/shoot; a creative nearest is NULLED (:224-226) so (b) can run; (b) :408-441: null → self → !alive → Brutalfly (:418) → Mothra (:421) → Vortex (:424) → `isIgnoreable` (:427) → sight (:430) → EntityMob true (:433) → Player `!isCreativeMode` (:436-439) → false (:440) | (a) :205: nearest → `!invulnerable` → sight; an invulnerable nearest is NOT nulled, so `target == null` (:214) fails and (b) never runs; (b) :344-360, same chain (Monster for EntityMob :357; `!invulnerable` :358) | `FIXED (ENT-S-132, wave 3) — the creative nearest nulled (orig :224-226, port EntityBrutalfly.java:216) so the 1-in-3 mob hunt (:228) falls through; the sight step inside the non-creative arm (:218); the strafe's creative mapping was ENT-S-109's — was DIVERGES` (the S1 note "a creative nearest player shadows the mob hunt in the port" closes) | filter order: a creative/invulnerable nearest player shadows the mob hunt in the port; 1.7.10 fell through to it |
...
| creative gate | :548 `isCreativeMode` | vanilla `Abilities.invulnerable` + spectator via `Player.canBeSeenAsEnemy` (P5) | FIXED (ENT-S-132, wave 3) — the vanilla goal's conditions rebuilt non-combat with orig's !isCreativeMode (instabuild) as the selector (EntityCaterKiller.java:117-118); the vanilla hold (TargetGoal.canContinueToUse → canAttack) still reads invulnerable — T3b's rebuild or T5 ; the vanilla hold's canAttack and team terms still release the pick next pass (T3b / T5) — was DIVERGES | the ENT-S-109 mapping class, reached through the vanilla goal; not on ENT-S-109's list |
...
| creative gate | :223 `isCreativeMode` | vanilla `invulnerable` + spectator (P5) | FIXED (ENT-S-132, wave 3) — present at HEAD (ENT-S-108: instabuild at the orig position, CaveFisher.java:191; the snapshot's port cite :81-87 named the removed goals); pinned — was DIVERGES | the ENT-S-109 mapping class via the vanilla goal; not on its list |
| PEACEFUL gate | none | none | MATCH (engine, P6) | |
...
| creative gate | :242 `isCreativeMode` | vanilla `abilities.invulnerable` (`Player.canBeSeenAsEnemy` :903-905 via `TargetingConditions` :73) | FIXED (ENT-S-132, wave 3) — present at HEAD (ENT-S-108, DungeonBeast.java:183); pinned — was DIVERGES | the ENT-S-109 class at a vanilla-goal site; not on that record's list |
...
| creative gate | :496 | vanilla `invulnerable` | FIXED (ENT-S-132, wave 3) — present at HEAD (ENT-S-108, EntityEmperorScorpion.java:345); pinned — was DIVERGES | ENT-S-109 class, unlisted site |
...
| filter order | PlayNicely (:62) → nearest player (:65) → `shouldAttackPlayer` (:83-93): no pumpkin helmet (:84-87), player's look vector within `1 − 0.025/d` of the knight's mid-height (:88-91), player `canEntityBeSeen(knight)` (:92) → screaming on + return; else stareTimer reset, screaming off (:77-78) | vanilla chain only: self → spectator/dead → (no selector) → `canAttack` (PEACEFUL / creative) → range 64 → mob sight | FIXED (ENT-S-132, wave 3) — shouldAttackPlayer restored as the Reaper's twin (EnderKnight.java:121-140: pumpkin :84-87, the mid-height cone :88-91, the :92 player-side ray through OreSpawnSight.canSee(player, this) — the ENT-S-121 census's dropped call, ruling item 11) and wired as the goal's selector; the pick's screaming / stare-sound side effects (:68-74, :77-78) are the "other" row and stay absent — was DIVERGES | the enderman stare gate and the pumpkin exclusion are absent — any visible player is hunted |
| PlayNicely gate | :62 | none (:40-48) | FIXED (ENT-S-115, wave 1) — was DIVERGES | — |
| creative gate | none in the pick; the legacy loop drops a creative `EntityPlayerMP` target the same tick (`td.bq` :155-182), so a creative starer is picked (screaming toggles :74) then dropped, shadowing a farther survival player | creative skipped inside the scan (`invulnerable`, shared refs) | `FIXED (ENT-S-132, wave 3) — no creative term in the pick (the conditions rebuilt non-combat, EnderKnight.java:83-84) and the same-tick drop after it (canUse :90-94, isCreative → instabuild): a creative starer shadows a farther survival starer; the hold (holdsLegacyTarget :115-119) drops on instabuild, not canAttack — the revenge pick's canAttack overridden to the same mapping (D1) — was DIVERGES` (the scan-set / tie-break rows — nearest-of-any-mode-then-filter — stay T3b's: a creative non-starer does not shadow in the port) | shadowing lost (the Kraken KT-A pattern) and `invulnerable` for `isCreative` |
| PEACEFUL gate | none | vanilla player refusal | MATCH (engine, P6) | inert (re-rated 2026-09-04, T9: was PORT-ONLY — no port-written code, the engine's own `canAttack` refusal, rated MATCH (engine, P6) on every other Monster row per V13; nothing to delete, never add a `canAttack` override) |
...
| creative gate | none in the pick; legacy loop drops a creative target the same tick (`td.bq` :155-182) — a creative starer shadows a farther survival starer | creative skipped inside the scan (`invulnerable`) | FIXED (ENT-S-132, wave 3) — as the Knight: EnderReaper.java:80-81 (pick, no creative term), :87-91 (the same-tick drop), :112-116 (the hold); the registration 6-arg → 3-arg with the selector in the initializer, identical values — the revenge pick's canAttack overridden to the same mapping (D1) — was DIVERGES | shadowing lost; `invulnerable` for `isCreative` |
| PEACEFUL gate | none | vanilla player refusal | MATCH (engine, P6) | inert (re-rated 2026-09-04, T9: was PORT-ONLY — no port-written code, the engine's own `canAttack` refusal, rated MATCH (engine, P6) on every other Monster row per V13; nothing to delete, never add a `canAttack` override) |
| allies / species exclusions | none | none | MATCH | — |
| ignore screen | none | none | MATCH | — |
...
| creative gate | :409 | vanilla `invulnerable` | FIXED (ENT-S-132, wave 3) — present at HEAD (ENT-S-108, EntityHerculesBeetle.java:267); pinned — was DIVERGES | ENT-S-109 class, unlisted site |
| PEACEFUL gate | none | vanilla player refusal | MATCH (engine, P6) | inert (re-rated 2026-09-04, T9: was PORT-ONLY — no port-written code, the engine's own `canAttack` refusal, rated MATCH (engine, P6) on every other Monster row per V13; nothing to delete, never add a `canAttack` override) |
...
| target set / release | custom scan stores nothing; `fly_with_rider` reads `getAttackTarget()` first (:358), drops it when dead (:359-362); `updateAITasks` :340-342 1-in-200 `setAttackTarget(null)`; `attackEntityFrom` :318-331: Leon attacker → no damage/no target, `tamed && player` → damage taken but no target (:324-326), else `setAttackTarget(attacker)` + `setLastAttacker` + navigate 1.2 (:327-329); `EntityAIHurtByTarget(this,false)` :95 | `flyWithRider` :491-495 same; `customServerAiStep` :515-517 1-in-200 `setTarget(null)`; `hurt` :402-412: Leon attacker → false (:403), `isTame() && player` → **false before super.hurt** (:404), else `setTarget` + moveTo 1.2 (:409-412); `HurtByTargetGoal` :157 (VAN-HBT, 300-tick unseen memory, no sight); PORT-ONLY `OwnerHurtByTargetGoal` :155 and `OwnerHurtTargetGoal` :156 (orig registers no owner goals) | FIXED (ENT-S-131, T5b) — the AI tick's 1-in-200 final through `RevengeGoal.release()` (EntityLeon.java:568-571; the goal :170-171, :194-212; the hunt goal holds no memory and ends on the null as orig :93's task did) — the release half re-rated DIVERGES (2026-09-04 night, the ENT-S-129 refuter A's re-assert note); RECORDED (MOD-033, ENT-S-125) — the owner pair registers only in modern; classic = orig :92-95; the scan-set row's PORT-ONLY predicate rides with the same key — was PORT-ONLY (owner goals) | release rule: vanilla's TargetGoal re-assert refilled the slot the tick nulled (:545 at HEAD) within a tick; orig :340-342 ended both target tasks; out of targeting scope but adjacent: orig :321 applies the damage to a tamed Leon hit by a player and only skips the retaliation; port :404 skips the damage too |
...
| aspect | 1.7.10 (file:line) | port HEAD (file:line) | status | note |
|---|---|---|---|---|
| scan set | `EntityLivingBase, bbox.expand(16,8,16)` Mantis.java:398; `updateAITasks` :205 else-branch `nextInt(8)==0` (re-pick :186 `stuck>50 \|\| nextInt(300)==0 \|\| distSq<2.1`), retaliation target tried first (:207-213) | `LivingEntity, inflate(16,8,16)` EntityMantis.java:232-233; :167 `nextInt(8)==0` (re-pick :147, threshold 4.0), retaliation first (:168-170) | MATCH | the port-only target goals that were at :67-68 (`HurtByTargetGoal`, `NearestAttackableTargetGoal<Player>`) are removed from both modes (ENT-S-125) — see release row; cites re-read 2026-09-04 after the removal |
...
| creative gate | :273 `field_75098_d` | VAN-TC `Player.canBeSeenAsEnemy` → `abilities.invulnerable` ((v) `Player.java:966-968`) | FIXED (ENT-S-132, wave 3) — present at HEAD (ENT-S-108, Nastysaurus.java:262); pinned — was DIVERGES | same class as ENT-S-109 (invulnerable vs instabuild) but through the vanilla predicate, not a listed site |
...
| aspect | 1.7.10 (file:line) | port HEAD (file:line) | status | note |
|---|---|---|---|---|
| scan set | `EntityLivingBase, bbox.expand(16+6t, 10+4t, 16+6t)` (t = scale 0.5..4) PitchBlack.java:544-546; callers: `onUpdate` :281-288 (activity==0, 1-in-10 → activity 1), `onUpdate` :259-275 (server, 1-in-250 heal branch: 1-in-5 & solid ground within 10 → scan, null → activity 0), `updateAITasks` :361-363 (activity==1, else-branch 1-in-8; re-pick :343 `nextInt(150)==0 \|\| distSq<2.1`) | same formula PitchBlack.java:509-513 (t from tier: 0.5,1,2,3,4); callers `customServerAiStep` :421-431 (activity==0, 1-in-10 → activity 1 + nav stop), :448-449 (1-in-8 else-branch; re-pick :437-438) | FIXED (ENT-S-129, wave 2) — the orig :259-280 branch (heal 1 + scale, the null-scan deactivation, the spontaneous activation) transcribed — was DIVERGES | scan set: the :259-275 heal-branch call site (the only path that RESETS activity to 0 when nothing is found) is absent from the port (tick :392-403 has no heal branch) |
...
| creative gate | :244 `field_75098_d` | VAN-TC `invulnerable` for the NearestAttackable goal; stare goal `isCreative()` (= instabuild) :48 | FIXED (ENT-S-132, wave 3) — the vanilla goal's conditions rebuilt non-combat with the ENT-S-106 screen and orig's !isCreativeMode (instabuild) as the selector (Pointysaurus.java:105-107); the vanilla hold still reads invulnerable — T3c's rebuild or T5; PointysaurusStareGoal:48 is MOD-034's port-only goal, untouched ; the vanilla hold's canAttack and team terms still release the pick next pass (T3b / T5) — was DIVERGES (goal path) |  |
...
| other | reach 4+w/2 (:203), dice `nextInt(5)==0 \|\| nextInt(6)==1` (:205), navigate 1.25 (:209) | preset 4.0 / 5,6 / 1.25 (`DinosaurMeleeAttackGoal.java:36`); PORT-ONLY eye-contact aggression :60-65 | RECORDED (MOD-034, ENT-S-125) — as the scan-set row — was PORT-ONLY | |
...
| filter order | :234-265 PEACEFUL (:236), null, self, dead, ignore (:248), sight (:251), Player: creative→false, `return type<=0 \|\| type==10` (:254-260), `type not 0/10 && tamed EntityTameable → false` (:261-263), `return !isRoyalty` (:264) | :194-203 null/self/dead, ignore (:196), Player instabuild→false (:197), Player → `type<=0 \|\| type==10` (:199-201), true | FIXED (ENT-S-118, wave 2) — sight (:251); PEACEFUL (:236) FIXED (ENT-S-114, wave 1); the tamed-pet (:261-263) and royalty (:264) steps FIXED (ENT-S-128, wave 2; PurplePower.java:215-216) — was DIVERGES | filter order: PEACEFUL (:236), sight (:251), tamed-pet exclusion (:261-263) and royalty exclusion (:264) all missing |
...
| target set / release | attackTarget 1-in-50 clear (:242-244), dead → clear (:245-248); `attackEntityFrom` :285-291 sets no target; `EntityAIHurtByTarget` :58 (revenge target consumed at :245; 1.7.10 goal: no sight, `disableDamage` players excluded) | :126-127; `HurtByTargetGoal` :88 (VAN-HBT: no sight, `invulnerable` players excluded), consumed at :125 | FIXED (ENT-S-131, T5b) — the pass's 1-in-50 final through `RevengeGoal.release()` (Robot3.java:148; the goal :91-111) — was MATCH, re-rated DIVERGES (2026-09-04 night, the ENT-S-129 refuter A's re-assert note) | release rule: vanilla's TargetGoal re-assert refilled the slot the pass nulled (:127 at HEAD) within a tick; orig :242-244 ended the :58 task |
...
| target set / release | attackTarget 1-in-50 clear (:282-284), dead → clear (:285-288); `attackEntityFrom` :336-355 shield / 65-tick gate then attackTarget for `EntityLiving` attackers + navigate 1.2 (:346-351); `EntityAIHurtByTarget` :61 | :175-176; `hurt` :259-272 `Mob` attackers (:266-270); `HurtByTargetGoal` :89 | FIXED (ENT-S-131, T5b) — the pass's 1-in-50 final through `RevengeGoal.release()` (Robot4.java:196; the goal :91-111) — was MATCH, re-rated DIVERGES (2026-09-04 night, the ENT-S-129 refuter A's re-assert note) | release rule: vanilla's TargetGoal re-assert refilled the slot the pass nulled (:175 at HEAD) within a tick; orig :282-284 ended the :61 task |
...
| target set / release | attackTarget 1-in-50 clear (:214-216), dead → clear (:217-220); `attackEntityFrom` :259-265 sets no target; `EntityAIHurtByTarget` :56 (consumed at :217) | :113-114; `HurtByTargetGoal` :75 (consumed at :112) | FIXED (ENT-S-131, T5b) — the pass's 1-in-50 final through `RevengeGoal.release()` (Robot5.java:135; the goal :78-98) — was MATCH, re-rated DIVERGES (2026-09-04 night, the ENT-S-129 refuter A's re-assert note) | release rule: vanilla's TargetGoal re-assert refilled the slot the pass nulled (:114 at HEAD) within a tick; orig :214-216 ended the :56 task |
...
| creative gate | `isCreativeMode` :519 | vanilla `canBeSeenAsEnemy` → `abilities.invulnerable` (P1) | FIXED (ENT-S-132, wave 3) — the vanilla goal's conditions rebuilt non-combat with orig's !isCreativeMode (instabuild) as the selector (SeaViper.java:113-114); the vanilla hold still reads invulnerable — T3b's rebuild or T5 ; the vanilla hold's canAttack and team terms still release the pick next pass (T3b / T5) — was DIVERGES | missing gate (creative mapping); not in ENT-S-109's list |
...
| creative gate | `isCreativeMode` :363 | vanilla `invulnerable` (P1) | FIXED (ENT-S-132, wave 3) — present at HEAD (ENT-S-108, EntitySpitBug.java:301); pinned — was DIVERGES | missing gate (creative mapping); not in ENT-S-109's list |
...
| creative gate | `isCreativeMode` :243 | vanilla `invulnerable` (P1/P2) | FIXED (ENT-S-132, wave 3) — present at HEAD (ENT-S-108, TRex.java:245); pinned — was DIVERGES | missing gate (creative mapping); not in ENT-S-109's list |
...
| creative gate | `isCreativeMode` :503 | vanilla `invulnerable` (P1) | FIXED (ENT-S-132, wave 3) — present at HEAD (ENT-S-108, EntityTrooperBug.java:335); pinned — was DIVERGES | missing gate (creative mapping); not in ENT-S-109's list |
...
| target set / release | attack target from hurt (:490-492, `EntityLiving` attacker, not AttackSquid/WaterDragon); `setAttackTarget(null)` 1-in-200 every AI tick (:594-596); kept while alive (:694-698) | `hurt` :266-269 (`Mob`); the 1-in-200 release is `Presets.waterDragon` forgetTargetRoll 200 (ai/DinosaurMeleeAttackGoal.java:43, rolled in ai/BugMeleeAttackGoal.java:123-128 while engaged); `HurtByTargetGoal` release per P1 (FOLLOW_RANGE 32, :123) | FIXED (ENT-S-131, T5b) — the 1-in-200 every AI tick ahead of the hunt pass (WaterDragon.java:367-370, `== 0`, target or none), final through `RevengeGoal.release()` (the goal :141-161); `Presets.waterDragon` forget 200 → 0 (ai/DinosaurMeleeAttackGoal.java:43) — was MATCH — note (ENT-S-117, wave 2): the hunt's own pick lives in the slot under `scanPick` (:98), re-derived per pass; the hurt / revenge target sticky as orig (:395) — re-rated DIVERGES (2026-09-04 night, the ENT-S-129 refuter A's re-assert note) | release rule: the melee goal's forget rolled only while engaged and vanilla's TargetGoal re-assert refilled the slot it nulled within a tick; orig :594-596 rolled every tick and ended the :76 task |
...
| creative gate | none — a creative player is valid prey on Valentine's day | VAN-TC refuses `abilities.invulnerable` players (creative and spectator) | RECORDED (MOD-036, ENT-S-125) — as the PEACEFUL row: a deliberate parity exception kept in BOTH modes on the owner's safety ruling; pinned (`PortOnlyTargetingTests#mod036_girlfriend_valentine_gates_hold_in_classic` / `_modern`) — was DIVERGES | port-only gate |
...
- Ruling 2026-09-04 night → MOD-033 extension (implemented 2026-09-05): the T9 residual's four species gated under `petsDefendOwner` (Hydrolisc, VelocityRaptor: all three goals; Boyfriend, Girlfriend: the owner pair — commit 2b0c2cd, no stated intent) and Leon's tame predicate (modern only; classic the bare Enemy selector, orig Leon.java:93); Camarasaurus carries none; six pins (PortOnlyTargetingTests 15 → 21). Observation (the refuter): Leon's HurtBy @3 / hunt @4 and the Girlfriend's hunt @5 invert orig's order — filed as ENT-S-130.

- Wave 2, batch T5b → ENT-S-131 (with ENT-S-126, ENT-S-127 and PN-021; 2026-09-05): 5 row(s) marked FIXED — the ENT-S-129 refuter A's re-assert rows (Robot3 / Robot4 / Robot5's 1-in-50 inside the pass, Leon's 1-in-200, the Water Dragon's 1-in-200 moved out of the melee goal's tick to every AI tick), each final through a per-species RevengeGoal.release(); the flyers' flight-target write-before-test transcribed (ENT-S-126: StinkyIdleParityTests rows 17 / 18 re-based on the ray-refusal count, two boxed-in pins); 1.7.10's vanilla-task Creeper refusal port-wide through OrigTargets.vanillaTaskPrey at the four sites (ENT-S-127, four pins); the Stinky's coal ore read through #minecraft:coal_ores (PN-021, one pin). Observations: (1) orig Robot2..5 roll the 1-in-50 clear BEFORE the pass reads the target (Robot2.java:281-284, Robot3.java:242-245, Robot4.java:282-285, Robot5.java:214-217) where the port — T5's Robot2 included — reads first, so a forgotten target is engaged once more in the forgetting pass when the scan would not re-pick it (the GiantRobot :243 / AntRobot :109 shape; the ledger's robot rows rate the pass MATCH) — presented for wave 3, not moved; (2) EntityStinky's flight pick lacks orig Stinky.java:617-627's owner branches (nextInt(4) + 6 on the ground, nextInt(8) with the owner flying) that EntitySpyro carries, and both ports roll xdir before zdir where orig rolled zdir first — a flight / wander row outside this ledger.`
- Wave 3, batch T8 → ENT-S-132: 16 row(s) marked FIXED; skipped: MyValentineTarget (Girlfriend), RECORDED under MOD-036. Eight rows (the ENT-S-108 hunters) were already `instabuild` at HEAD and are pinned, not re-edited; three vanilla-goal sites (CaterKiller, SeaViper, Pointysaurus) carry orig's creative test as a non-combat selector with their T3b / T3c class and box kept; the Brutalfly / Mothra creative fall-through and Mothra's stage-1 sight step (the T2 cross-reference) restored; the Ender pair's pick admits any mode and drops a creative pick the same tick (the KT-A shadowing) with the hold on `instabuild` and the revenge pick's `canAttack` overridden to the same mapping (the refuter's D1); the Ender Knight's `shouldAttackPlayer` restored with the :92 ray through `OreSpawnSight.canSee` (ruling item 11 closed). Disclosed: the three rebuilt goals' vanilla hold still reads `invulnerable` (T3b / T3c / T5); the Ender pair's nearest-then-filter scan set stays T3b's. 60 pins in `CreativeGateParityTests` (own batch).
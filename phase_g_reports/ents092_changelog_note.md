...
The Purple Power orb looks as it did in 1.7.10 again: a translucent, glowing ball whose three rings of spokes jump to a new random orientation every frame, shimmering, instead of the solid, normally lit ball turning smoothly at three fixed rates the port had drawn. The classic renderer draws it that way now, and the developer-switch GeckoLib candidate (`-Dorespawn.dev.geckolibRenderers=purple_power`) draws the identical frame - the same rolls, the same blend, the same colour and light - so the two cannot drift apart.

Nothing player-visible yet: the keyframe leg's return adds the `[modern] artistAnimations` / `classicAnimationSpecies` keys (default ON, inert while every shipped clip file is empty), the phase-locked controller, the catmullrom repair in the replacement seam and the harness's keyframe reference leg; the Beaver keeps its classic code-driven pose on both renderers until the owner's in-game look accepts the regenerated clip (dev-switch candidate only, `-Dorespawn.dev.geckolibRenderers`).

- A hurt Dragonfly now aims its flight at the block 1.7.10 aimed at: below y 0 that is the block above its attacker's feet, where the old build aimed at the feet block itself (ENT-S-148). Its vertical steering eases toward the target at 1.7.10's own rate (the y blend 0.2 instead of the sideways 0.3), so it climbs and dives a touch more gently (ENT-S-149). The Stinky and the Spyro decide they have reached a flight target the way 1.7.10 did, by the whole-block distance from the block they are in rather than from their exact position to the block's centre, so they pick a new target at the same moments 1.7.10 did (ENT-S-151).

- **The Queen's hitboxes, three fixes.** For one server tick after a Queen spawned, her ten collidable hurtboxes sat at the world origin (0, 0, 0) instead of on her — a hit or an arrow in that tick met nothing there, and anything standing at the world origin could be shoved by a 10-block box; that is gone (it was the first tick of every MHLib-profiled mob's parts — the robot spiders' and ants' legs were re-fed in the same tick and never showed it; the King's and Godzilla's parts are not MHLib's and never had it). A sword blow on a wing, a head or a leg now does everything a blow on the body does — it knocks the mob back (the Queen shrugs it off; the robot spiders and ants take it like any mob), sets it on fire with Fire Aspect, can count as a crit or a sweep (the sweep now fans out around the mob's body rather than around the wing you hit, and no longer nudges the mob itself), shows the damage you dealt, and registers as your last hit — while the part's own damage multiplier still applies (heads 1.0, body and legs 0.5, wings and tail 0.25). And she is now frustum-culled like every other mob: a Queen more than about 78 blocks away on some axis (about 20 while Play Nicely shrinks her) and off-screen is no longer rendered, instead of being drawn in full behind you; nearer than that she is drawn whenever she is loaded, as before, and in view she is drawn at any distance. One side effect to know about: while the player whose game streams her animation (in single player, you; in multiplayer, the first player who came into range of her — not the nearest) does not render her, her hitboxes rest in her standing pose until that player looks again. The Prince (teen) also walks at his 1.7.10 pace — 0.32, not the 0.35 he had been quietly given back.

Nothing changes for a default install (this push: item 15 landed and the first Tier-2 slice, 2026-09-12/13). Behind the developer switch (`-Dorespawn.dev.geckolibRenderers=<name>` or `=candidate`) the GeckoLib Beaver now plays the keyframe transcription of its classic gait, teeth and tail rhythms — the transcription ships in its animation file and is proven within 2.5e-3 rad of the classic formulas — and six more creatures — the Tshirt, Mosquito, Cliff Racer, Brutalfly, Dragonfly and Cockateil (with the Ruby Bird, which shares its rig) — carry a GeckoLib version of their classic model proven to match the classic renderer within the harness's tolerances, each shipping an editable keyframe transcription of its wing, jaw or turning rhythm for the artist package; on the switched-on candidate the four one-rhythm creatures (Tshirt, Mosquito, Cliff Racer, Brutalfly) play that transcription, while the multi-rhythm Dragonfly and Cockateil keep posing from their classic code until a naming ruling lands. The classic renderers remain the default and pose exactly as before. The artist handoff rules now say: deliver `idle` and `walk` together (one without the other leaves a creature on its code-driven motion) and put no sound or particle keys on looping clips. The Ghost, Firefly, Cloud Shark and Gold Fish stay classic (their rigs do not fit the current harness; recorded for the owner).

Nothing changes for a default install (this push: ENT-S-161 and the second Tier-2 slice, 2026-09-13). Behind the developer switch (`-Dorespawn.dev.geckolibRenderers=<name>` or `=candidate`) the GeckoLib candidates now light every face of a creature by its true surface normal — the animation library's own renderer bent the lighting normal of paper-thin, tilted parts (wings, fins) and, at some viewing angles, of any thin plate — so the candidates are lit exactly as the classic renderers are. Three more creatures carry a GeckoLib version of their classic model proven to match the classic renderer within the harness's tolerances, each shipping an editable keyframe transcription of its rhythms for the artist package: the Firefly and the Gold Fish — held back last time because of that very lighting fault, drawn correctly now that each thin fin's two faces are also emitted in the classic order — and the Ant rig (the Ant, Rainbow Ant, Red Ant, Termite and Unstable Ant share it), the first creature after the Beaver whose leg swing scales with its walking speed on the candidate. Every switched-on candidate now plays its transcription in-game: the naming rule ruled this week — a creature with several rhythms and no walking gait carries the plain `walk` name on the rhythm its sheet calls primary — opened the Dragonfly, Cockateil and Ruby Bird, which had kept posing from their classic code. The classic renderers remain the default and pose exactly as before. For the artist package: key times round-trip within a ten-thousandth of a second, and a creature animated by its own GeckoLib controllers (the Queen) may carry sound and particle keys on its loops, the checker saying why the others may not. The Cloud Shark stays classic for now (a harness question on how thin fins are compared, recorded for the owner), as do the creatures whose motion the current transcription form cannot express yet.

Textures on 97 models now sit on their cubes the way the original 1.7.10 mod drew them (BUG-041, stage 2). The port had turned every model's "mirror = true" line into a mirror flag that actually took effect, so on 89 mobs (from the Ant, Bee and Beaver to the Kraken, the Godzilla family, the royal family and the Triffid), the Butterfly family (Butterfly, Luna Moth, Mothra, Vampire Butterfly) and seven held items (Battle Axe, Bertha, Chainsaw, Attitude Adjuster, Queen Battle Axe, Slice, SquidZooka) every face sampled its texture flipped left-to-right relative to the original — invisible on symmetric texels, visible wherever a texture is asymmetric (eyes, markings, text, blades). In 1.7.10 that line came after the box was built and never reached it, so the original rendered unmirrored; this release drops the flag everywhere the original never applied it. Nothing else moves: the geometry, poses, animations and sizes are unchanged and proven identical against the 1.7.10 sources, only the texel orientation of the asymmetric faces changes, exactly as the Ender Reaper's did in the earlier A/B build. That first look at the held-item models also turned up one geometry slip to fix later (the Chainsaw's blade tip sits 2.5 units off and its running tooth is missing), recorded, not yet changed. Behind the scenes: the harness now compares thin cutout faces by a fixed first-drawn rule and refuses a cutout rig without its face order; every creature's package for the artist carries a reference-only clip of its classic motion beside the sheet, with the plain-language formulas for the creatures still running on their classic code; and the first two artist packages — the Queen (idle and her awake stance) and the Beaver — are generated into the repository.

Behind the dev switch, fifteen more creatures now draw through the new renderer path on their classic motion: the Cloud Shark, the Bee, the Fairy (translucent, in its own sheet's colours), the Gamma Metroid, the Irukandji, the Skate, the Rubber Ducky, the Terrible Terror, the three Worms, the Cannon Fodder, the CaterKiller, the Cricket and the Hercules Beetle — thirty-eight of the hundred and six rigs so far. Each poses exactly as its classic code does today, proven bone for bone against the classic renderer, and none plays a hand-made animation until an artist delivers its idle and walk; the classic renderers remain the default and pose exactly as before. For the artist package: the Queen's pilot asks for her dormant hover and her bite (the impact where the hurt lands), the Beaver's package stands, every provisional note on the sheets now states the ruling it rests on, each creature's reference clip spans the shortest stretch after which every rhythm returns to its start (six seconds at most), and the build now regenerates and verifies those reference clips on every run. Four places where the port's classic poses simplify the original's (the Gamma Metroid, the Rubber Ducky, the Terrible Terror, the Cricket) are recorded for a later pass, unchanged.

The artist folder now holds every creature an animator will touch: 112 species folders in one package, bosses first, each with its rig, textures, sheet and Blockbench file. The 44 creatures already drawn through the new renderer path carry their reference clips; the 68 not yet in-game carry the rig the converter builds from the port's own model, proven part for part against the 1.7.10 source, with the sheet saying plainly that the rig is not yet in the game and that its bone names are final. Every one of those 68 sheets is a DRAFT written from the creature's code — what it is, what it does, which clips to make, what an animator might add — for the owner to edit, and each boss sheet pre-declares which bones will carry its hitbox parts, marked provisional until the profiles land. Four species are still without a folder, named in the package: the Giant Robot pair, whose model draws its legs and arms twice, and the Boyfriend and Girlfriend, drawn on vanilla's biped with no original of their own to compare against. In the earlier sheets, the four creatures whose classic poses lost motion in the port (the Gamma Metroid, Rubber Ducky, Terrible Terror and Cricket) now say what the original moved more, so an animator can animate toward it. Nothing changes in-game.

The artist folder now grows to 116 species: the GiantRobot and Jeffery carry their real bone set (the arms and legs the classic renderer drew twice now stand as their own bones), the Crab's twenty-four leg and claw bones the same way, and the Boyfriend and Girlfriend ship as geometry alone, since 1.7.10 drew them on the vanilla player rig; every folder's sheet now names the goal each creature runs and the state that fires its attack pose, read from the code rather than guessed.

Every creature not yet drawn through the new renderer path now carries its classic motion in the port's own words: the 1.7.10 setupAnim of sixty-two rigs transcribed line for line into the descriptor a landing will use, so the day a creature's rig lands, its animation proof is already written. Three creatures wait for their landing to decide the matter (the Boyfriend and Girlfriend, who animate on the vanilla player rig, and the Princess, whose power orbs spin on the model itself rather than on the creature).

And the reference clips reach every creature: 87 of the ninety artist-tier species now carry a reference walk, a reference idle where the creature has one, and a reference attack where its code reads an attack, each sampled from the classic motion under the same rule as before (the earlier forty-four clips reproduce byte for byte as the new walk clips). The clips are not baked into the Blockbench files (the generator's own round trip cannot promise the keys back byte for byte, and Blockbench's real precision is unread), so the folder's README says how to import them: Animation, Import Animations, the three files beside the sheet. An animator may start from a reference clip's keys and improve on them in idle, walk or aggro_idle; only the reference file itself, sent back under its own name, is refused.

The reference clips now cover every state a creature's code can be in, not only its walk and its attack: 297 clips over 112 registries, a walk and an idle for each, and where the code branches on a state, that state alone (ten flights, fourteen sits, a scream, a display, a crouch, an open mouth), each named in the animator's words by the creature's sheet, with the one state no word could be found for flagged rather than hidden; six creatures whose attack pose only shows under another flag now say which flag instead of carrying a still clip. The clips weigh a third of what they did, keyed the way the finished animations are (a smooth curve within one degree and a thirty-second of a block of the sampled motion), and every one of them opens inside the creature's Blockbench file, with the files beside the sheet remaining the source of truth.

Behind the dev switch, thirteen more creatures draw through the new renderer path on their classic motion: the Crab, whose eight-legged walk the port had been drawing as three legs and now draws as the twenty-four legs and claws the original showed, the Kyuubi, the Leaf Monster, the Alosaurus, the Attack Squid, the Band P, the Baryonyx, the Camarasaurus, the Cassowary, the Creeping Horror, the Cryolophosaurus, the Easter Bunny and the Flounder — fifty-one of the hundred and six. The Boyfriend and Girlfriend wait: they animate on the vanilla player rig, whose swing reads a moment between ticks that the new path does not yet carry, and that is the owner's call.

Behind the dev switch, fourteen more creatures draw through the new renderer path on their classic motion: the Pointysaurus, the Whale, the Molenoid, the Rat, the Spit Bug, the Stink Bug, the Trooper Bug, the Velocity Raptor, the Ghost Skelly (translucent, as the classic draws it), the Hydrolisc, the Lizard, the Mantis, the Cave Fisher and the Chipmunk — sixty-five of the hundred and six. The Lurking Terror waits on a harness question about its thin wings, not on anything in its rig. The Stinky's folded-legs pose now carries its name.

Behind the dev switch, twelve more creatures draw through the new renderer path on their classic motion: the Ender Knight, the Ender Reaper, the Frog, the Gazelle, the Nastysaurus, the Peacock, the Sea Viper, the Urchin, the Ostrich, the Spyro, the Stinky and the Triffid — seventy-seven of the hundred and six, and every Tier-2 creature but six is now in (the Dungeon Beast, the Lurking Terror and the Scorpion held, the Boyfriend, the Girlfriend and the Ghost awaiting the remainder). The Dungeon Beast waits on how the new path should carry a whole-body turn the classic renderer applies, and the Scorpion on the same thin-cube harness question as the Lurking Terror; both keep their classic renderer until then.

The renderer comparison that gates every creature learned to tell a genuine difference from a coin toss: where two faces sit within a hundred-thousandth of a block of each other on both renderers, a pixel that flips between them is counted and reported rather than failed, capped so that a rig which ties everywhere still fails. Under it the Lurking Terror and the Scorpion clear, ten creatures regain their rest-pose check, and no previously passing creature changes verdict. The new renderer path also learned to carry a whole-body turn the classic renderer applies before drawing, proven to the same tolerances on the Dungeon Beast and the Kraken, and to leave out parts a creature builds but never draws.

The new renderer path learned to carry a creature as a real chain of bones — each link's position and turn derived from its parent, proven link by link against the classic model on the Alien and the Emperor Scorpion to within a millionth of a block — but neither creature switches over yet: the new path draws a chain parent-first, and both classic models draw their chains in an order no parent-first walk can reproduce. That is the owner's question; until it is answered the two keep their classic renderer.

A quiet mistake that reached every converted creature is corrected at its root: the converter had been writing rigs mirrored left for right, and the comparison that gated them looked at both renderers in a frame where the mirror could not show. The converter now writes rigs in the same convention the Queen's hand-made rig uses, every converted rig, every clip and every proof was regenerated under it, and the comparison now reproduces the two renderers' real in-game chains, which agree exactly; the folder for the animator is rebuilt on the corrected rigs. The owner's own look at an asymmetric creature in the dev client, against its classic renderer, is the acceptance.

The Alien, the Alien Boss and the Emperor Scorpion now draw through the new renderer path as true chains of bones — each tail ring, leg segment and claw a child of the link before it, proven link by link against the classic model to within a millionth of a block — eighty of the hundred and six. Where the classic renderer drew a chain's links out of order, the new path draws them parent-first, and the picture is the same to the pixel.

Behind the dev switch, fourteen of the great creatures draw through the new renderer path on their classic motion: the Kraken and the Dungeon Beast, each carrying the whole-body turn its classic renderer applies, the Basilisk, Godzilla, the Hammerhead, the T-Rex, the Leon and Leonopteryx, the Cephadrome, the Dragon and its baby, the Giant Robot and Jeffery with their twice-drawn limbs standing as their own bones, the Pitch Black, the Sea Monster, the Prince and the Prince Adult — ninety-four of the hundred and six, every one still on its single classic hitbox. The King waits on the second, see-through pass that draws its wing membranes, and the four butterflies on one flat-pose sample of the Mothra that ties too many pixels for the rule.

Behind the dev switch, the Prince Teen and the Water Dragon draw through the new renderer path on their classic motion — ninety-six of the hundred and six; every Tier-1 creature is now through but the King, the four butterflies and the Princess, all waiting on the remainder.

Behind the dev switch, the remainder is in: the Lurking Terror and the Scorpion (their thin-cube ties now counted rather than failed), the Ghost with its see-through body, the King with the second, see-through pass that draws its wing membranes, and — the first creatures on the vanilla player rig — the Boyfriend and Girlfriend, whose swing now reads the same moment between ticks the classic renderer reads and whose armor, held item, head and elytra draw through the new path, together with the Princess and her spinning power orbs; a hundred and three of the hundred and six. The four butterflies alone keep their classic renderer, on one flat-pose sample of the Mothra that ties more pixels than the rule allows.

...
- [x] All `DeferredRegister` instances point to the correct `Registries.*` key.
- [x] `OreSpawnMod` constructor receives `IEventBus`, `ModContainer` (1.21.1
      signature). ✓
- [x] All `@EventBusSubscriber` annotations specify `bus = MOD` for setup events
...
- [x] `OreSpawnChunkGenerator` correctly extends `NoiseBasedChunkGenerator` and
      registers `CODEC` via `MapCodec`.
- [x] `applyBiomeDecoration` phase used for cross‑chunk structures (correct).
...
- ⚠ **`OreSpawnChunkGenerator.recentlyPlaced` (line 63) is a non‑volatile
  ...
  **Fix:** replace with `private static final AtomicInteger recentlyPlaced =
  new AtomicInteger(0);` and use `getAndDecrement()` / `compareAndSet()`. Same
...
- [x] `ModItems` uses `DeferredRegister.createItems()` and `register(...)`. ✓
- [x] All food items (1.21.1‑native) wired through `Item.Properties.food(...)`.
...
- [x] No `GL11.*` calls anywhere (only one comment in `TheQueenRenderer.java:23`).
- [x] All transparent/cutout rendering goes through `RenderType.cutout()` /

...
extends EntityAnimal {
    ...
    private boolean scan_it(int x, int y, int z, int dx, int dy, int dz) {
        ...
        for (i = -dy; i <= dy; ++i) {
            for (j = -dz; j <= dz; ++j) {
                ...
                if ((bid = this.field_70170_p.func_180495_p(new BlockPos(x - dx, y + i, z + j)).func_177230_c()) != Blocks.field_150349_c || (d = dx * dx + j * j + i * i) >= this.closest) continue;
                this.closest = d;
                this.tx = x - dx;
                this.ty = y + i;
                this.tz = z + j;
        ...
        for (i = -dx; i <= dx; ++i) {
            for (j = -dz; j <= dz; ++j) {
                ...
                if ((bid = this.field_70170_p.func_180495_p(new BlockPos(x + i, y - dy, z + j)).func_177230_c()) != Blocks.field_150349_c || (d = dy * dy + j * j + i * i) >= this.closest) continue;
                this.closest = d;
                this.tx = x + i;
                this.ty = y - dy;
                this.tz = z + j;
        ...
        for (i = -dx; i <= dx; ++i) {
            for (j = -dy; j <= dy; ++j) {
                ...
                if ((bid = this.field_70170_p.func_180495_p(new BlockPos(x + i, y + j, z - dz)).func_177230_c()) != Blocks.field_150349_c || (d = dz * dz + j * j + i * i) >= this.closest) continue;
                this.closest = d;
                this.tx = x + i;
                this.ty = y + j;
                this.tz = z - dz;
    ...
    private int findBuddies() {
        List var5 = this.field_70170_p.func_72872_a(Baryonyx.class, this.func_174813_aQ().func_72321_a(20.0, 10.0, 20.0));
        return var5.size();
    }
    ...
    public boolean func_70878_b(EntityAnimal otherAnimal) {
        ...
        Baryonyx baryonyx = (Baryonyx)otherAnimal;
        return this.func_70880_s() && baryonyx.func_70880_s();
    }
}

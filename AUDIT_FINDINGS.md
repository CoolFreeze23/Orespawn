...
- **Resolution:** FIXED (2026-09-04, owner: "ENT-S-116: go, one refuter"). Two one-line gates at the orig
  ...
  noAi, persistent) and discarded there: `s116_stinky_583_coal_ore_eat (renamed from s116_stinky_583_flower_eat when ENT-S-119 replaced the routine)` (activity 1 through the setter; rolls
...
### ENT-S-119 — The Stinky's idle block-eat is not the 1.7.10 routine: the port eats flowers on a one-sided slab where orig hunted coal ore over six-face shells, without the burp, while sitting, and without the idle attack pass (found by the ENT-S-116 refuter; FIXED 2026-09-04)
...
- **Resolution:** FIXED (2026-09-04, owner: "ENT-S-119 and 123 join the T5/T6 wave" — a parity bug in classic). The
  1.7.10 routine transcribed into `EntityStinky.java`: the fields `closest`, `tx`, `ty`, `tz` (orig Stinky.java:54-57 →
  :81-85); `scanIt` (orig :435-496 → :329-394, with `isCoalOre` :396-398 — `is(Blocks.COAL_ORE)`, the BeehiveFeature.java:38-39
  mapping of `field_150365_q`) probing the six faces of the (dx, dy, dz) shell in orig's order (+x :443, −x :451, +y :461,
  −y :469, +z :479, −z :487), each hit nearer than `closest` by squared distance from the scan origin taking it;
  `eatCoalOre` (orig :584-604 → :295-327) — `closest` reset to 99999, the shells i = 1, 2, 3, 4, 6, 8 around
  `((int) x, (int) y + 1, (int) z)` (the `if (i < 4) continue; ++i` skip of :594-595 kept verbatim, `j = min(i, 2)`), the
  first shell that finds anything breaking, `getNavigation().moveTo(tx, ty, tz, 1.25)` (:598), and under `closest < 12`
  (:599, from the ORIGIN) the block set to air with flag 2 (:600), `heal(1.0f)` (:601) and `PLAYER_BURP` at 0.5 / pitch
  `nextFloat * 0.2 + 1.5` (:602, the EntityGammaMetroid.scanForStone idiom) — `eatFlowers` (HEAD :290-308) removed. The
  ENT-S-116 gate line (activity → roll → flag) now sits inside the `!isOrderedToSit()` block after `doMovement()`
  (:286-291 — orig :511 reaches :582-583 only through `!isSitting()`), read after the pass so a flip to activity 2 skips
  the eat roll as orig :582 does. `doMovement` runs for every activity: the 1-in-300 retarget roll guarded by
  `activity == 2` (orig :552 → :411-412), the existing 1-in-7 idle attack pass (orig :568-581 → :422-442: the roll, then
  `!= PEACEFUL`, then `findSomethingToAttack`; activity 2, the flight target on the prey, the bite inside (3 + w/2)²) now
  reached ahead of the activity-1 return (:444-445), which replaces HEAD's early return at :314. Roll ledger per idle
  tick after: activity 1 — 200, 100, 100 (+20), 7, 50; activity 2 — 200, 100, 100 (+20), 300, 7, then the pick rolls —
  orig's order and bounds, of the entity random (ENT-S-093). Pinned by `StinkyIdleParityTests` (own batch
  `stinkyIdleParity`, TEST-003), a generator over 19 rows in the ENT-S-116 shape — the Stinky frozen at rel (20,1,24)
  of the 48x16x48 empty_large, 10 below max health, `closest` pre-set to a sentinel, its navigation swapped by
  reflection for a recording `GroundPathNavigation` (a never-grounded frozen mob refuses every real path), the port's
  `customServerAiStep` driven once by reflection under `ForcedRoll` pins beneath a `RollLog` that records the bound
  order, a `PlayLevelSoundEvent.AtPosition` listener on `NeoForge.EVENT_BUS` (the StructureTestsC seam; javap:
  `ServerLevel.playSeededSound` fires `EventHooks.onPlaySoundAtPosition` ahead of the broadcast) hearing the burp,
  `PLAY_NICELY` false for the drive and restored, blocks razed and spawns discarded in the finally: rows 01-05 coal ore on
  shell 1's +x and −y faces, shell 2's +z and +y faces, shell 3's −x face — found (closest 1 / 1 / 4 / 4 / 9), walked to
  at 1.25, eaten to air, healed 90→91, one burp at 0.5 / 1.5..1.7, the bounds [200, 100, 100, 7, 50]; 06 shell 4 (+x,
  distSq 16) found and walked to but standing, no heal, no burp; 07 / 09 a block on shell 5 / shell 7 only — never
  scanned (closest 99999, no walk); 08 / 10 shell 6 (−z, 36) and shell 8 (+x, 64) found and walked to, standing; 11 two
  coal ores on shell 2's +x face, the farther (distSq 9) scanned first and the nearer (4) taken and eaten, the farther
  standing; 12 a dandelion at the old port's site ignored (closest 99999, standing, no heal, no burp); 13 a sitting
  Stinky with coal ore in reach — the eat never runs (the sentinel untouched, the block standing, the bounds [200, 100]);
  14 the 1-in-7 pinned to fire with a frozen Zombie 3 east (a Monster — orig :668's EntityMob; a pig is never taken):
  activity 2 (field and synched), the flight target on the Zombie's block one up, the Zombie bitten, the bounds [200,
  100, 100, 7] (no eat roll after the flip); 15 the same under PEACEFUL (`MinecraftServer.setDifficulty`, restored):
  the roll spent, no scan — activity 1, target and Zombie untouched, the bounds [200, 100, 100, 7, 50]; 16 the roll
  missed: the same quiet outcome. Every row fails with its port line reverted (the old `eatFlowers` eats none of the
  coal and eats the dandelion; the early return drops the 7 from the bound order; the call outside the sitting block
  eats while sitting). `PlayNicelyGriefingGateTests`' Stinky row follows the routine — `s116_stinky_583_coal_ore_eat`
  (`StinkyCoalOreProbe`: a COAL_ORE one east of the scan origin, rolls 200→0, 100→0, 7→0, 50→0, eaten and healed with
  the flag off, standing with it on); the Metroid row untouched. javac rc 0 on the four files (no gradle in this lane).
  Refuted once, upheld: `scanIt` a probe-for-probe transcription, the shell sequence 1, 2, 3, 4, 6, 8 traced, the
...
- **Resolution:** FIXED (2026-09-04, owner: "ENT-S-119 and 123 join the T5/T6 wave" — a parity bug in classic). The
  second `canSeeTarget` call restored at the orig position of the flight-target acceptance in both files: orig
  Stinky.java:640 / Spyro.java:647 `if (bid == air && !canSeeTarget((double) x, (double) y, (double) z)) bid = stone` →
  port EntityStinky.doMovement :466-468 / EntitySpyro.doMovement :439-441 `if (getBlockState(newTarget).isAir() &&
  canSeeTarget(newTarget.getX(), newTarget.getY(), newTarget.getZ()))` — accept iff air and the ENT-S-118 feet ray (eye
  + 0.75 → the candidate's block corner, the int coordinates widened as orig widened them; OUTLINE / Fluid.NONE) reaches
  it; a refused candidate leaves `found` false and the loop trying, as orig's `bid = stone` did. Pinned by
  `StinkyIdleParityTests` rows 17 (`s123_17_stinky_640_flight_ray_wall`) and 18 (`s123_18_spyro_647_flight_ray_wall`):
  the flyer frozen in activity 2, untamed, `currentFlightTarget` set to its own block (so the 2.1 test forces a pick),
  the private `doMovement` driven by reflection with the pick pinned to the one candidate `((int) x + 6, (int) y + 1,
  (int) z + 6)` for all 50 tries (nextInt(5) → 0, nextInt(2) → 1 twice, the Stinky's nextInt(6) → 3 / the Spyro's
...
- **Evidence:** 1.7.10's `IMob` is an interface — `EntityMob` implements it, and so do `EntitySlime` (hence
  `EntityMagmaCube`), `EntityGhast`, `EntityDragon` and OreSpawn's own Mothra (orig Mothra.java:52) — and
  `IMob.mobSelector` (`field_82192_a`) is the `instanceof IMob` entity selector. Six OreSpawn target tasks filtered
  on it, each on an `EntityLiving.class` list: Dragon.java:116 `EntityAINearestAttackableTarget(this,
  EntityLiving.class, 0, true, false, IMob.mobSelector)` (registered when `PlayNicely == 0`, :115-117),
  Leon.java:93, ThePrinceAdult.java:113, ThePrinceTeen.java:117 (the same task, each behind the same gate), and
  Boyfriend.java:141 / Girlfriend.java:167 `MyEntityAINearestAttackableTarget(this, EntityLiving.class, 15.0f, 0,
  true, true, IMob.mobSelector)`, whose helper applies the selector as `selectEntitiesWithinAABB(targetClass, box,
  selector)` (MyEntityAINearestAttackableTarget.java:56). The port had mapped `IMob` to vanilla `Monster` at all
  six — `NearestAttackableTargetGoal<>(this, Monster.class, …)` at Dragon.java:155 (ENT-S-117's channel (a)),
  EntityLeon.java:164, ThePrinceAdult.java:149, ThePrinceTeen.java:160, Boyfriend.java:148, Girlfriend.java:213 —
  which drops every hostile that is no `Monster` subclass: Slime, MagmaCube, EnderDragon (all `IMob` in 1.7.10),
  the Ghast (refused in the port by `Mob.canAttackType` regardless) and orig's Mothra (an `AmbientCreature` in the
  port). What a player saw: a Leonopteryx, a Prince, a Boyfriend, a Girlfriend or a Dragon standing beside a slime,
  a magma cube or the Ender Dragon and ignoring it, where 1.7.10 went for it as for any monster. The remaining IMob
  tests in orig are identities and stand unchanged: the Boyfriend's / Girlfriend's `EntityCreeper.class` tasks
  (:138 / :164 — no port counterpart, ENT-A-054) and `MyEntityAIAvoidEntity.java:55` on the `EntityMob.class`
  lists of Chipmunk / Ostrich / VelocityRaptor (port `AvoidEntityGoal<Monster>`). Survey: T3c "IMob → Monster"
  (Leon, `phase_g_reports/targeting_survey_2026-09-04.md` :1529, :569), the Princes' §2 rows (:1008, :1036), the
  MyEntityAINearestAttackableTarget block (:1163), the Dragon's ENT-S-117 disclosure (:298, :1784). Refuter B of
  ENT-S-117 had recorded the literal alternative `Mob.class + instanceof Enemy` for the owner's ruling. No MOD record.
- **Resolution:** FIXED (2026-09-04 — owner: "Convention: `Mob` + `Enemy`, port-wide, wherever 1.7.10 tested IMob.
  Update the Dragon's channel accordingly"). Six port sites in six files, the goal re-typed `Mob` with the
  `instanceof Enemy` selector composed ahead of the site's own predicate, priorities / intervals / sight flags /
  the ENT-S-115 live `canUse` gate unchanged, one site comment each citing the orig line:
  ...
  | 6 | Girlfriend | Girlfriend.java:167 | Girlfriend.registerGoals :215, same | same imports |

  The vanilla `Enemy` marker is the 1.21.1 reading of 1.7.10's interface: every `Monster` implements it, and so do
  Slime, MagmaCube, Ghast, EnderDragon, Shulker, Phantom and the Hoglin family. The Ghast is still refused — by vanilla's `Mob.canAttackType` (Mob.java:253-255) — as 1.7.10 refused it through
  `EntityLiving.canAttackClass` = `cls != EntityCreeper.class && cls != EntityGhast.class` (client jar sha1
  e80d9b3b…, `sw.a(Class)`, settled by the refuter; no OreSpawn class overrides it); the same line refused the
  Creeper at the four vanilla-task sites (Dragon, Leon, both Princes), which the port's four goals take — pre-existing
  under HEAD's `Monster` form, outside this ruling, filed as ENT-S-127; at the Boyfriend / Girlfriend sites orig's
  `MyEntityAITarget` granted Creeper and Ghast explicitly, so the Ghast refusal stays the disclosed divergence there.
  1.21.1's Shulker (an AbstractGolem), Phantom (a FlyingMob) and Hoglin (an Animal) are Enemy without being Monsters —
  dropped before, taken now, no 1.7.10 counterpart; Zoglin is a Monster and was taken already; orig
  MyEntityAINearestAttackableTarget.java:44-52's untamed-or-sitting gate on the Boyfriend / Girlfriend goal stays
  absent on the port goal (a T3c residual noted on the ledger row). Disclosed, unchanged: orig's Mothra (`implements IMob`) is still no
  prey of the six goals (the port Mothra is an `AmbientCreature` with no `Enemy`) — the ENT-S-117 disclosure
  stands, `|| e instanceof Mothra` would restore her; the Boyfriend's / Girlfriend's helper residuals
  (MyEntityAITarget.java:96-116 — PigZombie / Enderman refused, Ghast / Mothra granted; the per-goal 15 / 20 boxes;
  the dropped Creeper task) and the T3c box / cadence widening (Leon 16 → 40, the Princes 16 → 64 / 32, every tick →
  1-in-10) are not this ruling's and stay open where the ledger lists them. Pins: new `IMobConventionTests` (own
  batch `imobConvention`, TEST-003; a `@GameTestGenerator` over a 25-row table in orig file order,
  `imobconventiontests.s124_NN_<species>_<prey>`): per site the goal read off the target selector by
  `targetType == Mob.class` (no `Monster`-typed goal may remain) and asked `canUse()` under the ForcedRoll seam
  with a Slime 8 blocks off taken and read back as the goal's pick (an Enemy that is no Monster — the row the old
  form fails), a Zombie taken, a pig refused, the Slime refused with PlayNicely flipped on (the ENT-S-115 gate
  composes; flag restored in a finally), and for the Leon its tame rule composing (tamed and holding a target the
  Slime is refused, the slot emptied she is taken); `PlayNicelyGateParityTests`' five GoalProbe rows (Boyfriend,
  Girlfriend, Leon, ThePrinceAdult, ThePrinceTeen) now read `Mob.class`; `ProactiveHuntParityTests`' Dragon rows
  read `Mob.class`, and the shape row (`s3a_20_dragon_116_goal_registered_priority_range`) asserts the new shape —
  `Mob` type, no `Monster` goal left, the `TargetingConditions` selector present and answering true for a Slime,
  false for a pig — with its priority / revenge-order / interval / follow-range assertions kept. Synchronous; flags
  restored in a finally; spawns discarded; no mock players; no documenting-only test. Compiled (javac, §7); the
  gametest run is the gate's. Refuted once, upheld: all six hunks the ruled form with HEAD's interval / mustSee / mustReach, the sweep exhaustive
...
- **Evidence:** orig Stinky.java:638-641 and Spyro.java:645-648 assign the candidate to the flight target BEFORE the
  ...
  to about 300 ticks (the :552 retarget clock) and pays the fifty rays once per retarget. The port
  (EntityStinky.java:466-468, EntitySpyro.java:439-441, the pick loop around them) assigns only on acceptance and
  keeps the prior, already reached target, so `closerToCenterThan(2.1)` (EntityStinky :447; Spyro :411-446 while
  `getActivity() != 3`) is true again next tick and the fifty-try pick — since ENT-S-123 fifty `level().clip` rays —
  re-runs EVERY tick for a boxed-in flyer (a caged pet, a flyer in a small room). Orig :548-551's null-initialised
...
- **Status (2026-09-06):** FIXED (2026-09-06, wave 4 — Leon's IMob hunt @3 ahead of its revenge goal @4 in both modes (EntityLeon.java:176 / :197; orig Leon.java:93's @1 over :95's @2, the modern-only owner pair keeping @1 / @2); the Girlfriend's IMob hunt @3 in both modes (Girlfriend.java:264; orig :167), ahead of the Jealousy tasks @4 / @5 — the modern-only owner pair moved @3 / @4 → @1 / @2 (Girlfriend.java:229-230): the Leon precedent applied to keep MOD-033's promise — at @3 the pair tied with the hunt, and `WrappedGoal.canBeReplacedBy` needs a strictly lower priority, so a running hunt was never pre-empted by `OwnerHurtByTargetGoal` and a hunting Girlfriend ignored her owner being hit for the hunt's duration (the wave-4 refuter A); at @1 / @2 the pair sits strictly ahead of both hunts and the Jealousy tasks and ties with the two Valentine tasks (orig :161-162), which run only on a valentine-angry Girlfriend and, registered first, start first on a same-pass tie; the owner may still move it; the `mod033_companions_defend_owner_*` expected selector (PortOnlyTargetingTests COMPANIONS — the Girlfriend's classic `3:MyEntityAINearestAttackableTargetGoal<Mob>`, the modern-only `1:OwnerHurtByTargetGoal` / `2:OwnerHurtTargetGoal`) and the IMobConventionTests descriptions updated; pinned `TargetingWave4ParityTests` s130_01 (a Zombie in the box displaces a Pig attacker the revenge goal held, and wins the same pass) / s130_02 (a Zombie displaces the untamed rival the Jealousy goal held, and wins the same pass))
- **Ratified (owner, 2026-09-06, item 25):** the Girlfriend's modern owner pair at priorities 1 and 2 on the Leon precedent.
...
- **Ruled (owner, 2026-09-05): parity, classic — wave 4**, one batch with ENT-S-130 / 133 / 134 / 137 / 138 / 142 /
  ...
  paragraph. HELD behind the Phase G chain's no-look slices (addendum item 23 (8)): the batch gates only when a chain
  slice is waiting on the owner.
- **Status (2026-09-06):** FIXED (2026-09-06, wave 4 — the 1-in-50 rolled BEFORE `e = getAttackTarget()` at the four sites (Robot2.java:214-215, Robot3.java:147-148, Robot4.java:195-196, Robot5.java:134-135; orig :281-284 / :242-245 / :282-285 / :214-217): the forgetting pass reads null, its scan refuses the attacker and engages nothing — no look, no path, no attack pose, no laser; pinned s133_01-04 (the pass under a forced roll engages nothing; the control under a quiet roll engages the held Zombie — the rows discriminate through the path, the attack pose and the LaserBall; the yaw signal is inert, −90 already facing the Zombie on +x); the T5b rows s131_01-03 unchanged)
...
- **Ruled (owner, 2026-09-05): parity, classic — wave 4**, one batch with ENT-S-130 / 133 / 134 / 137 / 138 / 142 /
  ...
  paragraph. HELD behind the Phase G chain's no-look slices (addendum item 23 (8)): the batch gates only when a chain
  slice is waiting on the owner.
- **Status (2026-09-06):** FIXED (2026-09-06, wave 4 — orig Stinky.java:617-631's owner branches in `EntityStinky.doMovement` (:472-486): the owner on the ground `nextInt(4) + 6`, the owner flying `nextInt(8)`, ownerless `nextInt(5) + 6`, the target at the owner's truncated cell as before; the dice order (xdir ahead of zdir, orig zdir first) upheld as ruled — noted in the entry, not changed; pinned s134_01-03 (each branch's box under pinned dice, the ownerless roll pinned too so HEAD's box is told apart))
...
- **Ruled (owner, 2026-09-05): parity, classic — wave 4**, one batch with ENT-S-130 / 133 / 134 / 137 / 138 / 142 /
  ...
  paragraph. HELD behind the Phase G chain's no-look slices (addendum item 23 (8)): the batch gates only when a chain
  slice is waiting on the owner.
- **Status (2026-09-06):** FIXED (2026-09-06, wave 4 — orig MyEntityAINearestAttackableTarget.java:44-52's refusals at the head of both hunts' `canUse`, ahead of the PlayNicely read (Boyfriend.java:175 / :188 — `!isTame()`, the :44-46 tameable test; Girlfriend.java:249-250 / :267-268 — `!isTame()` for :44-49 and `isOrderedToSit()` for :50-52's `isSitting`, the Jealousy goals' idiom — the mechanism-faithful read would be `isInSittingPose()`, coincident on the server today); the Valentine tasks untouched (MyValentineTarget.java:47-59 carries none); the ledger :1172 row re-rated from MATCH; the companion pins re-based on a tamed, standing companion (ScanSetParityTests s135_33-38 / s135_41 through `spawnCompanion`, TargetReleaseParityTests s129_39 / s129_40, PlayNicelyGateParityTests' companion GoalProbe sites, PreyListParityTests' companion GOAL rows, IMobConventionTests' Boyfriend / Girlfriend rows; TargetSorterParityTests already tamed) — the pinned values unchanged; pinned s137_01-05 (an untamed Boyfriend / Girlfriend refuses a Zombie and a Creeper on both hunts, tamed takes — the IMob hunt's pick the sorter's Creeper, its distance² halved to 34 over the Zombie's 64 (ENT-S-139, s139_82), the Creeper task's the Creeper; a sitting tamed Girlfriend refuses, standing takes; a sitting tamed Boyfriend still hunts; the untamed Girlfriend's Valentine Player task still picks under the Feb-14 clock))
...
- **Ruled (owner, 2026-09-05): parity, classic — wave 4**, one batch with ENT-S-130 / 133 / 134 / 137 / 138 / 142 /
  ...
  paragraph. HELD behind the Phase G chain's no-look slices (addendum item 23 (8)): the batch gates only when a chain
  slice is waiting on the owner.
- **Status (2026-09-06):** FIXED (2026-09-06, wave 4 — five sites cast as orig: the four named — EntityBrutalfly.java:145 / :148 (orig :172 / :174), Mothra.java:394 / :397 (orig :182 / :184), entity/ai/AmbientFlightGoal.java:109 / :112 through a new `castCell(Mob)` (:180-182; orig Dragonfly.java:122 / :124 — the base cast reaches the Dragonfly, the Firefly (orig Firefly.java:125 / :127), the Mosquito (orig EntityMosquito.java:94 / :96) and the VampireButterfly (the port's own class for orig EntityButterfly's vampire type, on the base goal)), entity/ai/DragonflyHuntGoal.java:91 (orig :145-146, `(int) (posY + 1.0)`) — and a fifth found by the lane and closed in the same batch, presented for ratification: entity/ai/ButterflyIslandsHuntGoal.java:67 / :69, a SEPARATE site (its `tick` never reaches the base's :109 / :112; it closes orig EntityButterfly.java:152 / :154 and, through LunaMothFlightGoal, EntityLunaMoth.java:124 / :126); the rows s135_43 and s129_41 re-derived on the cast cell; pinned s138_01-05 at a fractional negative y (rel 1.5 over the floor at rel 0; the rows on the integer x / z lattice — rel 20.0 / 24.0 — so the cast and the floor agree on x / z at any origin, the harness's x / z being random per run, and y alone discriminates): a parked flight target inside the near band by the cast and outside it by the floor decides the retarget, and the reverse, for the Brutalfly, Mothra, the Dragonfly's goal (the base cast, s138_03) and the butterfly's copy (s138_05); the Dragonfly's prey cell read back at `(int) (posY + 1.0)` (s138_04))
- **Ratified (owner, 2026-09-06, item 25):** the fifth site (the butterfly goal's own copy of the base lines) closed in the wave-4 batch.
...
- **Ruled (owner, 2026-09-05): parity, classic — wave 4**, one batch with ENT-S-130 / 133 / 134 / 137 / 138 / 142 /
  ...
  paragraph. HELD behind the Phase G chain's no-look slices (addendum item 23 (8)): the batch gates only when a chain
  slice is waiting on the owner.
- **Status (2026-09-06):** FIXED (2026-09-06, wave 4 — `if (this.isInWaterRainOrBubble() || this.isOnFire())` at EnderKnight.java:259, the Reaper's line (EnderReaper.java:258), with the :116-119 cite; pinned s142_01 (a frozen Knight with a water source at its feet, `aiStep()` once under the pinned random: 4 east, 4 south, the scream off; dry, nothing) and the Reaper's twin s142_02)
...
- **Ruled (owner, 2026-09-05): parity, classic — wave 4**, one batch with ENT-S-130 / 133 / 134 / 137 / 138 / 142 /
  ...
  paragraph. HELD behind the Phase G chain's no-look slices (addendum item 23 (8)): the batch gates only when a chain
  slice is waiting on the owner.
- **Status (2026-09-06):** FIXED (2026-09-06, wave 4 — `Params.lunaMoth()` = (10, 6, 2, 0.5, 0.68, 0.1, 0.75f, 1.0f, 100, 4.0, 25) (AmbientFlightGoal.java:69-75; orig :118, :126, :129, :149-154); the torch scan in `LunaMothFlightGoal.onRetargetSkipped` (:72-89) under `!level().isDay() && nextInt(10) == 0` (orig :133, the OriginalSpawnGates.isDaytime mapping) on every tick the retarget did not fire, reached from ButterflyIslandsHuntGoal.tick's else branch after the inherited Islands hunt (:74-92 — orig ran the butterfly loop ahead of the moth's; the hook a no-op for the butterfly and Mothra); the scan itself orig :54-115 `scan_it` face for face (`scanIt` :97-155 — six faces; HEAD's `findClosestTorch` read the ±x faces alone, an extension flagged in the records) over TORCH + WALL_TORCH + the Extreme Torch (`isTorch` :161-166); the single flight target and one retarget roll for orig's two loops kept (the ENT-S-141 shape, disclosed); LunaMothFlightGoal.java rewritten, 167 lines; pinned s143_01 (the preset), s143_02 (night, open sky, a non-retarget tick: the torch 5 east sought), s143_03 (day: not), s143_04 (a torch on the +z face found), s143_05 (a retarget tick wanders, never the torch))
...
- **Ruled (owner, 2026-09-05): parity, classic — wave 4**, one batch with ENT-S-130 / 133 / 134 / 137 / 138 / 142 /
  ...
  paragraph. HELD behind the Phase G chain's no-look slices (addendum item 23 (8)): the batch gates only when a chain
  slice is waiting on the owner.
- **Status (2026-09-06):** FIXED (2026-09-06, wave 4 — `enderTeleportTo(x, y, z)` (EnderKnight.java:308-318 / EnderReaper.java:307-317; named apart from the engine's `Entity.teleportTo(double, double, double)`) shared by `teleportRandomly` (:294 / :293) and `teleportToEntity` (:336 / :335): the origin read ahead of the move (orig :162-164), the landing through `randomTeleport`, and on a true return `SoundEvents.ENDERMAN_TELEPORT` (1.0f, 1.0f) at the origin through `level().playSound(null, x0, y0, z0, …, HOSTILE, …)` (orig :203 `playSoundEffect`) and at the entity through `playSound(…)` (orig :204) — nothing on a refused landing (orig :188-191); the mapping: 1.7.10's "mob.endermen.portal" → the modern event `minecraft:entity.enderman.teleport` (`SoundEvents.ENDERMAN_TELEPORT`, the same event vanilla 1.21.1's `EnderMan.teleport` plays at both ends); pinned s144_01 / s144_02 (a `PlayLevelSoundEvent` ear hears it once at the origin and once at the landing on a landed random teleport, nothing on a refused one — a spot 32 below the floor))
...
- **Ruled (owner, 2026-09-05): parity, classic — wave 4**, one batch with ENT-S-130 / 133 / 134 / 137 / 138 / 142 /
  ...
  paragraph. HELD behind the Phase G chain's no-look slices (addendum item 23 (8)): the batch gates only when a chain
  slice is waiting on the owner.
- **Status (2026-09-06):** HELD (2026-09-06, wave 4 — transcribed and withdrawn before the gate: the static `AttributeModifier` (`orespawn:attacking_speed_boost`, `(double) 6.2f`, ADD_VALUE) and the :100-107 swap at the head of `aiStep` were transcribed exactly, but 1.7.10's legacy mover saturated the number — `EntityCreature.updateEntityActionState` wrote the movement-speed attribute into `moveForward`, and `moveFlying` normalised the input to length ≤ 1 and scaled it by a non-AI-enabled mob's fixed 0.1f `getAIMoveSpeed` — to a ≈3.1× sprint (≈1.4 → ≈4.4 blocks/s), while 1.21.1's mover (`MoveControl.tick` → `Mob.setSpeed(attribute)`, `getFrictionInfluencedSpeed`, `getInputVector` scaling by the unclamped speed) would accelerate the same number at ≈6.52 blocks/tick toward ≈14 blocks/tick. The register's resolution reserved the value choice, so the owner rules between: a literal 6.2 (≈14 blocks/tick under the modern mover); the value that reproduces the saturated ≈3.1× (ADD_VALUE ≈ 0.67 on the 0.32 base, or MULTIPLY_BASE ≈ 2.1); vanilla's EnderMan re-tune 0.15; and the pre-existing question of the base speeds of every legacy-AI species under the modern mover — HEAD's un-boosted Knight already moves at ≈4.4 blocks/s, 1.7.10's boosted pace (filed as ENT-S-150, raised by the wave-4 refuter B). The hunks are out of the tree (EnderKnight.java / EnderReaper.java carry ENT-S-142 / 144 alone), the s145 pins withdrawn (`TargetingWave4ParityTests` 28 rows), the MiscTargetingParityTests `spawnLive` re-base reverted with them)
- **Ruled (owner, 2026-09-06, item 23):** re-applied with the value the ENT-S-150 survey decides (the saturated ≈3.1× sprint), after the survey; not the literal 6.2.
...
- **Evidence:** GeckoLib 4.8.4 bytecode (javap over the pinned jar). A replacement is a `GeoReplacedEntity`, i.e. a
  `SingletonGeoAnimatable`, whose `animatableCacheOverride()` returns `new SingletonAnimatableInstanceCache(this)`
  (0-8); `GeckoLibUtil.createInstanceCache(GeoAnimatable)` hands that override back at offsets 1-12, so its
  Entity / BlockEntity branch (15-35: singleton when neither, instanced otherwise) is never reached for a
  replacement (refuter A, D3 — the same class either way). So every replaced species renderer held a singleton cache:
  `protected final Long2ObjectMap<AnimatableManager<?>> managers` (a `Long2ObjectOpenHashMap`, constructor 6-13) with
  `getManagerForId(long)` = `if (!managers.containsKey(id)) managers.put(id, new AnimatableManager(animatable));
  return managers.get(id);` (0-48). No GeckoLib code path removes an entry. The key is the entity id
  (`GeoReplacedEntityRenderer.getInstanceId` = `currentEntity.getId()`, 0-8; the draw-time lookup is
  `GeoModel.handleAnimations`, `getAnimatableInstanceCache()` at 6 and `getManagerForId` at 12). An `AnimatableManager`
  holds `boneSnapshotCollection` (`Map<String, BoneSnapshot>`, one per animated bone — nine for the Beaver),
  `animationControllers`, `extraData` and three timing fields (constructor 0-58 runs `registerControllers`). So on a
  client, every entity id ever drawn by any of the twelve replaced renderers (Beaver, Coin, Elevator, Island,
  IslandToo, Robot1-5, RockBase, Vortex — `PhaseGDevRenderers`, dev switch) kept its manager until the JVM exited;
  the Slice 2 review recorded it as a cost multiplier before the pattern reaches 100+ species. `TheQueen` is not
  affected (an Entity animatable on an `InstancedAnimatableInstanceCache`: one manager per entity instance,
  collected with the entity).
- **Resolution:** FIXED (2026-09-05, Phase G slice (a); owner's sequencing ruling, scope addendum item 23 (8)(a),
  superseding addendum C.8). Four pieces, no draw-path change, no mixin change, `TheQueen` untouched:
  (1) `entity/client/OreSpawnAnimatableInstanceCache` extends `SingletonAnimatableInstanceCache` — the class GeckoLib
  would pick, made explicit — with `evict(long)` (`managers.remove(id) != null`), `evictAll()` (count, then clear) and
  `size()`; eviction is safe because the next draw of that id re-creates a manager through `getManagerForId` and pays
  one first-tick snapshot, exactly what a never-seen entity pays. (2) `OreSpawnGeoReplacement` builds that cache
  explicitly and exposes it (`animatableCache()`); `OreSpawnGeoReplacedEntityRenderer`'s constructor registers it —
  at renderer construction on a bootstrapped client, never in the replacement's constructor, which the headless s4
  probe (`S4CandidateRuntime.instantiate`) runs without a bootstrapped registry (R0 below) — with
  (3) `entity/client/GeoReplacementCaches`, a registry keyed by `EntityType` (register replaces — renderers are
  rebuilt on every resource reload and the old renderer, singleton and cache drop together; the previous cache is
  returned; `unregister`, `cacheFor`, `evict(Entity)` by `getType()` + `getId()`, `evictAll()`, `managersHeld()`);
  it loads on the game-test server (no client class referenced). (4) `client/GeoReplacementCacheEvictor`, an
  `@EventBusSubscriber(modid, value = Dist.CLIENT)` on the game bus (the `KeybindHandler` idiom; no `bus =`):
  `EntityLeaveLevelEvent` → `evictIfClient(event.getLevel(), event.getEntity())`; `LevelEvent.Unload` →
  `evictAll()` when the level `isClientSide()`; `ClientPlayerNetworkEvent.LoggingIn` → `evictAll()` (refuter A, D1:
  `Minecraft.clearClientLevel(Screen)` — reached from `ClientPacketListener.handleConfigurationStart` at 55, a
  server-driven transfer / reconfigure — nulls the level (67) and calls `updateLevelInEngines(null)` (72) without a
  bus post, and the next `setLevel` then posts nothing; `ClientHooks.firePlayerLogin` (0-13) posts `LoggingIn` from
  `handleLogin` at 316, after `setLevel` at 181, so the dropped level's managers go before the new level's first
  draw; a no-op after an ordinary login). The static core `evictIfClient(Level, Entity)` returns false without
  touching anything unless `level.isClientSide()`.
- **Hook sites (NeoForge 21.1.223 bytecode; a correction to the slice brief):** the client-side removal hook is
  `ClientLevel$EntityCallbacks.onTrackingEnd(Entity)` — `Entity.unRide` (1), `players.remove` (12),
  `Entity.onRemovedFromLevel()` (19), then `new EntityLeaveLevelEvent(entity, level)` posted on `NeoForge.EVENT_BUS`
  (22-37), then the part-entity map cleanup (43-97); `onDestroyed(Entity)` is a bare `return` (0) in this build and
  posts nothing. The level hook has two sites in `Minecraft`: `setLevel(ClientLevel, ReceivingLevelScreen$Reason)`
  posts `new LevelEvent$Unload(this.level)` for the level being replaced (7-26, guarded `ifnull 27`, ahead of
  `updateLevelInEngines` at 57) and `disconnect(Screen, boolean)` posts it for the level being dropped (130-149,
  guarded `ifnull 204`, ahead of `updateLevelInEngines(null)` at 211).
- **Thread argument:** the cache map is a non-concurrent `Long2ObjectOpenHashMap` and every touch is on the client main
  thread. The draw: `GeoModel.handleAnimations` (6 / 12) runs inside the entity render pass, which
  `Minecraft.runTick(boolean)` drives through `GameRenderer.render` (offset 422) on the same thread as `tick()` (171)
  and `runAllTasks()` (120). The per-entity eviction: `ClientPacketListener.handleRemoveEntities` first calls
  `PacketUtils.ensureRunningOnSameThread(packet, this, minecraft)` (6), then `ClientLevel.removeEntity(int,
  RemovalReason)` (`Entity.setRemoved` at 20 → `TransientEntitySectionManager$Callback.onRemove` → the callbacks'
  `onTrackingEnd`). The level eviction: `Minecraft.setLevel` / `disconnect` are `Minecraft` methods on its own thread.
  The one foreign thread that sees the same events is an integrated server: it posts `EntityLeaveLevelEvent` for its
  server entities (same entity ids) and `LevelEvent.Unload` per server level on the SERVER thread; the evictor's
  `isClientSide()` guard returns before touching anything. The registry's own map is a `ConcurrentHashMap`
  (registration at renderer construction, on the client main thread during a resource reload; reads at eviction).
- **Measurement:** two names in the MHLib counters dump (`-Dmhlib.counters=true`, one INFO line per 100 client ticks,
  `MHLibClient.onClientTick`), registered by `GeoReplacementCaches` through two new `MHLibCounters` factories —
  `counter(String)` (the constructor stays private) and `gauge(String, LongSupplier)`; `sumAndResetAll()` appends every
  gauge's current value after the counters, in registration order, without resetting it; `formatDump` unchanged; the
  nine existing names and their order unchanged (pinned). `orespawn.geo.evictions`: managers evicted in the interval
  (one per replaced-renderer entity that left the client level, plus every manager dropped by a level unload;
  increments guarded by `ENABLED`). `orespawn.geo.managers_held`: the gauge, managers currently held by every
  registered replacement cache. Expected reading: with N replaced-renderer mobs in view `managers_held = N` (the
  number drawn and still in the level); walk them out of tracking range or kill them and the next dump shows
  `evictions = N`, `managers_held = 0`; disconnect and `managers_held` reads 0 on the title screen (a server transfer
  reads 0 once the new login completes). `orespawn.geo.evictions`' increments are unpinnable in the suite
  (`ENABLED` is false there; refuter A, D6) — the owner's look reads them. Before this slice
  the gauge (had it existed) could only ever rise: one manager per entity id ever drawn, for the session. The client
  evictor's static initialiser forces the registration, so both names print from mod construction on, even with every
  species on its classic renderer.
- **Pins:** new `GeoCacheEvictionTests` (own batch `geoCacheEviction`, TEST-003; a `@GameTestGenerator` over five
  synchronous rows `geocacheevictiontests.opt029_NN_<row>`, `orespawn:empty_large` named in full, 100 ticks; every
  registration a row makes restored in a finally, every spawn discarded there; frozen mobs on the floor at rel y 0,
  F0.7): 01 the cache on a real `BeaverGeoReplacement` — the same manager instance for a repeated id, 100 ids → size
  100, `evict(37)` true → 99, again false, the evicted id re-created as a new instance on the next draw (→ 100, evicted
  again → 99), `evictAll()` 99 → 0, then 0; 02 the registry — twenty frozen Beavers "drawn" through
  `getManagerForId(getId())`, `managersHeld()` 20 above the base and the value `sumAndResetAll()` prints for
  `orespawn.geo.managers_held` equal to it, the dump's key order (the nine MHLib names first in declaration order,
  `orespawn.geo.evictions` after them, the gauge after every counter), `evict(beaver)` true for each → held back to
  base and the gauge following, once more → false; 03 a pig (no cache) — `evict` false, nothing thrown, held
  unchanged; 04 Beaver + Coin replacements registered, five managers each — `evictAll()` returns base + 10, held 0,
  both caches empty; 05 the guard — a Beaver with a manager, `GeoReplacementCacheEvictor.evictIfClient(serverLevel,
  beaver)` false with nothing touched, then `GeoReplacementCaches.evict(beaver)` true. Assertion messages carry the
  finding id, the expected and the actual values. Compile proof: javac of the whole main tree and the gametest tree
  rc 0 (no gradle in this lane); the gametest run is the gate's (expected `All 1199 required tests passed`:
  1194 + these 5).
- **R0 (orchestrator's probe run, before the refuters):** the lane's first shape registered the cache from the
  replacement's constructor through `descriptor.entityType()`; the headless s4 geo probe run directly on the compiled
  classes died with `IllegalArgumentException: Not bootstrapped (… minecraft:game_event)` from `ModEntities.<clinit>` via
  `CoinGeoReplacement.lambda$static$0` ← `GeoReplacementDescriptor.entityType` ← `OreSpawnGeoReplacement.<init>` — the
  4b record's reason for the lambda suppliers. Moved to the renderer constructor; the probe re-run passes.
- **Not done / disclosed:** no in-game capture (client rendering is outside the suite's reach by construction); the
  three client listeners are pinned by their bytecode cites and the static core, not by a `ClientLevel`; the
  production registration (`OreSpawnGeoReplacedEntityRenderer`'s constructor, a client class) is unpinned in the
...
- **Closed (owner, 2026-09-06, Q8 (b)):** a recorded divergence — PN-023; no renderer plumbing for what sits behind the pause menu; the hook keeps its shape for the day a per-render effect has a live signature.

### ENT-S-148 — The Dragonfly's hurt re-aim floors where 1.7.10 cast `(int)`: `EntityDragonfly.hurt` sets the flight target on `attacker.blockPosition()` (REPORT, 2026-09-06; found by the wave-4 lane reading ENT-S-138's sites)
- **Evidence:** orig Dragonfly.java:178-185 `attackEntityFrom` — :182 `currentFlightTarget.set((int) e.posX, (int) e.posY, (int) e.posZ)` for the damage source's entity; port EntityDragonfly.java:89-97 `hurt` — :94 `this.huntGoal.setFlightTarget(attacker.blockPosition())` (`Mth.floor` per axis). The BUG-027 / MOD-024 class ENT-S-138 closed at five sites (the Brutalfly's and Mothra's self cells, `AmbientFlightGoal.castCell` for the base flyers, `DragonflyHuntGoal`'s prey cell, `ButterflyIslandsHuntGoal`'s own copy); this one was not on its list. Player-visible only below zero on an axis, one block at most, in where a hurt Dragonfly turns.
- **Resolution:** REPORT — `new BlockPos((int) attacker.getX(), (int) attacker.getY(), (int) attacker.getZ())` at the site, one pin at a fractional negative y (the s138_04 shape). Effort XS. The ledger's Dragonfly "other" row (:322, `hurt → flight target := attacker (:178-184)` / `hurt → setFlightTarget(attacker) (:90-96)`) re-rated DIVERGES from MATCH.
- **Ruled (owner, 2026-09-06, item 22):** parity, classic — one XS batch with ENT-S-149 and ENT-S-151, one refuter; after ENT-S-146's 4c work and the (e) records.

### ENT-S-149 — The Dragonfly's vertical steering blend is 0.3 where 1.7.10 blended y by 0.2f (REPORT, 2026-09-06; found by the wave-4 lane reading the moth's numbers)
- **Evidence:** orig Dragonfly.java:155-157 — :155 `motionX += (signum * 0.5 − motionX) * 0.30000000149011613` ((double) 0.3f), :156 `motionY += (signum * 0.7f − motionY) * 0.20000000149011612` ((double) 0.2f), :157 `motionZ` as x; port entity/ai/AmbientFlightGoal.java:76-77 `Params.dragonfly()` = (10, 5, 2, 0.5, 0.7, **0.3**, 1.0f, 4.0f, 300, 2.1, 50) — one `blend` for all three axes, applied to y at :128 (`my = motion.y + (signum(dy) * steerY − motion.y) * blend`). The Dragonfly climbs and dives toward its flight target half again as fast as 1.7.10's. The other presets' blends match their orig on all axes (the butterfly's and moth's 0.1f, the firefly's and mosquito's per their files — not re-read by this lane beyond the retarget lines).
- **Resolution:** REPORT — a `blendY` in `Params` (or the Dragonfly's own y blend 0.2) at the orig position; a pin on one tick's `deltaMovement.y`. Effort S. Observation folded in: the presets carry `double` literals (0.1, 0.7, 0.3) for orig's `(double) 0.Nf` — a 1.5e-9 difference per tick, no signature; listed, not proposed.
- **Ruled (owner, 2026-09-06, item 22):** parity, classic — the XS batch with ENT-S-148 and ENT-S-151, one refuter.

### ENT-S-150 — The legacy-AI movement mapping: 1.7.10's updateEntityActionState fed the movement-speed attribute into a clamped forward input and moved at a fixed 0.1f AI speed, so the Ender pair's +6.2 attacking boost saturated to a ≈3.1× sprint; the modern mover scales speed by the unclamped attribute, so the same number would run ≈14 blocks/tick and HEAD's un-boosted Knight already moves at 1.7.10's boosted pace (REPORT, 2026-09-06; raised by the wave-4 refuter B on ENT-S-145)
- **Evidence:** orig EnderKnight.java:29-30 / EnderReaper.java:29-30 — the `attackingSpeedBoostModifier` (UUID 020E0DFB-…, `(double) 6.2f`, operation 0, unsaved) added to the movement-speed attribute at :100-105 while a target is held — the 1.7.10 vanilla Enderman's own numbers, on mobs that never enabled the new AI. In Mojang's 1.7.10 client jar the legacy mover reads the attribute only through `EntityCreature.updateEntityActionState` (`td.bq`, bytecode 552-566): the attribute value is written into `moveForward`; `EntityLiving.moveEntityWithHeading` (`sv.e(FF)`) hands `moveStrafing` / `moveForward` to `Entity.moveFlying` (`sa.a(FFF)`, bytecode 18-50), which normalises the input pair to length ≤ 1 and scales it by the mob's `getAIMoveSpeed` (`sv.bl()` → for a non-AI-enabled mob `sw.bk()`'s fixed 0.1f) — so the attribute's 6.2 (0.3 base + 6.2 = 6.5) is clamped to a unit input at the fixed 0.1f, and the sprint is the friction-limited ≈3.1× of the un-boosted walk (≈1.4 → ≈4.4 blocks/s). NeoForge 21.1.223's mover has no clamp: `MoveControl.tick` sets `Mob.setSpeed(speedModifier × attribute)`, `LivingEntity.getFrictionInfluencedSpeed` returns the attribute-scaled speed, and `Entity.getInputVector` multiplies the unit input by that speed — a transcribed 6.2 (0.32 base + 6.2 = 6.52) accelerates at ≈6.52 blocks/tick toward a friction-limited ≈14 blocks/tick. Vanilla 1.21.1's own `EnderMan` re-tuned the same modifier to `(double) 0.15f` for this mover. The un-boosted comparison: HEAD's Knight (MOVEMENT_SPEED 0.32 under the modern mover) already walks at ≈4.4 blocks/s — 1.7.10's BOOSTED pace — which is the pre-existing, wider question of every legacy-AI species' base speed under the modern mover (the same `getAIMoveSpeed` saturation applied to their walks).
- **Resolution:** REPORT — two items for the owner. (i) The Ender boost's value (ENT-S-145, transcribed and HELD before the wave-4 gate): a literal 6.2 (≈14 blocks/tick under the modern mover); the value that reproduces the saturated ≈3.1× (ADD_VALUE ≈ 0.67 on the 0.32 base, or MULTIPLY_BASE ≈ 2.1); vanilla's EnderMan re-tune 0.15; or hold it behind (ii). (ii) A read-only survey of every legacy-AI species' base speed under the modern mover (which species never enabled the new AI in 1.7.10, their attribute bases, the fixed 0.1f walk they actually moved at, and what the port's attribute gives them today), proposed as its own lane before any per-species re-tune. Effort: (i) XS once ruled (the withdrawn hunks re-apply with the number; the s145 pins return); (ii) M, read-only.
- **Ruled (owner, 2026-09-06, item 23):** (ii) first — the read-only survey of every legacy-AI species' movement speed under the modern mover as its own lane (the boost is one case of a class); (i) after it — the value that reproduces 1.7.10's saturated ≈3.1× sprint (MULTIPLY_BASE ≈ 2.1, or the survey's mapping), the withdrawn ENT-S-145 hunks re-applied with the number and its pins. Wave 5 (with whatever the survey files) gates only when a chain slice waits on the owner.

### ENT-S-151 — The Stinky's and Spyro's flight-target reach is measured from the exact position to the block centre with a squared threshold, where 1.7.10 read the integer-lattice distance² below 2.1f: `closerToCenterThan(this.position(), 2.1)` for `getDistanceSquared((int) posX, (int) posY, (int) posZ) < 2.1f` (REPORT, 2026-09-06; the wave-4 refuter A's filing)
...
- **Ruled (owner, 2026-09-06, item 22):** parity, classic — the XS batch with ENT-S-148 and ENT-S-149, one refuter.
...
- **Impact:** MEDIUM (suite reliability) — the harness slice (`phase_g_reports/harness_slice_2026-09-04.md`,
  presented 2026-09-04 and ruled "apply the ranked fixes that are pure isolation — flag restoration in finally, batch
  separation, spacing; no widened waits, no retries") found four default-batch tests unsafe beside others as written:
  `leaf_monster_prey_allowlist_play_nicely` held the GLOBAL `PLAY_NICELY` true across a 150-tick window in
  `defaultBatch:0` (the only multi-tick PlayNicely window in the default batch; unrestored on its phase-3 failure path)
  — the hypothesised cause of i050's wave1e "was 0.0" (its `vortex.ai()` exits null on `PLAY_NICELY.get()`, and in the
  585 layout i050's wilderness-bordered cell started late enough to land inside the window); `boyfriend_tempt_panic_door`
  and `leon_ducky_tame_untame_tempt` left a CREATIVE mock `ServerPlayer` in `level.players()` for the rest of the run when
  they timed out before their tempt hand-off (a despawn anchor for every non-persistent mob in later tests — the bug003
  / i127-minion hazard); `big_bertha_shockwave_damage_and_pvp` restored `BIG_BERTHA_PVP` only in its tick-44 callback,
  leaving it true after an assertion throw at ticks 25/28/36/44; and `ConfigGateTests.checkVeinRatio` returned with
  `LESS_ORE` true (contained today only by its caller's `finally`). 1.21.1's framework offers no finally for a scheduled
  callback or a timed-out test (`GameTestHelper` has no `addCleanup`).
- **Evidence:** harness_slice_2026-09-04.md §1 (flip-site sweep: the one multi-tick default-batch PlayNicely writer at
  EntityLogicTestsA.java:1486/:1506; wave1e timeline 09:44:31-09:44:53 with i050 among the 5 late starters of
  `defaultBatch:0`; the excluded exits), §3(a) (the seven default-batch mock-player sites; boyfriend :784/:829-832 and
  leon :1622/:1627-1630 remove only on the hand-off or on success), §3(b) (big_bertha :339-406 toggles restored only at
  tick 44; item064 :159-165), §3(c)/AUDIT TEST-003 amendment (the HashSet<Method> order model). Framework: NF
  GameTestInfo.java:95-113 (listeners notified in the finishing tick, `testFailed` for throw and timeout, `testPassed`
  for succeed), :131-137 (callback throw → `fail`), GameTestHelper.java:65 (`public final GameTestInfo testInfo`),
  GameTestBatchFactory.java:18 (batch HashMap). Record of the lane: `scratchpad/waves/HARNESS_records.md` (this file).
- **Resolution:** FIXED (2026-09-04, harness slice lane; pure isolation only — no assertion, tolerance or wait changed).
  (1) `leaf_monster_prey_allowlist_play_nicely` → its own batch `playNicelyWindow` (the boss017 / `playNicelyIsolation`
  precedent, one test per batch) and `PLAY_NICELY` restored to its captured prior on every exit path;
  (2) boyfriend / leon tempt players removed from `players()` exactly once on every exit path (hand-off, success, throw,
  timeout), the dead success-only removal lines dropped; (3) `BIG_BERTHA_PVP` restored to its prior on every exit path
  (the tick-44 restore kept); (4) `checkVeinRatio` restores `LESS_ORE` in a `finally` (the caller's finally kept as the
  second layer); (5) i050's three drag-pull messages carry `playNicely=` and `gameTime=` (the bug003 idiom; the flag is
  NOT pinned inside i050). Mechanism for (1)-(3): a new `EntityLogicTestsA.onTestExit` registers a `GameTestListener` on
  `helper.testInfo` and runs the restore once from `testPassed` / `testFailed` — the framework's finally for every
  terminal path, executed in the finishing tick before any bucket-mate ticks. Order-model consequence (recomputed): the
  remaining 145 default-batch tests keep their relative order; `chipmunk_apple_tame_dead_bush_release` crosses 50 → 49
  into bucket 0 (synchronous survival player in a finally — safe) and `i138_inca_pyramid_content` crosses 100 → 99 into
  bucket 1 (`buildNow` RNG draws behind wide statistical bands — its draws change, not its verdict); the new batch
  iterates before `defaultBatch` (bin 33 < 37 of 64), so every default grid cell after leaf_monster's old slot is
  unchanged (i050 keeps the wave1e cell, now without the window) and only the first two default tests move one cell.
  javac rc 0 on the three files (no gradle in this lane); the gametest run is the gate's. Remaining from the slice's
  ranking, for other lanes: F1 (i127's structural pin / owner-scoped acid negative / minion cull / cow kept alive) and
  F5 (the `buildNow(level, origin, type, RandomSource)` seam for the layout-count assertions incl. dsb_item020).
  Refuted once, upheld: the exit hook fires exactly once on every terminal path (GameTestInfo.tick notifies its

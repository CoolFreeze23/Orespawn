...
def expand_explicit_instances(name: str, part: dict[str, Any], absolute_pivot: list[float],
    ...
    """The folder's gaps (owner 2026-09-14, items 2 and 3): ``step_scope: explicit``.
...
# THE HIERARCHY FORM (owner 2026-09-15, closing set, item 4; design section 5 "FK-chained"; the FK slice): a manifest
# entry's `hierarchy` is {child: parent} over the compiled model's TOP-LEVEL parts - the classic flat rig whose child
# pivots the classic setupAnim rewrites by trigonometry (the Alien's neck / head / jaw, tail and claw chains, the Emperor
# Scorpion's tail, legs and claws). The converter emits each declared child as a GeckoLib bone PARENTED to its chain
# parent: the part names preserved, its pivot the absolute bind pivot in Blockbench terms (unchanged), its bind rotation
# the LOCAL one - the parent's flat (world) bind rotation inverted onto the child's, R_local = R_parent^-1 * R_child, so
# the bake's world rotation at bind is the classic's (GeckoLib composes a parent's rotation onto its children, RenderUtil
# .prepMatrixForBone: translate to the bone, to its pivot, rotate Z then Y then X, away from the pivot - the same ZYX
# order as ModelPart.translateAndRotate, and the conjugation through the entity frame's reflection preserves composition).
# The hook expresses the classic world transforms through parent-relative rotations and positions (FlatRig); the harness's
# chain-link leg (tools/g1_render_parity.py chain_link_parity) compares the world matrices at every link.
# THE HIERARCHY RULES (owner 2026-09-15, closing set continued second, item 35 (5); TEST-016 / TEST-017): "A hierarchy rig
# draws parent-first; the draw-order leg's rule for a hierarchy entry is the key's pre-order, and the visual leg is the
# judge of what the order change shows." The seam draws a bake in PRE-ORDER (GeoRenderer.renderRecursively draws a bone's
# cubes, then its children; DrawOrder.apply refuses a key that is not a pre-order of the tree), so for a hierarchy entry
# the key IS the tree's pre-order - the classic order lifted over the declared tree, siblings by their subtree's first
# classic draw - and a classic order that is not one (a parent drawn after a child, a subtree interleaved with another:
# the Alien's tail rings before their root, the Emperor Scorpion's left pincer after the right arm) is not refused but
# RECORDED in the evidence (`hierarchy_preorder`: the classic order, the units whose position moved, the findings) as the
# deviation the draw-order leg reports and the visual leg judges. A flat entry (no hierarchy) is refused as before.
# The former measurement field `hierarchy_draw_order` (the first attempt's preorder_diagnostic form) is retired: refused.
HIERARCHY_PREORDER_RULE = (
    "a hierarchy rig draws parent-first (owner 2026-09-15, closing set continued second, item 35 (5)): the key is the "
...
def hierarchy_declared(spec: dict[str, Any], compiled: dict[str, Any]) -> dict[str, str]:
    ...
    if retired:
        raise ValueError(f"{model_id} carries the retired field(s) {retired}: a hierarchy entry's draw-order key is the "
                         "tree's pre-order by rule (owner 2026-09-15, item 35 (5)); there is no measurement form")
    if declared is None:
        return {}

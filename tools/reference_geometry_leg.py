...
The independent leg (BUG-040): the port's classic model is compared against
the ORIGINAL model's geometry, parsed from the decompiled 1.7.10 source
(read-only), never from the port. The port side is the compiled dump
`G1ModelProbe vanilla` already writes (`<id>.compiled.json`: definition tree
with per-part local pivot, initial rotation, cubes with origin, size, uv,
mirror, deformation; texture size).
...
def main() -> int:
    ...
    for spec in manifest["models"]:
        if "reference_source" not in spec:
            # TEST-010 (a): a geometry-only entry with no 1.7.10 ModelPart source (the Boyfriend and the
            # Girlfriend: HumanoidModel subclasses 1.7.10 drew with vanilla ModelBiped) is reported as UNPAIRED and
            # never compared - its compiled dump is the converter's input for the artist package's rig, and
            # nothing here can be compared against; no report file, so the checked-in proof set stays the paired
            # entries' alone.
        ...
        if reference["status"] != "PARSED":
            ...
            # Pinned divergences: the owner-decided parity bugs a model still carries. The leg passes
            # only when the observed category counts equal the pin exactly; a new category or a larger
            # count is a regression, a smaller one means a fix landed and the pin must be cleared with it.

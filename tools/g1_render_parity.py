...
# rotationZYX(zRot, yRot, xRot) = Rz * Ry * Rx); on the GeckoLib side the bone's world matrix as the bake renders it
# (the probe's `bone_poses_classic`: the pose stack at the bone, conjugated into classic space), closed by the bone's
# absolute bind pivot - a classic cube corner is local to the part's pivot, a GeckoLib corner absolute, so
# classic_world == bone_pose * T(pivot / 16) (the render-instance leg's relation). Both are compared within the GEOMETRY
# leg's tolerance (the ruling: linear entries and the translation column alike, in blocks) and reported per link.

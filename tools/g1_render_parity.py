...
def render_capture(sample: dict[str, Any], texture: Image.Image,
    ...
    2 (2026-09-02) did before the contract. The rasteriser's own tie rule at
    an EXACT depth tie is first-wins (a later fragment replaces the front only
    when nearer by more than 1e-9) where the game's LEQUAL depth test is
    last-wins; parity is unaffected because both captures share the rule and the order.
    """

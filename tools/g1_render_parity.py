...
# (draw-order resolved in both renderers; decision 2, 2026-09-02: not a parity target).
CONTEST_DEPTH_EPSILON = 1.0e-6
# Decision 2026-09-02: a species whose pinned excluded fraction exceeds this needs a
...
def render_capture(sample: dict[str, Any], texture: Image.Image,
    ...
    Both real renderers resolve that by draw order, which decision 2
    (2026-09-02) excludes from parity; the mask lets the comparison skip
    exactly those pixels and report how many there were.
    """
...
def visual_parity(model_id: str, spec: dict[str, Any], compiled: dict[str, Any],
    ...
    if "max_contested_fraction_pin" not in spec:
        ...
        if max_contested > contested_pin:
            raise AssertionError(
                ...
                "raising the pin is an decision"
            )
    ...
    return {
        ...
        "z_fight_policy": (
            ...
            "excluded from the comparison and painted in the diff (decision 2, 2026-09-02)"
...
def contested_line(visual: dict[str, Any]) -> str:
    if visual.get("contested_exclusion_applied", True):
        return (f"- Visual z-fight pixels excluded (decision 2): maximum contested fraction "

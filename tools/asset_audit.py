...
Hooks (owner ruling 2026-09-14, addendum item 10): a GeoReplacementDescriptor
written ahead of its rig - the classic setupAnim transcribed into the PoseInputs
form, unregistered, its geo and clip landing with the species' slice (item 14).
A descriptor listed in HOOKS below has the TEXTURE_REF_MISSING findings for ITS
OWN geo/entity/<rig>.geo.json and animations/entity/<rig>.animation.json
acknowledged while the rig is pending; every other reference it makes (textures,
another rig) is audited as usual. A HOOKS entry whose rig ships, or that names no
descriptor, is HOOK_STALE - an ERROR, never acknowledgeable - so the list empties
as the slices land.
...
# Hooks (owner ruling 2026-09-14, addendum item 10): descriptor class name -> the rig it
# names. Each is the classic setupAnim transcribed verbatim into the PoseInputs form ahead
# of its rig, unregistered (no renderer binds it, so the references are dead at runtime);
# its geo/entity/<rig>.geo.json and animations/entity/<rig>.animation.json land with the

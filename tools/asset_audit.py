...
Checks
  ...
  8. GECKOLIB/MHLIB every geo/animation JSON parses with bones/clips; every
                      ...
                      of unique strings naming exactly the rig's bones (never
                      acknowledgeable); every shipped geo/entity/*.geo.json is
                      reconciled: a seam rig, or a member of OUTSIDE_SEAM with its
                      dated reason (never acknowledgeable); and no reference-only
...
# The reference-only clips (one per reachable state since 2026-09-14): the package sampler's output,
# tools/reference_clips/<registry>_reference_walk / _idle / _attack.animation.json (the single
# <registry>_reference.animation.json before 2026-09-14) - the classic hook sampled at fixed inputs for the artist to
# look at - is carried beside each species' sheet and NEVER shipped; the package checker refuses a returned one
# under its own name, and this audit refuses one anywhere under src/main/resources (never
# acknowledgeable).
...
# mobs multipart hitboxes. Profile main-size law (ENT-S-088): a
...
# permutation per cube of each), else the client would WARN / ERROR-log and fall back.
...
OUTSIDE_SEAM = {
    ...
    # reading of "any shipped orespawn rig" would need a key nothing consumes - left open.
    #
    "the_queen",
}
...
def _strip_java_comments(text):
    ...
    neither cut a statement short nor count as a construction (slice (c)'s; the kf17 gate, where a
    trailing comment's ';' hid two rigs' geo literals from seam_rigs)."""
    out = []
...
def _flat_cubes(geometry):
    """The rig's ZERO-THICKNESS cubes as "bone[index] WxHxD" (TEST-007): a cube with a 0 size component and no
    inflation (its own or its bone's) has two coplanar real faces that both draw under
...
def _face_order_problem(description, bone_cubes, required):
    ...
    the client's FaceOrder.read + FaceOrder.apply refuse (ENT-S-146): the key's bones must be
...
def check_geckolib(java_texts):
    ...
    # G2 root-order contract (on the landing, B-D3 / B-D6): every rig the replacement seam
    # draws - the model resource of a GeoReplacementDescriptor, which the shared
    # OreSpawnGeoReplacementModel sorts into the classic draw order at its first bake -
    # must ship that order under
    ...
    for asset, java_path in sorted(seam.items()):
        ...
        if problem:
            err("GECKO_GEO_DRAW_ORDER_MISSING", geo_path.stem,
                ...
                geo_path)
        # ENT-S-146: the face-order key must be exactly what the client's FaceOrder.apply accepts (the
        # cube-bearing bones, their cube counts, six-name permutations), and a rig whose descriptor
        # requires it (cubeFaceOrderRequired() true in the descriptor's source, the java file the rig is
        # attributed to) must ship it; absent is the norm for every other rig.
        ...
        if problem:
            err("GECKO_GEO_FACE_ORDER_INVALID", geo_path.stem,
                ...
                geo_path)
        # TEST-007: a cutout rig with a zero-thickness cube that omits the classic within-cube face order is
        # refused - the key is what keeps the candidate's coplanar pair in the classic renderer's order (the
        # Cloud Shark's fins showed the other texture island without it). A rig whose descriptor requires the key
        # is covered by the rule above; this one catches the omission.
    ...
    # The reference-only clips never ship (per state since 2026-09-14): a *_reference_<state>.animation.json (or the old
    # single *_reference.animation.json) anywhere under src/main/resources is an ERROR naming the file, never
    # acknowledgeable - the package copies them beside the sheet from tools/reference_clips/ and the checker refuses a
    # returned one under its own name; the jar carries none.

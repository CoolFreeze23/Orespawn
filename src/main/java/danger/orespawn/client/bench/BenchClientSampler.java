 ...
 *
 * <p>Requests from the server thread: one latest-wins slot ({@link #request}: a
 * stop, or the seconds of a start) instead of two flags, because two flags cannot tell {@code
 * stop} then {@code start} from {@code start} then {@code stop} within one frame. The render
 * thread takes the slot on its next frame and, before anything else, ENDS the run in flight
 * (stopped early) -- then begins the new one if the request was a start. So a {@code scene}
 * (abort, i.e. stop) followed by {@code start} in the same frame starts the new run instead of
 * killing it a frame later, and a {@code start} followed by {@code stop} leaves nothing running,
 * which is what the server session holds in both cases.</p>
 */

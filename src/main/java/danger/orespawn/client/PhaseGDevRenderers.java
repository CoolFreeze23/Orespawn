...
public final class PhaseGDevRenderers {
    ...
    /**
     * The FK slice (2026-09-15, owner's closing set item 4 under the hierarchy rules of item 35 (5)): the Alien rig's two
     * registries and the Emperor Scorpion ON THE HOOKS already written, shipped as real parent-child hierarchies (FlatRig);
     * the classic renderers the default. The Alien Boss's classic side is the AlienRenderer (orig ClientProxyOreSpawn.java:435
     * drew both registries with the one RenderAlien; the port registers AlienRenderer, typed on Alien, for the AlienBoss type
     * through registerEntityRenderer's {@code EntityType<? extends T>} widening) - {@code select} widens it the same way, so
     * the switch keys the two registries apart ({@code alien}, {@code alien_boss}).
     */
    ...
    /**
     * The remainder slice (2026-09-15, owner's closing set item 4): the seven remaining rigs ON THEIR HOOKS - the Lurking Terror
     * and the Scorpion (the pair-contested rule), the Ghost (ENT-S-160 (a)), the Boyfriend, Girlfriend and Princess (item 3: the
     * seam's partial tick and the classic layers), the King (the second pass, TEST-018); the Butterfly rig's four consumers
     * REPORTED, not landed (TEST-019: no provider, their classic renderers alone); the classic renderers the default.
     */

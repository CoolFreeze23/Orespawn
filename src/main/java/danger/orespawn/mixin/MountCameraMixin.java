...
/**
 * S7b — the riding camera (sitting FAIL-4, design). Technique per
 * Shoulder Surfing Reloaded (MIT; studied, not copied): wrap the
 * vanilla third-person zoom-arm move inside {@code Camera.setup} (ordinal
 * 0 — setup has a SECOND move in its sleeping branch, and {@code
 * WrapOperation} instead of a redirect so Shoulder Surfing itself, which
 * redirects the same instruction, chains rather than crashes —

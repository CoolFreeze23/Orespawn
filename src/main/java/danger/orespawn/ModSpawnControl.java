    ...
    private static Map<EntityType<?>, Supplier<Boolean>> getSpawnControls() {
        if (spawnControls == null) {
            spawnControls = Map.ofEntries(
                    ...
                    // Phase E decision (2026-08-11): AppleCow / GoldenAppleCow
                    // are wiki-only (no 1.7.10 source class) — natural
                    // spawns additionally require phase14ContentEnable.
                    // EnchantedAppleCow is the original EnchantedCow (orig
                    // OreSpawnMain.java:3599, display name "Enchanted Golden
                    // Apple Cow" :2765) and stays on the plain CowEnable gate.
                    ...
                    // Wiki-only hostile (same decision); currently has no
                    // natural spawn data, the entry is a guard in case one is added.

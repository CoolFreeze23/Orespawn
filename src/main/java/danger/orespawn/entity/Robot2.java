...
public class Robot2 extends Monster implements Robot2Pose {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(1, this.revengeGoal); // released by the pass's 1-in-50, as orig's task ended on a nulled attack target (ENT-S-129)
    }
     ...
     * memory ({@link #release}). The hold itself stays vanilla's. ENT-S-129.
     */
    ...
    protected void customServerAiStep() {
        ...
        if (this.getRandom().nextInt(6) == 1 && !OreSpawnConfig.PLAY_NICELY.get()) {
            if (this.getRandom().nextInt(50) == 1) { this.setTarget(null); this.revengeGoal.release(); } // orig :281-283 — rolled BEFORE the read (:284): a forgotten attacker is not engaged in the forgetting pass (ENT-S-133); the task ended on the nulled target; vanilla's TargetGoal would re-assert its memory (ENT-S-129)

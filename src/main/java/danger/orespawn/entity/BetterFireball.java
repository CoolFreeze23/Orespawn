...
public class BetterFireball extends LargeFireball {
     ...
     * parity bug, fixed in classic; MOD-031 proposes a config-gated modern option). A
     * block hit switches on the hit side ({@code field_72310_e} 0..5 = down, up,
     * north, south, west, east, :236-260) to the neighbour on that side and, if that
     * cell is air ({@code func_147437_c} = isAirBlock, :261), sets {@code Blocks.fire}
     * there ({@code func_147449_b} = setBlock with flags 3, :262) — before the explosion
     * (:265-267) and the setDead (:268), and for small shots too (the small gate covers
     * only the explosion). Port: the hit face's relative position, {@code
     * Level.isEmptyBlock} (= isAir) and {@code setBlockAndUpdate} (flags 3), with
     * the fire state from {@code BaseFireBlock.getState} as 1.21.1's own fireballs use
     * it (soul fire over soul soil / soul sand, plain fire elsewhere; 1.7.10 had the one
     * fire block). Orig's conditions are kept where vanilla's differ: no mobGriefing
     * / EntityMobGriefingEvent gate (1.21.1 {@code SmallFireball.onHitBlock} has one for
     * Mob owners) and no {@code BaseFireBlock.canBePlacedAt} survival check (orig
     * placed fire on any air cell; fire that cannot survive there removes itself on its
     * own tick in both versions). super first: {@code Projectile.onHitBlock} is the

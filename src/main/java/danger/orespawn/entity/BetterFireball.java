...
public class BetterFireball extends LargeFireball {
     ...
     * shots as {@code new BetterFireball(level, this, accel)}). ENT-S-098, the
     * 2026-09-03 decision: it used to chain to {@code LargeFireball(level,
     * shooter, movement, 1)}, which is {@code super(EntityType.FIREBALL,
     * ...)} (1.21.1 LargeFireball.java:23-26), so a shot was typed {@code
     * minecraft:fireball}: the better_fireball registration (its .sized,
     * noSummon, the renderer binding) governed only EntityType#create
     * instances, clients rebuilt shots as plain LargeFireballs, and NBT saved them
     * under the vanilla id. LargeFireball has no type-taking shooter constructor,
     * so this goes through the (EntityType, Level) chain with the mod's own type and
     * replays, in the same order, exactly what the vanilla chain assigned
     * (AbstractHurtingProjectile.java:37-50 -> Fireball.java:27-29 ->
     * LargeFireball.java:23-26): moveTo the shooter's feet at the fresh 0/0
     * rotation, reapplyPosition, the normalised-times-{@code
     * accelerationPower} (0.1) delta movement with hasImpulse (vanilla's private
     * assignDirectionalMovement), setOwner, then setRot from the shooter.
     * Vanilla's private explosionPower keeps its field initialiser 1, the
     * literal the old chain passed, so the LargeFireball-side explosion is
     * unchanged; the port's own power/damage/small flags are untouched. The 1.7.10
     * kinematics (:58-67: shooter position + rotation, motion zeroed, accel = dir/|dir| * 0.1) are the same values.
     */
     ...
     * 0.3125x0.3125 (:84). The port's setSmall only flipped the flag; the
     * 2026-09-03 decision (ENT-S-095) restores the shrink the modern way:
     * the projectile chain
     * (LargeFireball/Fireball/AbstractHurtingProjectile/ Projectile) has no
     * dims hook of its own and {@code getDefaultDimensions} is a
     * LivingEntity method, so the non-living override point is {@code
     * Entity#getDimensions(Pose)} (the vanilla AreaEffectCloud pattern), keyed
     * on the flag and applied through {@code refreshDimensions()}, which
     * rebuilds the live AABB via reapplyPosition() whether or not the entity
     * is in a level yet (shooters call setSmall before addFreshEntity).
     * As in 1.7.10 the flag is neither synced nor saved: the client keeps the type's 1x1 box exactly as its World-ctor copy did (orig :46-49).
     */
    ...
    /**
     * orig BetterFireball.java:232-264, the block half of func_70227_a (ENT-S-104, a
     * parity bug, fixed in classic; MOD-031, gates the fire on mobGriefing while
     * effective, below). A block hit switches on the hit side ({@code field_72310_e}
     * 0..5 = down, up, north, south, west, east, :236-260) to the neighbour on
     * that side and, if that cell is air ({@code func_147437_c} = isAirBlock, :261),
     * sets {@code Blocks.fire} there ({@code func_147449_b} = setBlock with flags 3, :262)
     * — before the explosion (:265-267) and the setDead (:268), and for small shots too
     * (the small gate covers only the explosion). Port: the hit face's relative position,
     * {@code Level.isEmptyBlock} (= isAir) and {@code setBlockAndUpdate} (flags 3),
     * with the fire state from {@code BaseFireBlock.getState} as 1.21.1's own
     * fireballs use it (soul fire over soul soil / soul sand, plain fire elsewhere;
     * 1.7.10 had the one fire block). In classic, orig's conditions are kept where
     * vanilla's differ: no mobGriefing / EntityMobGriefingEvent gate (1.21.1 {@code
     * SmallFireball.onHitBlock} has one for Mob owners) and no
     * {@code BaseFireBlock.canBePlacedAt} survival check (orig placed
     * fire on any air cell; fire that cannot survive there removes itself on its own tick
     * in both versions). MOD-031 (decision of 2026-09-04: "accepted as a modern
     * option, default on; classic stays 1.7.10"): while {@code
     * OreSpawnConfig.fireRespectsMobGriefing()} is effective (master AND key, read here
     * at impact, never snapshotted) the fire is placed only if {@code
     * EventHooks.canEntityGrief(level, getOwner())} -- vanilla the
     * gate vanilla's fireballs use ({@code LargeFireball.onHit} for every owner;
    ...
    protected void onHitBlock(BlockHitResult result) {
        ...
        if (this.level().isClientSide) return;
        // MOD-031 (decision of 2026-09-04: "accepted as a modern option, default on; classic
     ...
     * setDead :268). ENT-S-102, the 2026-09-04 decision ("fix with a test"): this
     ...
     * {@code Explosion.isFlaming} / {@code isSmoking}). ENT-S-104 (classic):
     * the port used to feed mobGriefing into 1.21.1's single {@code fire} slot, so
     * an OreSpawn fireball placed no explosion fire while the rule was off; it now
     * passes fire = true and lets {@code ExplosionInteraction.MOB} resolve the
     * destruction flag, which for a null source IS the mobGriefing gamerule: 1.21.1
     * {@code Level.explode} (the 13-argument form, bytecode) maps MOB to {@code
     * EventHooks.canEntityGrief(level, source) ? getDestroyType(RULE_MOB_EXPLOSION_DROP_DECAY) :
     ...
     * blast. MOD-031 (decision of 2026-09-04: "accepted as a modern option, default on;
    ...
    protected void onHit(HitResult result) {
        ...
        if (!this.level().isClientSide) {
            if (!this.small) {
                ...
                // (decision of 2026-09-04: "accepted as a modern option, default on; classic

 ...
 * lanes, 2026-09-14; the Slice 4b form of {@link Robot2Pose} and {@link RotatorPose}): the activity that selects the
 * standing or the flying part set (orig ModelLeon.java:729/852 {@code getActivity() == 0}; the port's
 * LeonModel.poseFrom), the attacking flag that speeds and widens the flight beat and opens the flying jaw, the
 * being-ridden flag that stills the flight bob and swaps the head yaw source, the sitting check that stills the
 * standing wing sway, the per-entity render scratch whose {@code rf1} ACCUMULATES the ridden head yaw across frames
 * (orig ModelLeon.java:1013-1024, orig Leon.java:64; ENT-S-093 - the Rotator's precedent for per-frame RenderInfo
 * accumulation) and the yaw pair that feeds it ({@code prevRotationYaw - rotationYaw}, orig :1014). The classic model

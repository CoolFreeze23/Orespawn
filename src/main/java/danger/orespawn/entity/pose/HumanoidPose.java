 ...
 * interface the entity already satisfies (drafted by the fourth Tier-2 slice T2d, TEST-010 (b); landed by the remainder
 * slice, under -; the Slice 4b form of {@link CrabPose}). {@code ModelBoyfriend} / {@code ModelGirlfriend} declare no
 * {@code setupAnim}: the pose the classic {@code HumanoidMobRenderer} draws is vanilla {@code
 * net.minecraft.client.model.HumanoidModel.setupAnim} (NeoForge 21.1.223, HumanoidModel.java:137-286
 * by the jar's LineNumberTable) over the six floats plus these entity reads, and over the model fields {@code
 * LivingEntityRenderer.render} sets per frame - {@code attackTime} (bytecode offsets 39-49: {@code
 * getAttackAnim(partialTick)}), {@code riding} (52-89), {@code young} (92-100; not read by {@code setupAnim}) - and
 * {@code HumanoidModel.prepareMobModel} sets ({@code swimAmount}, :129-131: {@code getSwimAmount(partialTick)}). The
 * GeckoLib hook reads the same values through this interface at the seam's partial tick ({@code PoseInputs.partialTick()}),
 * the classic model's {@code poseFrom} the same way, and the harness's {@code ProbeSubject} answers every one at its
 * rest value unless a declared state raises it.
 *
 *

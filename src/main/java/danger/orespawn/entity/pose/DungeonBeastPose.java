 ...
 * the {@link Robot2Pose} shape): the per-entity render scratch whose {@code ri1} / {@code ri2} are the jaw latch
 * re-rolled at each falling zero-crossing (orig ModelDungeonBeast.java:546-573), the attacking flag that fixes the tail
 * amplitude and the re-roll (orig :506, :550), and the entity's RNG the re-roll draws from (orig :551-552). The classic
 * model and the GeckoLib hook both pose from this, so the parity harness can drive them headlessly with a declared state
 * and a seeded source instead of a live entity.
 */

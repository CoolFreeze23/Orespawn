 ...
 * owner 2026-09-14, addendum item 10; the Slice 4b form of {@link Robot2Pose}, the {@link RotatorPose} RenderInfo and
 * the {@link Robot2Pose#getRandom()} RNG): the per-entity render scratch whose {@code ri1} / {@code ri2} latch which
 * claw and whether the tail strikes, rolled once per rhythm cycle on the frame the 3 x wingspeed cosine crosses zero
 * upward (orig ModelScorpion.java:198 {@code r = e.getRenderInfo()}, :200-211; the port's ScorpionModel.poseFrom);
 * the attacking flag that picks the roll's ranges (orig :203 {@code e.getAttacking() == 0}: 20 / 25 at rest, 4 / 3

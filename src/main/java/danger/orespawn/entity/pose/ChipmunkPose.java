 ...
 * the Slice 4b form of {@link Robot2Pose}): the sitting flag that stills the tail - orig ModelChipmunk.java:168 {@code
 * if (!c.func_70906_o())} (EntityTameable.isSitting; the port's {@code
 * TamableAnimal.isInSittingPose()}). The classic model and the GeckoLib hook both pose from this, so the parity

...
public class EntitySpyro extends TamableAnimal implements SpyroPose {
    ...
    private LivingEntity findSomethingToAttack() {
        if (OreSpawnConfig.PLAY_NICELY.get()) return null; // orig Spyro.java:698-700 — PlayNicely != 0 returns null ahead of the scan (ENT-S-115)
        AABB searchBox = this.getBoundingBox().inflate(12.0, 6.0, 12.0);
        List<LivingEntity> targets = this.level().getEntitiesOfClass(LivingEntity.class, searchBox);
        // OPT-021: nearest-first pick without the full list sort; TargetSelection
        // preserves the removed sort's order and stable-tie semantics exactly.
        // orig Spyro.java:709 — the loop takes the first candidate that passes isSuitableTarget AND the feet ray canSeeTarget(posX, posY, posZ) (ENT-S-118)
    ...
    /**
     * orig Spyro.java:436-438 ({@code canSeeTarget}): {@code worldObj.rayTraceBlocks(Vec3(posX, posY + 0.75, posZ),
     * Vec3(pX, pY, pZ), false) == null} — a block-only ray from 0.75 above the feet to a point; the scan (:709) aims it

...
public class UltimateArrow extends AbstractArrow {
    ...
    /**
     * ENT-S-103 (a parity bug, fixed in classic) — Punch knockback the 1.7.10
     * way. orig UltimateBow.java:52-54 seeded the arrow with the bow's Punch level
     * ({@code func_70240_a} = setKnockbackStrength) and orig
     ...
     * which this arrow is decided OUT of (tag, pinned by
     ...
     * decision and would double the push if that decision ever changed. Vanilla calls this
     * only after a successful hurt on a LivingEntity (AbstractArrow.onHitEntity), orig :182's
     * gate; orig :183 further required EntityLiving (1.21.1 {@link Mob}), so a player
     * hit with ultimate_sword_pvp on takes the vanilla 0.4 hurt knockback but no Punch
     * push, as in 1.7.10 (with pvp off, {@link #onHitEntity} no-sells players before this point).
     */
    @Override

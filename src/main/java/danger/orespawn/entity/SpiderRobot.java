...
public class SpiderRobot extends Mob implements ICustomHitboxProfileSupplier, IModernLeggedRobot {
     ...
     * [modern] enabled master forces CLASSIC while off — (BOSS-017
     * snapshot pattern — a config flip affects newly constructed
     * spiders only). CLIENT: created lazily off the server's {@link
     * #DATA_MODERN_GAIT synced flag}; the client's own config is never
     * consulted. Classic-mode spiders on either side never construct
    ...
    public void tick() {
        ...
        if (this.level().isClientSide()) {
            ...
            if (isModernMovement()) {
                ...
                // OPT-013 / MHLib harvest 3: the client mirror just moved the eight leg parts
    ...
    /**
     * Q1 decision: modern-mode spiders are STEERABLE by a mounted
     * player; classic keeps the faithful 1.0 gap (a mounted player cannot
     * steer — this returns the vanilla null). The SpiderDriver is NEVER
     * controlling in either mode (it is not a Player); its classic
     * velocity-set shoving coexists untouched with this path.
     */
    ...
    /**
     * orig SpiderRobot.java:86-88 ({@code func_70692_ba}, canDespawn): {@code return false;} — never despawns, where the 1.21 Mob default (and EntityLiving's) would. ENT-S-171.
     */
    @Override

...
public class EntityCaterKiller extends Monster implements CaterKillerPose {
     ...
     * runs, and an every-set clear turned the scan's own pick into a sticky one (ENT-S-117 window). The port-wide
     * convention decided in ENT-S-129 (the Water Dragon's ENT-S-117 form); the hurt hand-off is in {@link #hurt}.
     * ENT-S-135.
     */
    @Override
    ...
    private final class RevengeGoal extends HurtByTargetGoal {
        ...
        /** The goal's own memory: the pass's 1-in-200 may clear only what this goal stored, as orig :468-470 could only clear a stored attacker (ENT-S-129). */
        LivingEntity held() {
    ...
    protected void customServerAiStep() {
        ...
        if (this.random.nextInt(4) == 0) {                                   // orig :462
            ...
            if (this.random.nextInt(200) == 0 && this.getTarget() != null && this.getTarget() == this.revengeGoal.held()) { // orig :468-470 — the roll spent every pass, the clear only on the revenge goal's own target (a goal-held player pick is not a stored attacker)
                this.setTarget(null);
            ...
            if (current == null || current == this.scanPick) {
                ...
                // cancelled the set, and a stale scanPick would stall the scan (ENT-S-108 hardening).

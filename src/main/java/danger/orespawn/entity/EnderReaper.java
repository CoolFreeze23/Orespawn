...
public class EnderReaper extends Monster {
    ...
    boolean shouldAttackPlayer(Player player) {
        ...
        return dot > 1.0 - 0.025 / dist && OreSpawnSight.canSee(player, this); // orig :92 — player.canEntityBeSeen(this): the PLAYER's ray under the 1.7.10 convention, routed by hand because the receiver-gated mixin cannot reach a player receiver (ENT-S-121)
    }
    ...
    /**
     * orig EnderReaper.java:159-206 {@code teleportTo} — the landing search (:169-191, the port's {@code LivingEntity.randomTeleport}:
     ...
     * (ENT-S-144). Named apart from {@code Entity.teleportTo(double, double, double)}, which the engine owns; the Knight's twin.
     * The sound is {@code SoundEvents.ENDERMAN_TELEPORT} ("entity.enderman.teleport"), the modern event of 1.7.10's "mob.endermen.portal".
     */

...
public class Nastysaurus extends Monster implements NastysaurusPose {
    ...
    private void selectTarget() {
        ...
        if (OreSpawnConfig.PLAY_NICELY.get() && current != this.scanPick) current = null; // orig :215-217 — `e = rt; e = null`: only a foreign occupant (orig rt, never the scan's pick) is blanked for the pass, the slot untouched; the scan's own pick runs on to the gated scan and is cleared as at HEAD (ENT-S-115)
        LivingEntity rt = this.revengeGoal.held();
        if (rt != null && rt != current && !OreSpawnConfig.PLAY_NICELY.get()) { // orig :218 — `e = rt` while the scan's pick occupies the slot: rt is still rolled and re-taken every pass (ENT-S-129)
        ...
        // (ENT-S-108 hardening). A pass that set nothing — both null, the PlayNicely-blanked
        // pass of orig :215-217 — claims nothing: the slot may still hold the revenge target the
        // pass never touched (ENT-S-115).

...
public class Hammerhead extends Monster {
    ...
    protected void registerGoals() {
        ...
        // (:194-221) consumed rt (:182-184) alone. The port's pass reads the slot as a fallback (ENT-S-115), so a
        // registered revenge goal would re-supply an attacker rt had forgotten (:198-201) until vanilla's own
    ...
    protected void customServerAiStep() {
        ...
        if (this.random.nextInt(3) == 1) {
            ...
            if (currentTarget == null && !playNicely) { // port-only read of the slot (HurtByTargetGoal's channel): under the flag orig's pass consulted nothing (:194-209) and set attacking 0 (:219-221), so the read is gated with the pass; the slot itself is untouched (ENT-S-115)

...
public class MyEntityAINearestAttackableTargetGoal<T extends LivingEntity> extends NearestAttackableTargetGoal<T> {
    ...
    public MyEntityAINearestAttackableTargetGoal(Mob owner, Class<T> targetClass, double targetDistance, boolean mustSee,
        ...
        if (!mustSee) conditions.ignoreLineOfSight(); // vanilla's own `!mustSee → ignoreLineOfSight`, which the rebuild had dropped; every registration passes true, so inert today
        if (this.targetType == Player.class) conditions.ignoreLineOfSight(); // orig MyEntityAITarget.java:96 — a Player is answered BEFORE the sight step :108 (and the reach block :117, see canAttack): the Valentine Player task takes a player it cannot see, as orig did

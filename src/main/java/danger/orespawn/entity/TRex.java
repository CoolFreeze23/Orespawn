...
public class TRex extends Monster {
    ...
    private void selectTarget() {
        ...
        if (OreSpawnConfig.PLAY_NICELY.get() && current != this.scanPick) current = null; // orig :185-187 — `e = rt; e = null`: only a foreign occupant (orig rt, never the scan's pick) is blanked for the pass, the slot untouched; the scan's own pick runs on to the gated scan and is cleared as at HEAD (ENT-S-115)
        LivingEntity rt = this.revengeGoal.held();
        if (rt != null && rt != current && !OreSpawnConfig.PLAY_NICELY.get()) { // orig :188 — `e = rt` while the scan's pick occupies the slot: rt is still rolled and re-taken every pass (ENT-S-129)
        ...
        // pass of orig :185-187 — claims nothing: the slot may still hold the revenge target the

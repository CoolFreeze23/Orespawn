 ...
 *
 * <p><b>Gait</b> (S1 reference technique, walk-only per design decision
 * Q2): a planted foot steps when it drifts outside a speed-widened trigger
 * radius around its rest target, gated by inhibitors — the mirrored pair
 * partner and the same-side neighbors must be planted, and a fresh-landed
 * foot holds a short cooldown. Timers never *initiate* steps (distance
 * does); they only inhibit. The swing target leads the rest target by
 * the body velocity projected over the swing duration (with one fixed-point
 * refinement, since the duration itself depends on the displaced target),
 * so feet land where the rest point will be, not where it was.</p>
 *
 ...
 *
 * <p><b>Vertical retrigger × climb assist</b> (flagged interaction): both
 * pull on the same lever — the S2 livelock guard suppresses re-steps that
 * don't change the footing, while the climb assist WANTS re-steps onto
...
public final class ModernSpiderGait {
     ...
     * comfort invalidation or the stranded re-step, the two agreed
    ...
    private void updateBodyDynamics(Mob robot) {
        ...
        // S5 seat resolution (the S3b notes, resolved per the ride design):
     ...
     * (design decision: swinging parts assert against the SERVER
     * trajectory, never the latency-lagged rendered leg).
     */
     ...
     * review, as faithful — do NOT in any slice): the mapping is exact for
     * vectors and angles, but the vanilla renderer additionally
     * translates the whole model by a constant (+1.501 blocks vertical at
     * render), and rotates by the interpolated BODY yaw while this
     * conversion — like classic's updateLegs, which uses

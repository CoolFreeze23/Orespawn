 ...
 * so feet land where the rest point will be, not where it was. A
 * vertical-only retrigger fires only when a re-step would actually change
 * the footing — otherwise a ledge or wall column more than {@link
 * #VERTICAL_RETRIGGER} off body level would re-step the same spot forever (independent-review livelock finding).</p>
 *

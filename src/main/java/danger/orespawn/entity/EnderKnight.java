...
public class EnderKnight extends Monster {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(1, new HurtByTargetGoal(this) {
            ...
            protected boolean canAttack(LivingEntity t, TargetingConditions c) {
                // orig td.bq :155-182 — the revenge memory held any living attacker but a creative one (ENT-S-107: instabuild); vanilla's canAttack would also refuse abilities.invulnerable (ENT-S-132)
        ...
        this.targetSelector.addGoal(2, new NearestAttackableTargetGoal<>(this, Player.class, 0, true, false, null) {
            {
                this.targetConditions = TargetingConditions.forNonCombat()
                        .selector(e -> e instanceof Player p && EnderKnight.this.shouldAttackPlayer(p)); // orig EnderKnight.java:67 (ENT-S-132); no range term — the 64 is the nearest search's own bound (:65), unscaled by visibility (ENT-S-135)
            }

            @Override
            protected void findTarget() {
                // orig EnderKnight.java:65 — World.getClosestPlayerToEntity(this, 64.0): the single nearest player of ANY mode within 64
                // of the knight's position (plain distSq, strict <, first wins ties; no alive / creative screen — mc1710 ahb.a(sa,D)),
    ...
    boolean shouldAttackPlayer(Player player) {
        ...
        return dot > 1.0 - 0.025 / dist && OreSpawnSight.canSee(player, this); // orig :92 — player.canEntityBeSeen(this): the PLAYER's ray under the 1.7.10 convention, routed by hand because the receiver-gated mixin cannot reach a player receiver (ENT-S-121, the Reaper's shape)
    }

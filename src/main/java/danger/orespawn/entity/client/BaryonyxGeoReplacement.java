...
/**
 * GeckoLib Baryonyx (the hook lanes, 2026-09-14, addendum item 10): {@link ModelBaryonyx#setupAnim} verbatim on the
 * converted rig, ON THE HOOK (no keyframe layer, no transcription - the self-gate stays closed until an artist
 * delivers {@code idle} and {@code walk}). Wingspeed 0.25f (orig ModelBaryonyx.java:14,69 / ClientProxyOreSpawn.java:
 * 428): the THRESHOLD gait on the six hind-leg parts ({@code cos(age * 1.3 ws) * PI * 0.15 * limbSwingAmount} above a
 * walking speed of a tenth, the right leg positive, the left the negative, the calves around -0.17) and the two front
 * claws' always-on 0.7-ws wave about Z ({@code PI * 0.25}, mirrored). No entity state. The nineteen sail spines, the six
 * crest segments and the snout ornament are zero-thickness cubes (the seam draws every cube with its true transformed
 * normal, ENT-S-161, and its two coplanar faces in the classic order - below).
 *
 * <p>Shadow follows {@link BaryonyxRenderer}'s constructor ({@code super(context, model, 1.0f)}: the literal it
 * passes - it declares no SHADOW constant); its render scale is a private 1.0, halved for a baby, applied around
 * {@code super.render} (a uniform scale commutes with the rotations and the flip, so the scale slot carries it): the
 * identity for an adult, 0.5 for a baby.</p>
 */
public final class BaryonyxGeoReplacement extends OreSpawnGeoReplacement<Baryonyx> {
    ...
    /** BaryonyxRenderer's private {@code SCALE = 1.0f}: the equal literal. */

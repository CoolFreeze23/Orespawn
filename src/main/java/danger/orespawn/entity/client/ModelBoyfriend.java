...
public class ModelBoyfriend extends HumanoidModel<Boyfriend> {
    ...
    /**
     * The parity probe's entry (the remainder slice, TEST-010 (b)): this model declares no {@code setupAnim} - the
     * classic renderer draws vanilla {@code HumanoidModel.setupAnim} (21.1.223, HumanoidModel.java:137-286) after
     * setting {@code attackTime} / {@code riding} / {@code swimAmount} per frame - so the probe, which has no entity, poses
     * through {@link HumanoidClassicPose}: the renderer's three field sets at the frame's partial tick, then vanilla's body
     * transcribed statement by statement over these parts. The in-game classic path is untouched: {@code
     * HumanoidMobRenderer} still calls vanilla's own method. The seven-float form ({@code partialTick} last) is the one
     * the probe prefers over the six-float {@code poseFrom} where a model declares it.
     */

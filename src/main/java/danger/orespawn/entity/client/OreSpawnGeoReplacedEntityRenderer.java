...
public abstract class OreSpawnGeoReplacedEntityRenderer<E extends Entity, A extends OreSpawnGeoReplacement<E>>
    ...
    protected void applyRotations(A animatable, PoseStack poseStack, float ageInTicks, float rotationYaw,
        ...
        // SLOT form - conjugated through the seam frame F = M, vanilla's own scale(-1, -1, 1) and translate(0, -1.501, 0)
        // (GeoReplacementDescriptor.RenderTransform, TEST-015) - innermost here, as the classic applies it last before
        // its parts.
        this.descriptor.renderTransform().applySlot(poseStack);
        // THE SEAM'S HEIGHT COMPENSATION (TEST-015 (2)): the classic chain lifts its origin 1.501 blocks
        // (LivingEntityRenderer.render 413-417); GeckoLib's chain lifts the bake's 1.5 datum by 0.01 (actuallyRender
        // 722-727, after this method). With the converter in the Bedrock convention the baker's x negation is the
        // classic's flip, and this translate closes the last 0.009 blocks, so the seam's chain after the slot equals M
        // exactly and the two renderers draw every rig in the same place.
     ...
     * carries. The level is the seam's own constant: this shared base names no species model.
     */
    @Override
    ...
    /**
     * ENT-S-161 (PN-027): every cube of a replacement is drawn with its quads' true transformed normals
     * - GeckoLib's own {@code renderCube} minus {@code RenderUtil.fixInvertedFlatCube}, which mangles the normal
     * of a ROTATED zero-thickness cube ({@link TrueNormalCubeRenderer}). The harness's capturing
     * renderer draws through the same static, so the proof draws what the seam draws.
     */
    @Override
    ...
    /**
     * THE CLASSIC LAYERS (the remainder slice): the vanilla {@link RenderLayer} classes a {@code HumanoidMobRenderer}
     * carries, attached to the seam's renderer and handed what they read from a classic renderer - a {@link
     * RenderLayerParent} whose model is the species' classic {@link HumanoidModel} ({@code ModelBoyfriend} / {@code
     * ModelGirlfriend}, baked from the same layer the classic renderer bakes) with its seven parts posed from the
     * bake's bones IN CLASSIC TERMS (the hooks' basis mapping inverted: {@code xRot = -rotX}, {@code yRot = -rotY}, {@code
     * zRot = rotZ}, the position through {@link OreSpawnGeoReplacement#classicPosition}; the visibility from the
     * bone) and the three renderer-set fields as {@code LivingEntityRenderer.render} sets
     * them (21.1.223 bytecode 39-100: {@code attackTime}, {@code riding}, {@code young}) - and the frame's six floats as
     * vanilla hands its layers (632-684: {@code limbSwing}, {@code limbSwingAmount}, {@code partialTick}, {@code
     * ageInTicks}, {@code netHeadYaw}, {@code headPitch} - the very inputs the classic hook received this
     * frame, {@link OreSpawnGeoReplacementModel#lastPoseInputs()}). THE FRAME: vanilla draws its layers right after
     * {@code renderToBuffer}, under the same chain M (395-417, TEST-015); the seam's bones were drawn under {@code
     * modelRenderTranslations} (the matrix {@code actuallyRender} records before the bones, 736-745) times the bake map B,
     * and {@code F_geo . B = M} under the entity yaw (the seam frame, {@code RenderTransform}), so the layers run on a pose
     * stack multiplied from this layer's level to {@code modelRenderTranslations . B} - the classic M frame. What the layers
     * then draw ({@code HumanoidArmorLayer}: the armor models posed by {@code copyPropertiesTo} from the parent's parts;
     * {@code ItemInHandLayer}: {@code translateToHand} on the parent's arm; {@code CustomHeadLayer}: the parent's head;
     * {@code ElytraLayer}: its own model from {@code copyPropertiesTo}) is the vanilla code, unchanged. The parity probe
     * cannot pose a sample wearing armor or holding an item (no entity, no bootstrapped item registry): the layers are
     * proven by construction - the same classes, the same constructor arguments, in the classic order - and by an
     * in-game look.
     *
     */

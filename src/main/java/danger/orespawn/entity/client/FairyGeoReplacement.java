 ...
 * and {@code walk}). Wingspeed 1.5f (orig ModelFairy.java:16,34 / ClientProxyOreSpawn.java:477): the outer
 * wings about Y at the wingspeed itself, the inner wings at 0.85 of it, the arms about X at 0.15 / 0.12 and about Z
 * at 0.1 / 0.11 of it (orig :134-137, :146-149), and the HEAD-LOOK idiom (orig :138-145): yaw = {@code
 * toRadians(netHeadYaw) * 0.45f} clamped to +-0.45 rad, pitch = {@code toRadians(headPitch)}. The four wings are
 * zero-thickness cubes (the seam draws every cube with its true transformed normal, ENT-S-161, and its two coplanar
 * faces in the classic order - below).
 *
...
public final class FairyGeoReplacement extends OreSpawnGeoReplacement<Fairy> {
    ...
    /** The SPEC's {@code locomotion: flyer} (tools/artist_specs/fairy.json): {@code flying} is {@code !onGround()} (contract section 3; the weights slice: every landed species carries its seed's word, the asset audit pins it). */
    @Override

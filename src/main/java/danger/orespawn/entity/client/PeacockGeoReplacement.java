...
/**
 * GeckoLib Peacock (the hooks): {@link ModelPeacock#poseFrom} verbatim on the converted rig, ON THE HOOK (no
 * keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code walk};
 * the landing slice adds the geo, the wiring and the proofs). Wingspeed 0.75f (orig ModelPeacock.java:14,33 /
 * ClientProxyOreSpawn.java:478): the THRESHOLD idiom on the two legs' pitch - above a walking speed of a tenth ({@code
 * (double) limbSwingAmount > 0.1}) {@code cos(age x 1.3 ws) x PI x 0.15 x amount}, mirrored, 0 at or below it - and the
 * DISPLAY branch ({@code getBlink() > 0}): the three head feathers pitched 0.401 / -0.174 / -0.698 and the seven
 * tail feathers raised 1.047 rad and fanned -+0.4 / 0.8 / 1.2 about Z, else the head feathers folded at -1.06 and
 * every tail feather at 0 - constants written every frame. The entity is read through {@link PeacockPose}
 * (the Slice 4b doctrine).
 *

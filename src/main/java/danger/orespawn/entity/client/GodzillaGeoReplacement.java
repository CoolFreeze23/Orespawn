...
/**
 * GeckoLib Godzilla (the hooks, owner 2026-09-14, addendum item 10; landed by the first Tier-1 slice T1a, 2026-09-15, the
 * owner's closing set item 4): {@link ModelGodzilla#poseFrom} verbatim on the converted rig, ON THE HOOK (no keyframe layer, no
 * transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code walk} ). The port's classic model
 * as it is, {@code ANIM_SPEED} 1.0f (orig ClientProxyOreSpawn.java:462 passes {@code new ModelGodzilla(0.2f)} ; the port's
 * model is what the hook transcribes): the THRESHOLD idiom on each leg ({@code limbSwingAmount > 0.001f} selects the 0.75
 * cosines / sine, else 0) with the right leg four and five eighth-turns behind; the toes' POSITION writes (y lifted
 * {@code sin x 18 x amount} while the sine is positive, z {@code 6 + 35 x cos x amount} , through {@link #moveTo} ) copied to
 * the nine toes of each foot and the lower leg, the upper leg FOLLOWING the lower by 55 units along its pitch, the thigh's z a
 * quarter of the toes' sweep; the eighteen toes' pitch reset to 0 every frame; the ATTACKING branch ({@code getAttacking() !=
 * 0}): the tail's yaw fan (0.25 .. 2.25 x a 1.75 / 0.75 cosine, the eight tail parts FOLLOWING one another by 25 / 20 / 20 / 25
 * / 27 / 28 / 18 units in x / z, the spikes copying their ring), the jaw's 1.5 cosine over 0.52 rad, the arms' 1.75 x PI x 0.16
 * swing over the 0.1 x PI x 0.02 idle drift; the HEAD-LOOK idiom (yaw {@code toRadians(netHeadYaw) x 0.55} on the head and both
 * jaws, the lower jaw's pivot FOLLOWING the head's yaw by 11 units, pitch {@code toRadians(headPitch)} on the head and the top
 * jaw, the lower jaw adding it to its opening); the two arm chains (upper arm / lower arm / hand about Y at 1x / 1.5x / 2x the
 * swing, the lower arm and the hand FOLLOWING by 50 / 45 units with 10-unit sine dips, the six finger parts riding the hand and
 * curling at 1x / 2x / 3x). The entity is read through {@link GodzillaPose} (the Slice 4b doctrine). Every value the classic
 * reads back from a part it just wrote is held in a local; the coordinates the classic never writes (the toes' and thighs' x,
 * the head's, the upper arms' and the tail base's pivots) are read through {@link #classicPosition} (the bind). <p>Scale and
 * shadow follow {@link GodzillaRenderer} : 2.0 render scale, a quarter of it while {@code getPlayNicely() != 0} , and a 1.0 x
 * 2.0 shadow (ENT-S-092). The twelve back spikes are zero-thickness cubes (the seam draws every cube with its true transformed
 * normal and its two coplanar faces in the classic order, ENT-S-161 / TEST-007 - below).</p>
 */

 ...
 * (a rig lands through the seam on its classic hook; no keyframe layer, no transcription - the self-gate stays closed
 * until an artist delivers {@code idle} and {@code walk}). Three frequencies at wingspeed 1.0f (orig
 * ModelCloudShark.java:13,24 / ClientProxyOreSpawn.java:471): {@code leftfin.yRot = 1.15f + cos(age * 0.7f * ws) * PI
 * * 0.15f}; at 1.5f {@code rightfin.yRot = -0.9f + cos * PI * 0.15f} and {@code fins.yRot = cos * PI *
 * 0.25f}; {@code jaw.xRot = 0.5f + cos(age * 0.5f * ws) * PI * 0.1f} (orig :81-87). The side fins are ROTATED
 * zero-thickness cubes and the tail fin is one: the seam draws every cube with its true transformed normal
 * ({@link TrueNormalCubeRenderer}, ENT-S-161) and its two coplanar faces in the classic order (below).
 *
 *
...
public final class CloudSharkGeoReplacement extends OreSpawnGeoReplacement<CloudShark> {
    ...
    /** The SPEC's {@code locomotion: flyer} (tools/artist_specs/cloud_shark.json): {@code flying} is {@code !onGround()} (contract section 3; the weights slice: every landed species carries its seed's word, the asset audit pins it). */
    @Override

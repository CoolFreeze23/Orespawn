...
/**
 * GeckoLib Alien (the hook survey): {@link ModelAlien#poseFrom} verbatim on the converted rig, ON THE HOOK (no
 * keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle} and
 * {@code walk}). Wingspeed 0.22f (orig ModelAlien.java:15,73 / ClientProxyOreSpawn.java:435): the GAIT-scaled legs
 * ({@code cos(age * 4.0 ws) * PI * 0.5 * limbSwingAmount}, no threshold, the four parts of each leg at offsets 0 / -0.4
 * / -0.8 / -0.8, the right leg the negative); the ATTACKING-branch head fan - folded flat at -1.85 rad at rest,
 * attacking the fifteen fan parts spread about Z in 0.261-rad steps and ripple about X on {@code
 * cos(age * 1.22 ws - k * pi/6) * PI * 0.1}; the HEAD-LOOK idiom (the neck 0.35 and the head 0.75 of {@code
 * toRadians(netHeadYaw)}, the head and its six followers FOLLOWING the neck by 3 units, the two jaws 8 units on -

...
/**
 * GeckoLib Alien (the hook survey 2026-09-14; the FK slice 2026-09-15: a real parent-child hierarchy of 26 links
 * through {@link FlatRig}, in-game behind the dev switch; the entity read through {@link AlienPose}): {@link
 * ModelAlien#poseFrom} verbatim on the flat rig, ON THE HOOK (no keyframe layer, no transcription; the self-gate stays
 * closed until an artist delivers {@code idle} and {@code walk}). Wingspeed 0.22f (orig ModelAlien.java:15,73 /
 * ClientProxyOreSpawn.java:435): the GAIT-scaled legs ({@code cos(age * 4.0 ws) * PI * 0.5 * limbSwingAmount}, no
 * threshold, the four parts of each leg at offsets 0 / -0.4 / -0.8 / -0.8, the right leg the negative); the
 * ATTACKING-branch head fan - folded flat at -1.85 rad at rest, attacking the fifteen fan parts spread about Z in
 * 0.261-rad steps and ripple about X on {@code cos(age * 1.22 ws - k * pi/6) * PI * 0.1}; the HEAD-LOOK idiom (the neck
 * 0.35 and the head 0.75 of {@code toRadians(netHeadYaw)}, the head and its six followers FOLLOWING the neck by
 * 3 units, the two jaws 8 units on - POSITION writes through {@link FlatRig#moveXZ}); and the per-entity LATCH (orig
 * :512-552, the Robot2 precedent): when the 3.5-ws rhythm crosses zero (a 0.2-tick look-ahead) the entity's own random
 * rolls {@code ri1} (claws: 1 left, 2 right, 3 both), {@code ri2} (1: the tail) and {@code ri3} (1: the jaw) from 15 each
 * at rest, 4 / 2 / 1 attacking; the five-ring tail and its spikes sway on the fast {@code PI * 0.5} or the slow {@code
 * PI * 0.05} wave (10-unit follows), the jaws open on {@code |cos|} of the fast {@code PI * 0.35} or the slow {@code PI
 * * 0.02}, and the two claws swing on {@code |angle|} multiples of the fast {@code PI * 0.2} or the slow {@code PI * 0.03} with 9 and 14-unit follows.

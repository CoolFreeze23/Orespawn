...
/**
 * GeckoLib Leonopteryx (the hook lanes, 2026-09-14; landed by the first Tier-1 slice T1a, 2026-09-15, the owner's
 * closing set item 4): the canonical registry id of the one Leon (TF-030: 1.7.10 has ONE entity, class Leon registered
 * as "Leonopteryx", orig OreSpawnMain.java:4377/4381; the port keeps {@code leon} as the save-compat alias and
 * {@code leonopteryx} as the canonical id, both built from {@link EntityLeon} and drawn by the one {@link LeonRenderer}
 * ). This descriptor shares {@link LeonGeoReplacement} 's geo, clip file, texture, scale, shadow, face order and hook
 * ({@link LeonGeoReplacement#poseRig}) under its own registry path - one profile per registry path even for a shared
 * rig (the Ant precedent).
 */

 ...
 *
 * <p>The missing-key policy (the reading of below was presented to the owner with the
 * landing). A rig that ships WITHOUT the key is not a build error of this mod but the
 * ordinary shape of a third-party resource pack: Blockbench's bedrock exporter rewrites
 * {@code description} and drops every foreign key, so a pack that re-exports one of the
 * shipped rigs loses the order - and the owner's rule is that a resource pack must never
 * crash the client. So {@link #read} answers an ABSENT key with an empty order, and
 * {@link #applyOrFallback} answers an empty order by leaving GeckoLib's own bone order
 * in place (the mob still draws; only its self-overlapping faces may
 * resolve the other way), logged once per resource at WARN. A key that is PRESENT and
 * wrong - a non-array, a non-string or repeated entry, an empty array, a name the rig
 * lacks, a rig bone the key lacks, an order that is not a pre-order of the rig's tree -
 * does not crash the client either, because "never" is absolute: it is loud as an ERROR
 * log, once per resource, naming the geo, the exact reason ({@link #read}'s or {@link
 * #apply}'s own message) and the pack author's fix (carry the {@code
 * orespawn:bone_draw_order} array over from the shipped rig's {@code description}), and
 * then takes the same fallback - {@link #apply} validates everything before it touches the
 * bake, so the order it falls back to is exactly the factory's. The alternative reading (a
 * crash report naming the geo) is one line away and left open. The shipped rigs must
 * never take either fallback: the asset audit ({@code tools/asset_audit.py};
...
public final class DrawOrder {
     ...
     * {@link FaceOrder} rather than parsing the resource once per key (ENT-S-146, one parse per bake
     * and reload, for every seam rig). The resource must exist and parse - GeckoLib baked it
     * moments earlier - so a failure is its own {@link IllegalStateException}, carrying the cause;
     * {@link #unreadable} is the seam's answer to it.
     */
     ...
     * differs from the shipped list; anything else is a rig that drifted from its
     * contract, and throws. Every check runs BEFORE anything is touched (on the
     * landing): the set both ways on the tree as it stands, then the pre-order against a
     * sorted COPY of the lists, and only then the in-place sort - so a wrong order
     * leaves the bake exactly as found, which is what the seam falls back to. An empty
     * order is refused here too: the missing-key fallback is {@link
     * #applyOrFallback}'s, never this method's.
     */

...
/**
 * GeckoLib Hammerhead (the hooks, owner 2026-09-14, addendum item 10): {@link ModelHammerhead#poseFrom} verbatim on the
 * converted rig, ON THE HOOK (no keyframe layer, no transcription - the self-gate stays closed until an artist delivers
 * {@code idle} and {@code walk}; the landing slice adds the geo, the wiring and the proofs). Wingspeed 0.33f (orig
 * ModelHammerhead.java:14,54 / ClientProxyOreSpawn.java:501): the THRESHOLD idiom on the twelve leg parts' pitch -
 * above a walking speed of a tenth ({@code (double) limbSwingAmount > 0.1}) {@code cos(age x 1.3 ws) x PI x 0.1 x
 * amount} and the same an eighth-turn ahead for the middle pair, 0 at or below it - around the rest pitches -0.087 /
 * -0.052 / -0.349 (the lower parts around 0, the right side mirrored); the HEAD-LOOK idiom (yaw {@code

...
/**
 * GeckoLib Hammerhead (the hooks, owner 2026-09-14, addendum item 10; landed by the first Tier-1 slice T1a, 2026-09-15,
 * the owner's closing set item 4): {@link ModelHammerhead#poseFrom} verbatim on the converted rig, ON THE HOOK (no
 * keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code walk}
 * ; the geo, the wiring and the proofs landed with T1a). Wingspeed 0.33f (orig ModelHammerhead.java:14,54 /
 * ClientProxyOreSpawn.java:501): the THRESHOLD idiom on the twelve leg parts' pitch - above a walking speed of a tenth
 * ({@code (double) limbSwingAmount > 0.1}) {@code cos(age x 1.3 ws) x PI x 0.1 x amount} and the same an eighth-turn
 * ahead for the middle pair, 0 at or below it - around the rest pitches -0.087 / -0.052 / -0.349 (the lower parts
 * around 0, the right side mirrored); the HEAD-LOOK idiom (yaw {@code toRadians(netHeadYaw) x 0.25} on the neck, its
 * armour, the four horns and their base, the head, the snout and the centre fan, the side fans and ears offset by
 * -+0.122 / 0.226 / 0.227); the back armour's yaw sway at 0.3 ws x PI x 0.03 around -+0.349; and the ATTACKING branch
 * ({@code getAttacking() != 0}: {@code cos(age x 1.3 ws) x PI x 0.13} , else 0) nodding the sixteen neck / head / horn
 * / fan / ear parts over their rest pitches (0.157 / 0.087 / 0.192 / 0.209 / 0.611 / -0.139 / -0.209 / -0.331 / 0.366).
 * The entity is read through {@link HammerheadPose} (the Slice 4b doctrine).
 *

...
/**
 * ENT-S-161 (PN-027): GeckoLib 4.8.4's default {@code GeoRenderer.renderCube} (bytecode 0-126)
 * transforms every quad's normal by the pose's normal matrix (offsets 76-93) and then hands it to {@code
 * RenderUtil.fixInvertedFlatCube(cube, normal)} (98) before {@code createVerticesOfQuad} (115). That
 * helper (0-129) multiplies the normal's x by -1 when x is negative and the cube is flat in y or z, its
 * y by -1 when y is negative and the cube is flat in x or z, and its z by -1 when z is negative
 * and the cube is flat in x or y - component-wise, on the TRANSFORMED (pose-space) normal. A flat cube
 * whose pose-space normals stay axis-aligned survives (the harness's bind pose without entity yaw: the
 ...
 * renderer keeps stock semantics until her own decision), recorded as PN-027 with the upstream report text.</p>
 */

...
/**
 * GeckoLib Leon (the hook survey): {@link LeonModel#poseFrom} verbatim on the converted rig, ON THE HOOK (no
 * keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle} and
 * {@code walk}). Wingspeed 0.22f (orig ModelLeon.java:15,116 / ClientProxyOreSpawn.java:500). The rig carries TWO part
 * sets and draws exactly one per frame by the entity's activity (orig ModelLeon.java:729/852; the port's {@code
 * visible} toggles, TF-030) - here {@link #setVisible} on the forty-nine standing and forty-nine flying parts.
 * STANDING (activity 0): the THRESHOLD gait ({@code cos(age * 1.8 ws) * PI * 0.25 * limbSwingAmount} above a

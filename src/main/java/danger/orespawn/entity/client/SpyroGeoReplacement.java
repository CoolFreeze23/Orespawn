...
/**
 * GeckoLib Spyro (the hooks, landed by the sixth Tier-2 slice T2f): {@link SpyroModel#poseFrom} verbatim on the
 ...
 * {@code limbSwingAmount} itself (SpyroModel.poseFrom {@code float ws = limbSwingAmount} ; orig ModelSpyro.java ran on
 * a wingspeed of 0.65f, ClientProxyOreSpawn.java:427 - the register line of the landing slice records where the
 * port's pose departs from the 1.7.10 one) - the THRESHOLD idiom on the wings about Z ({@code cos(age x 2.3 x ws) x PI
 * x 0.4 x limbSwingAmount} above a walking speed of a tenth, mirrored, halved by the ACTIVITY branch while flying
 * (3)) and on the twelve leg parts about X ({@code cos(age x 2.0 x ws) x PI x 0.25 x limbSwingAmount}, zeroed while
 * flying, alternating about the rests -0.087 / -0.17 / 0 / 0.139 / -0.174; activity 2 folds them at -1 rad (front) /
 * +1 rad (back)); the tail about Y on {@code cos(age x 1.2 x ws) x PI x 0.25} , stilled by the SITTING check or flying,
 * the front link at 1.6 of it and its pivot FOLLOWING the back link 3 units along (sin, cos) of the back link's yaw,
 * less 0.5 in x (the POSITION-write idiom, x and z through {@link #moveXZ} ; the back link's pivot is never written -
 * the bind), the two tail pieces riding the front link; and the HEAD-LOOK idiom in radians on twelve head parts,
 * the horns offset +-0.785 rad in yaw and -0.785 in pitch. The entity is read through {@link SpyroPose} (the Slice 4b
 * form). The two wings are zero-thickness cubes (ENT-S-161; the classic face order required below).
 *
 *
...
public final class SpyroGeoReplacement extends OreSpawnGeoReplacement<EntitySpyro> {
    ...
    /** The SPEC's {@code locomotion: flyer} (tools/artist_specs/spyro.json): {@code flying} is {@code !onGround()} (contract section 3; the weights slice: every landed species carries its seed's word, the asset audit pins it). */
    @Override

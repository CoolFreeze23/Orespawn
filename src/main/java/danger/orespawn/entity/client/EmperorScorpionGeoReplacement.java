...
/**
 * GeckoLib Emperor Scorpion (the hook lanes 2026-09-14; the FK slice 2026-09-15: a real parent-child hierarchy of 52 links
 * through {@link FlatRig}, in-game behind the dev switch; the entity read through {@link EmperorScorpionPose}): {@link
 * EmperorScorpionModel#poseFrom} verbatim on the flat rig, ON THE HOOK (no keyframe layer, no transcription - the self-gate
 * stays closed until an artist delivers {@code idle} and {@code walk}). Wingspeed 0.22f (orig
 * ModelEmperorScorpion.java:15,96 / ClientProxyOreSpawn.java:426): the GAIT-scaled eight-leg walk - four phases a quarter
 * turn apart ({@code cos(age * 2.0 ws - k * 1.570795) * PI * 0.12 * limbSwingAmount}, the left leg positive, the mirrored
 * right leg the negative) with a LOOK-AHEAD lift ({@code nextangle} at age + 0.1 rising selects {@code 0.47 *
 * limbSwingAmount - |newangle|}) folded down each five-segment leg as yaw, a zRot pair and a POSITION follow through {@link
 * FlatRig#moveTo} (6 / 9 / 1 units along, 11.5 x sin of the zRots up); the ATTACKING-branch mandibles ({@code 0.5 ws x PI x
 * 0.05} at rest, {@code 2.5 ws x PI x 0.15} attacking, about Z, mirrored); and the per-entity LATCH (orig :613-641, the
 * Robot2 precedent): when the 3.0-ws rhythm crosses zero the entity's own random rolls {@code ri1} (claws: 1 left, 2 right,
 * 3 both) and {@code ri2} (1: the tail) from 20 / 25 at rest, 4 / 3 attacking, and the claws (a yaw and a 12-unit cosine
 * follow of four parts) and the eleven-part tail chain (pitch offsets accumulating, each ring following the last by 9 / 10
 * / 3 units of sin / cos) pose on {@code newangle} or 0.

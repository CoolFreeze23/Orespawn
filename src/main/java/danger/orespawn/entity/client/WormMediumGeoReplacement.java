 ...
 * delivers {@code idle} and {@code walk}). orig ModelWormMedium.java:79-125: a three-segment chain of 12-unit links -
 * the tail pitches (0.45f) and rolls (0.25f), the body's pivot FOLLOWS the tail ({@code z = tail.z - sin(pitch) * 12},
 * {@code x = tail.x + sin(roll) * cos(pitch) * 12}, {@code y = tail.y - 12 + (12 - cos(roll) * cos(pitch) * 12)})
 * and pitches (0.35f) / rolls (0.15f) itself, the head and its inner copy follow the body the same way and pitch on
 * {@code 0.62f + cos(age * 0.55f) * PI * 0.15f} / roll on a 0.25f cosine; the four teeth follow the head one more
 * link on, offset +-1 unit (z for the top pair, x for the side pair), and open on a 0.55f cosine about 0.4 rad (X
 * for the top pair, Z for the side pair). A POSITION-write idiom through {@link #moveTo}; the tail's
 * pivot is never written (the bind), read through {@link #classicPosition}; every value the classic reads back from
 * a part it just wrote is held in a local.
 *

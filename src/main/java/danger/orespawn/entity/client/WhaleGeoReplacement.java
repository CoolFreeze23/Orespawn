 ...
 * {@code cos(age x 0.4) x PI x 0.16 x limbSwingAmount} (idle {@code cos(age x 0.05) x PI x 0.03}) at 0.5 / 1.25 / 2.25
 * of it down the three links - the jaw on a 0.03 cosine at 0.02 x PI about 0.087 rad; and the POSITION-write idiom:
 * the second tail link's pivot FOLLOWS the first 14 units along (cos, -sin) of its pitch and the two flukes follow the
 * second 8 units along its pitch (y and z only, through {@link #moveYZ}; the first link's pivot is never written - the
 * bind, read through {@link #classicPosition}); every value the classic reads back from a part it just wrote is held
 * in a local.
 *

...
public final class VortexGeoReplacement extends OreSpawnGeoReplacement<EntityVortex> {
    private static final GeoReplacementDescriptor<EntityVortex> DESCRIPTOR = new GeoReplacementDescriptor<>(
            () -> ModEntities.ENTITY_VORTEX.get(),  // lambda: a bound method ref would initialise ModEntities eagerly and trip Bootstrap.checkBootstrapCalled in a headless leg (OPT-029 R0)
            EntityVortex.class,
        ...
        /**
         * TEST-007: the Vortex's plate is a zero-thickness cube (128x64x0), so the rig ships the classic
         * within-cube face order ({@code orespawn:cube_face_order}, written by the converter from the manifest
         * entry's {@code cube_face_order: "classic"}) and the seam expects it - the plate's two coplanar faces
         * are then emitted in the classic renderer's order on the candidate.
         */
        @Override
    ...
    /** The SPEC's {@code locomotion: flyer} (tools/artist_specs/vortex.json): {@code flying} is {@code !onGround()} (contract section 3; the weights slice: every landed species carries its seed's word, the asset audit pins it). */
    @Override

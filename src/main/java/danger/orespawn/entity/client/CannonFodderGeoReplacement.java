 ...
 * delivers {@code idle} and {@code walk}). A port-authored six-part quadruped (1.7.10's {@code EntityCannonFodder}
 * was an unregistered tameable base class with no model or renderer of its own, so this rig has no 1.7.10 pair and
 * no reference leg): the HEAD-LOOK idiom (yaw and pitch in radians) and a vanilla-style gait on the walk
 * DISTANCE - {@code cos(limbSwing * 0.6662f) * 1.2f * limbSwingAmount}, the front-left and back-right legs positive,
 * the other two the negative (the code-driven form: a {@code limbSwing} gait is not a transcription-form channel,
 * so it runs on the hook).
 *

...
/**
 * GeckoLib Kraken (the hooks, owner 2026-09-14, addendum item 10): {@link ModelKraken#poseFrom} verbatim on the
 ...
 * it is, {@code ANIM_SPEED} 1.0f (orig ClientProxyOreSpawn.java:444 {@code new ModelKraken(1.0f)}): the two fins' roll
 * on 0.43 / 0.32 cosines (Z, x PI x 0.15 / 0.14); the six eight-ring tentacles through {@link #dangleTentacle} (the
 * classic helper, the same name): the root ring pitched and yawed on the tentacle's own {@code differ} / {@code ydiffer}
 * cosines around its {@code xoff} / {@code yoff}, each following ring's pivot FOLLOWING the last 30 units along its
 * pitch and yaw (POSITION writes through {@link #moveTo}) and lagging a further 0.314159 rad, the two front tentacles'
 * sign flipped for the right one and the ATTACKING branch ({@code getAttacking() != 0}) stiffening them (0.5 / 0.03 /
 * no offset over 0.2 / 0.1 / -0.25); the two suction cups riding 30 units past the eighth ring and copying its angles;
 * the mouth TWITCH latch (the Robot2 precedent, orig ModelKraken.java:1045-1057): on the falling zero crossing of a 0.66
 * cosine (the 0.1-tick look-ahead) the per-entity {@code RenderInfo}'s {@code ri1} / {@code ri2} are re-rolled from the

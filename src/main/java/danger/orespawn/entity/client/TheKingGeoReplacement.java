...
/**
 * GeckoLib The King (the hooks, landed by the remainder slice, 2026-09-15 - TEST-018, the second pass declared
 * and proven pass by pass): {@link ModelTheKing#poseFrom} verbatim on the converted rig, ON
 * THE HOOK (no keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle}
 * and {@code walk}; the geo, the wiring and the proofs landed with the remainder). The port's classic model as it
 * is, {@code WING_SPEED} 1.0f (orig ClientProxyOreSpawn.java:492 passes {@code new
 * ModelTheKing(0.65f)}; the port's model is what the hook transcribes); every rhythm on the ATTACKING branch ({@code
 * getAttacking() != 0}): the wings' roll (0.75 ws x PI x 0.21 attacking, 0.35 ws x 0.15 gliding) on the ten parts of
 * each wing - the third at five thirds and the fifth at seven thirds of it, the third's pivot FOLLOWING the root 84
 * units along its roll and the fifth's the third by 184 (POSITION writes through {@link #moveTo}), the even
 * parts and the four feather parts riding the third at -+0.261, the right wing mirrored; the thirteen claw parts
 * flexing on a 0.75 ws cosine x PI x 0.25 (else 0) around -0.925 / 0.384 / 0.645; the legs' swing on a 0.6 ws cosine x
 * PI x 0.45 (else 0) - a quarter of it on the thigh and upper leg around 0.785, half on the lower leg and foot around
 * -0.628, the lower leg FOLLOWING the upper by 50 units and the seven claws of each foot the lower leg by 66 (at
 * -0.1 / +0.15 rad); the tail's yaw chain at 0.56 / 0.19 attacking over 0.26 / 0.08, lagging pi / 4 per ring and
 * FOLLOWING by 54 (a -1 x nudge) / 42 / 41 / 34 / 34 / 40 / 43 / 58 units, the two rear ridges copying the first
 * ring's yaw and the sixth ridge the seventh ring; and the three heads through {@link #moveLeftHead} /
 * {@link #moveCenterHead} / {@link #moveRightHead} (the classic helpers, the same names: the one statement sequence
 * over each head's part set): their sweeps (attacking 0.3 / 0.28 / 0.32 ws sines x PI x 0.25 sideways and 0.2 /
 * 0.19 / 0.21 ws up-down, the jaws on 0.85 / 0.75 / 0.95 ws x 0.12 over 0.5; idle 0.17 / 0.13 / 0.19 ws x 0.08
 * sideways, 0.13 / 0.08 / 0.12 ws x 0.1 up-down, the jaws 0.45 / 0.65 / 0.55 ws x 0.04 over 0.25 - each jaw adding
 * its head's pitch), the side heads clamped to the centre head's sweep, and each head's four neck rings yawed and
 * pitched at 0.125 / 0.25 / 0.38 / 0.5 of it, FOLLOWING one another by 20 / 36 / 36 / 36 units first sideways, then
...
public final class TheKingGeoReplacement extends OreSpawnGeoReplacement<TheKing> {
    ...
    /** The SPEC's {@code locomotion: flyer} (tools/artist_specs/the_king.json): {@code flying} is {@code !onGround()} (contract section 3; the weights slice: every landed species carries its seed's word, the asset audit pins it). */
    @Override

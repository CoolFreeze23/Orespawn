...
/**
 * GeckoLib The King (the hooks): {@link ModelTheKing#poseFrom} verbatim on the converted rig, ON THE HOOK (no
 * keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code walk};
 * the landing slice adds the geo, the wiring and the proofs). The port's classic model as it is, {@code WING_SPEED} 1.0f
 * (orig ClientProxyOreSpawn.java:492 passes {@code new ModelTheKing(0.65f)}; the port's model is what the hook
 * transcribes); every rhythm on the ATTACKING branch ({@code getAttacking() != 0}): the wings' roll (0.75 ws x PI x
 * 0.21 attacking, 0.35 ws x 0.15 gliding) on the ten parts of each wing - the third at five thirds and the fifth at
 * seven thirds of it, the third's pivot FOLLOWING the root 84 units along its roll and the fifth's the third by 184
 * (POSITION writes through {@link #moveTo}), the even parts and the four feather parts riding the third at
 * -+0.261, the right wing mirrored; the thirteen claw parts flexing on a 0.75 ws cosine x PI x 0.25 (else 0) around
 * -0.925 / 0.384 / 0.645; the legs' swing on a 0.6 ws cosine x PI x 0.45 (else 0) - a quarter of it on the thigh

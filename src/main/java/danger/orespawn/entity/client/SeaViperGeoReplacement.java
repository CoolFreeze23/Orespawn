...
/**
 * GeckoLib Sea Viper (the hooks, owner 2026-09-14, addendum item 10; landed by the sixth Tier-2 slice T2f, 2026-09-15,
 * the owner's closing set item 4): {@link ModelSeaViper#poseFrom} verbatim on the converted rig (ENT-S-091 slice C's
 * line-for-line transcription of orig ModelSeaViper.java:295-402), ON THE HOOK (no keyframe layer, no transcription -
 * the self-gate stays closed until an artist delivers {@code idle} and {@code walk} ; the geo, the wiring and the
 * proofs landed with T2f). Wingspeed 0.5f (orig ModelSeaViper.java:14,51 / ClientProxyOreSpawn.java:497): a negative
 * walking speed clamped to 0 (orig :298-301); the base segment's yaw {@code cos(age x 1.3 ws) x PI x 0.1 x amount}
 * (orig :302) and the twenty-one segments chained off their predecessor through {@link #doseg} (the classic helper, the
 * same name; orig :394-402): each 9 units along the previous segment's yaw foreshortened by {@code |cos(pitch)|}
 * (POSITION writes through {@link #moveTo} ; the segments' pitch is never written - the bind, read through
 * {@link #classicRotX} , the base's since T2f) and yawed on a travelling wave lagged {@code pi / 4} per index blended
 * toward the static S-curve as the swing amount drops (index 2 twice, as the original); the ATTACKING branch (orig
 * :324-346 {@code getAttacking() != 0} ): the jaw chattering wide (0.65 + 1.7 ws x PI x 0.17) and the tongue's four
 * parts flicking on a 4.7 ws cosine with a 1.5 ws z offset, else the idle breath (0.45 + 0.2 ws x PI x 0.02, the tongue
 * at 1.7 ws with a 0.5 ws offset) - the offset the original wrote as {@code offsetZ} in block units, folded into the
 * pivot's z as the bind plus sixteen times it (orig :332-334 / :343-345, the port's fold); and the HEAD-LOOK idiom
 * (orig :347-357: yaw {@code toRadians(netHeadYaw) x 0.5} on the eyes, the mouth, the head, the fangs and the tongue,
 * the forks splayed -+0.436, the lower jaw's pivot FOLLOWING the head's yaw by 2 units). The entity is read through
 * {@link SeaViperPose} (the Slice 4b doctrine). Every value the classic reads back from a part it just wrote is held in
 * a local; the base segment's and the head's pivots, never written, are read through {@link #classicPosition} (the
 * bind).

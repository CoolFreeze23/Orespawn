...
/**
 * GeckoLib The Prince (the hooks, owner 2026-09-14, addendum item 10): {@link ModelThePrince#poseFrom} verbatim on the
 ...
 * ModelThePrince.java:15,53 / ClientProxyOreSpawn.java:494): the THRESHOLD-OR-ATTACKING idiom on the six wing parts'
 * roll ({@code (double) limbSwingAmount > 0.1 || getAttacking() != 0}: a 2.3 ws cosine x PI x 0.4 x amount, else a 0.3
 * ws x 0.04 breath, around -+0.4 / 0.6 / 0.2); the THRESHOLD idiom on the legs (2.0 ws x PI x 0.25 x amount, else 0)
 * under the ACTIVITY branch ({@code getActivity() != 2 || attacking}: the gait, else both legs tucked at -1.0); the tail
 * sway (0.9 ws x PI x 0.06) stilled by the SIT order ({@code isOrderedToSit()}) and overridden by the ATTACKING lash
 * (1.3 ws x PI x 0.12), fanned x1.6 / 2.6 / 3.6 / 4.6 down the five tail rings FOLLOWING one another by 6 / 5 / 4 / 4
 * units in x / z (POSITION writes through {@link #moveTo}); the three-head LOOK split (two thirds of the head yaw /
 * pitch, the side heads at half of it on the side the head turns to, in radians on each head's head / snout / fin /
 * jaw, half on its neck) with the ATTACKING chatter on the three jaws (1.9 / 2.1 / 2.3 ws cosines x PI x 0.2 + 0.2);
 * the three necks pitched by the entity's HEAD EXTENSIONS ({@code getHead1Ext / 2 / 3}, degrees) and each head group

...
/**
 * GeckoLib Basilisk (the hook lanes, 2026-09-14, addendum item 10; landed by the first Tier-1 slice T1a, 2026-09-15, the
 * owner's closing set item 4): {@link ModelBasilisk#poseFrom} verbatim on the converted rig, ON THE HOOK (no keyframe layer,
 * no transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code walk} ). Wingspeed 0.3f
 * (orig ModelBasilisk.java:14,38 / ClientProxyOreSpawn.java:420): the GAIT-scaled serpentine ({@code cos(age * 1.3 ws - k *
 * pi4) * PI * 0.1 * limbSwingAmount}, no threshold) down the ten-ring body and tail chain, each ring a quarter turn behind
 * the last and its pivot FOLLOWING the last ring's yaw through {@link #moveTo} (12 / 11 / 12 / 12 / 12 / 12 / 10 / 10 / 10
 * units along cos / sin, the tail rings a further 0.5 / 1.0 / 1.5 / 1.0 / 1.0 units back); and the ATTACKING-branch jaw
 * ({@code -1.0 + cos(age * 0.45) * PI * 0.18} attacking, -1.1 at rest). The entity is read through {@link BasiliskPose} .

 ...
 *
 * <p>ANIM-025, THE DRAW FIX (the fourth Tier-2 slice T2d, owner 2026-09-14, item 3: "the port's renderToBuffer fix
 * lands with the Crab's slice, one refuter, exempt from the freeze as a rig matter"): 1.7.10's {@code render} (orig
 * ModelCrab.java:195-289) set the three leg parts' rotation point and yaw and DREW them at each of eight poses - four on
 * the left side (x 36) at z 0 / 10 / 20 / 30 with yaw -pi/2 + a, -pi/2 - a, -pi/2 + a, -pi/2 - a, then four on the right
 * (x -36) with the yaw negated, {@code a = cos(f2 * 1.7f) * PI * 0.15f * f1} - eight three-segment legs. The port had
 * moved the eight re-poses into {@code setupAnim} and drew each part ONCE at the last pose (one leg). Now
 * {@link #poseFrom} keeps the two inputs those poses read ({@code f1} = limbSwingAmount, {@code f2} = ageInTicks) and
 * {@link #renderToBuffer} re-poses and draws {@code leg1}, {@code leg2}, {@code leg3} eight times from them with the
 ...
 * transforms ({@code reference_crab}, {@code render_instances}, twenty-four leg bones). orig :310-311 drew {@code leg2}
 * and {@code leg3} a NINTH time at the eighth pose - a coincident duplicate (the same pose, the same triangles) the
 * ruled declaration's count of eight per part does not carry; not reproduced.</p>
 */

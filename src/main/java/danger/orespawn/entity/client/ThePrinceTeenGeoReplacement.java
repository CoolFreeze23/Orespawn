...
/**
 * GeckoLib The Prince (teen) (the hooks, owner 2026-09-14, addendum item 10): {@link ModelThePrinceTeen#poseFrom}
 * verbatim on the converted rig, ON THE HOOK (no keyframe layer, no transcription - the self-gate stays closed until an
 * artist delivers {@code idle} and {@code walk}; the landing slice adds the geo, the wiring and the proofs). Wingspeed
 * 0.65f (orig ModelThePrinceTeen.java:16,90 / ClientProxyOreSpawn.java:503): the wings by ACTIVITY and ATTACKING (a
 * 1.3 ws x PI x 0.2 x amount walking beat above a tenth, else a 0.3 ws x 0.04 breath; flying a 1.4 ws x 0.4 beat;
 * attacking 1.7 ws x 0.4) on the four links' roll (x1.25 on the second, around -0.4 / -0.6 / -0.2), the second link's
 * pivot FOLLOWING the first 22 units along its roll (POSITION writes through {@link #moveTo}), the four membranes

 ...
 * ON THE HOOK (and: the threshold question closed with option (c) - the Cricket lands through the seam on its classic
 * hook, no {@code limb_swing_threshold} property, no keyframe layer, no transcription; the self-gate stays closed
 * until an artist delivers {@code idle} and {@code walk}). The port's classic model as it is: the THRESHOLD
 * idiom on the four front / rear legs' yaw - above a walking speed of a tenth {@code cos(age * 1.0f) * PI * 0.25f *
 * limbSwingAmount} around their rest yaws (0.47, -0.54, -0.296, 0.384 rad), 0 at or below it (orig
 * ModelCricket.java:104-108, wingspeed 2.5f there where the port's frequency is 1.0f - the register line of this
 * slice records where the port's pose departs from the 1.7.10 one) - and the four hind legs' constant yaws
 * (+-0.436 / +-0.349 rad) and pitches (0.558 / -0.366 rad) written every frame (orig :117-125, the not-singing
 * branch).
 *

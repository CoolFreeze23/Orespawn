 ...
 * pairs on {@code cos(age * 2.0f * ws - k * 1.570795f) * PI * 0.12f * limbSwingAmount}, k = 0 / 1 / 2, the right side the
 * negative (orig :551-592; no threshold) - and the CLAW-SNAP LATCH (orig :593-612, the Robot2 precedent): at each falling
 * zero-crossing of a 3.0 ws cosine the per-entity {@link RenderInfo}'s {@code ri1} / {@code ri2} are re-rolled from the
 * entity's RNG, the ranges by the ATTACKING flag (20 / 25 at rest, 4 / 3 attacking), and while {@code ri1} is 1 or 3 the
 * two claws lift on {@code |cos|} through {@link #doLeftClaw} / {@link #doRightClaw} (orig :701-721, the helpers
 * transcribed under their names), else they rest at 0 / -0.54 / +0.35. The entity is read through {@link CaveFisherPose}
 * (ENT-S-093's interface, already on the entity: the render scratch, the attacking flag, the RNG).
 *

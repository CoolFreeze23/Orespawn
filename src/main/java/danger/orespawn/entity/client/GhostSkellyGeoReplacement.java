...
/**
 * GeckoLib Ghost Skelly (the hook lanes, 2026-09-14, addendum item 10; landed by the fifth Tier-2 slice T2e, 2026-09-15,
 * the owner's closing set item 4): {@link GhostSkellyModel#poseFrom} verbatim on
 * the converted rig, ON THE HOOK (no keyframe layer, no transcription - the self-gate stays closed until an artist
 * delivers {@code idle} and {@code walk}). No wingspeed: the two arm groups (sleeve, chains, arm) sway about Z at 0.2 /
 * 0.22 and about Y at 0.24 / 0.26, every one {@code PI * 0.05}; and the HEAD-SWIVEL LATCH (orig ModelGhostSkelly.java:
 * 99-122): the 0.05 rhythm's phase {@code |age * 0.05 mod 2 PI|} is compared with the last frame's ({@code rf2}) and on
 * each wrap the entity's own random decides ({@code nextInt(3) == 1}) whether {@code ri2} carries the swivel bit; the
 * head yaws a full {@code cos(age * 0.05) * PI * 2} while it does and holds 0 otherwise. The entity is read through
 * {@link GhostSkellyPose} (the Robot2 precedent for a per-entity RenderInfo latch).
 *
 ...
 * lanes returned the bare factory because the model carried no such object, and the landing slice T2e re-based it) so
 * the harness proves the two renderers' render type equal by identity.</p>
 */

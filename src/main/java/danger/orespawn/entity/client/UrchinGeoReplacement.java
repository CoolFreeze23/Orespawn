...
/**
 * GeckoLib Urchin (the hooks): {@link ModelUrchin#poseFrom} verbatim on the rig the landing slice converts, ON THE HOOK
 * (no keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code
 * walk}). Wingspeed 1.0f (orig ModelUrchin.java:14,34 / ClientProxyOreSpawn.java:487): the THRESHOLD idiom on the
 * eight fins - above a walking speed of a tenth, five gait cosines (0.7 / 1.7 / 1.65 / 1.75 / 1.8 x ws) at 0.15 x PI
 * x limbSwingAmount about their rest pitches and rolls (+-0.261 / +-0.523 rad), 0 at or below it - and the
 * ATTACKING branch (orig :175 {@code getAttacking() != 0}) that spins the center about Y on {@code (age x 0.2) mod
 * 2 PI} (0.02 at rest) and rocks the eight spines on eight cosines (0.7 ... 0.35 x ws at 0.06 x PI attacking, 0.07
 * ... 0.035 x ws at 0.02 x PI at rest) about their rest angles. The entity is read through {@link UrchinPose} (the
 * Slice 4b form).
 *

...
public class GhostModel<T extends Ghost> extends EntityModel<T> {
    /**
     * ENT-S-160 (a), the remainder slice (the ENT-S-146 move): the render-type FUNCTION the ghost is drawn with - {@code
     * RenderType::entityTranslucent}, the factory {@link GhostRenderer#getRenderType} applied as its own decision (the ghost.png
     * alpha honoured; the 1.7.10 GL_BLEND look) - stored on the model as {@code EntityModel(Function)} does (the {@link
     * FairyModel#RENDER_TYPE} / {@link GhostSkellyModel#RENDER_TYPE} form) so the GeckoLib descriptor hands over this very object
     * and the parity harness proves the two renderers' render type equal by identity. The classic path reads it back through
     * {@link GhostRenderer#getRenderType}, which applies this function to the texture as {@code Model.renderType} would;
     * {@code LivingEntityRenderer.getRenderType} itself is overridden there, as before.
     */

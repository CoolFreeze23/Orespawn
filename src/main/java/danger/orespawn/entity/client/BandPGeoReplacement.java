...
/**
 * GeckoLib Bandit / Pirate (the hook survey, landed by the fourth Tier-2 slice T2d): {@link
 * ModelBandP#setupAnim} verbatim on the converted rig, ON THE HOOK (no
 * keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code
 * walk}). Wingspeed 0.4f (orig ModelBandP.java:14,24 / ClientProxyOreSpawn.java:504): the THRESHOLD idiom - above a
 * walking speed of a tenth the legs swing at {@code cos(age * 1.3 ws) * PI * 0.25 * limbSwingAmount} (mirrored),
 * the arms the same swing (the left the negative), the belly bobs at {@code cos(age * 2.6 ws) * PI * 0.025 *
 * limbSwingAmount} around 0.07 and twists on minus half the leg swing; at or below it the legs hold, the belly breathes
 * at {@code cos(age * 0.6 ws) * PI * 0.005} and the arms sway at {@code cos(age * 0.3 ws) * PI * 0.02} - and the
 * HEAD-LOOK idiom (yaw and pitch in radians). No entity state.
 *

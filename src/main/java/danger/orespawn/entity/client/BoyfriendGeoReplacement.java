...
/**
 * GeckoLib Boyfriend (drafted by the fourth Tier-2 slice T2d, 2026-09-14, TEST-010 (b); landed by the remainder slice,
 * 2026-09-15, under the owner's closing set item 3 - "the seam carries the renderer's partial tick in PoseInputs; the biped
 * hooks read getAttackAnim(partialTick) verbatim; the classic item, armor, head and elytra layers are drawn by the seam's
 * renderer for those species as the classic renderer draws them"): the classic pose verbatim on the converted rig, ON THE
 * HOOK (Amendment 2: no keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle}
 * and {@code walk}). {@code ModelBoyfriend} declares no {@code setupAnim}: the pose its classic renderer draws is vanilla
 * {@code net.minecraft.client.model.HumanoidModel.setupAnim} (NeoForge 21.1.223, HumanoidModel.java:137-286 by the jar's
...
public final class BoyfriendGeoReplacement extends OreSpawnGeoReplacement<Boyfriend> {
    ...
    public static final class Renderer extends OreSpawnGeoReplacedEntityRenderer<Boyfriend, BoyfriendGeoReplacement> {
        public Renderer(EntityRendererProvider.Context context) {
            ...
            // THE CLASSIC LAYERS (owner's closing set item 3 (3)): the vanilla layer classes BoyfriendRenderer carries, in its order -
            // HumanoidMobRenderer(Context, M, float) adds the CustomHeadLayer, the ElytraLayer and the ItemInHandLayer (21.1.223
            // bytecode 7-67), then BoyfriendRenderer adds the HumanoidArmorLayer over the player armor layers (:30-33) - attached
            // to the seam's renderer through the adapter that hands them a classic ModelBoyfriend posed from the bake's bones.

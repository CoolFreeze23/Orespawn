...
/**
 * GeckoLib Nastysaurus (the hooks): {@link ModelNastysaurus#poseFrom} verbatim on the converted rig, ON THE HOOK (no
 * keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code
 * walk}; the landing slice adds the geo, the wiring and the proofs). Wingspeed 0.65f (orig
 * ModelNastysaurus.java:15,77 / ClientProxyOreSpawn.java:508): the HEAD-LOOK idiom (yaw {@code netHeadYaw % 360 x 0.35}
 * in radians on the head, the third neck ring, the jaws and the twenty-three teeth, half of it on the second

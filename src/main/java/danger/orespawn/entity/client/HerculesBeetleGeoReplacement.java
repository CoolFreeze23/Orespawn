 ...
 * delivers {@code idle} and {@code walk}). Wingspeed 1.0f (orig ModelHerculesBeetle.java:14,54 /
 * ClientProxyOreSpawn.java:489): the GAIT-scaled idiom on the eighteen leg parts - {@code cos(age * ws * 0.45f) *
 * PI * 0.12f * limbSwingAmount} about Y, the front and rear pairs around +-0.349 rad, the middle pair around 0 (orig
 * :286-292; no threshold) - and the ATTACKING-branch idiom on the nine jaw parts (orig :293 {@code getAttacking() ==
 * 0}: 0.051f x PI x 0.01f at rest, 0.51f x PI x 0.07f attacking, about X around 0.122 / 0 / 0.314 rad rests). The
 * entity is read through {@link HerculesBeetlePose} (the S4 doctrine).
 *

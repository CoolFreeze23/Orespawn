...
/**
 * GeckoLib Giant Robot (the hooks, owner 2026-09-14, addendum item 10): {@link ModelGiantRobot#poseFrom} verbatim on the
 ...
 * 0.25f (orig ClientProxyOreSpawn.java:516 {@code new ModelGiantRobot(0.25f)}, the model's {@code WING_SPEED}): the
 * MOVESCALE idiom ({@code limbSwingAmount * 0.65f} clamped to 1, orig ModelGiantRobot.java:158-161) scaling every walk
 * term; the hip's sway (X on {@code cos(-age ws)}, Y on {@code sin(-age ws)}, both x PI x 0.1, the yaw plus a constant
 * quarter turn) and its 4-unit BOB at twice the walk frequency (a POSITION write through {@link #moveTo}, orig

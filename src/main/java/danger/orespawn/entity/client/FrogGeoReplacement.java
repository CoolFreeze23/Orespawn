...
/**
 * GeckoLib Frog (the hooks, landed by the sixth Tier-2 slice T2f): {@link ModelFrog#poseFrom} verbatim on the
 * converted rig, ON THE HOOK (no keyframe layer, no transcription - the self-gate stays closed until an artist
 * delivers {@code idle} and {@code walk}). Wingspeed 1.0f (orig ModelFrog.java:14,27 / ClientProxyOreSpawn.java:512): the
 * THRESHOLD idiom on the front legs and lower hind legs about Y - above a walking speed of a tenth {@code cos(age * ws
 * * 1.4f) * PI * 0.55f * limbSwingAmount} , the right side the negative,
 * the hind pair at half, 0 at or below it (orig :133-141); the SINGING branch on the jaw - a 0.85 ws cosine x 0.15
 * around 1.22 rad while singing, 1.22 otherwise (orig :102-103, read through {@link FrogPose} ); the JUMP branch on the
 * upper hind legs' roll - +-2.44 rad while the vertical motion exceeds a tenth either way, +-0.227 otherwise (orig
 * :104-110, {@code getDeltaMovement().y} ); and the POSITION-write idiom (through {@link #moveTo} ) - each lower hind
 * leg's pivot FOLLOWS its upper leg 9 units along (cos, sin) of the roll (orig :111-114). The upper legs' pivots are
 * never written (the bind), read through {@link #classicPosition} ; the rolls the classic reads back are held in
 * locals; the followers keep their bind x.
 ...
 * public by the landing slice T2f; the hook survey passed an equal literal while it was private - the T2d form).</p>
 */

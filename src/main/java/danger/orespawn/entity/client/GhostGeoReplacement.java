...
/**
 * GeckoLib Ghost (the hook lanes, 2026-09-14, addendum item 10; landed by the remainder slice, 2026-09-15, under
 * ENT-S-160 (a) with (d)): {@link GhostModel#setupAnim} verbatim on the converted rig, ON THE HOOK (no keyframe layer,
 * no transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code walk}). The 1.7.10
 * rig's four slow cosines on the two arms (orig ModelGhost.render, ENT-S-091): the left arm about Z at 0.30 and about X
 * at 0.34, the right about Z at 0.32 and about X at 0.36, every one {@code PI * 0.05} around the +-0.33 rad splay; no
 * wingspeed, no gait, no entity state.
 *

 ...
 * {@code walk}). Wingspeed 2.0f (orig ModelBee.java:14,40 / ClientProxyOreSpawn.java:425): the wings
 * about Z at 1.1f, the pincers about Y at 0.3f, the left antenna's three parts and pom about X at 0.21f and about
 * Z at 0.31f, the right ones at 0.27f / 0.37f, and the ABDOMEN CURL - an attacking-branch idiom (orig :214 {@code
 * getAttacking() == 0}: 0.021f x PI x 0.023f at rest, 0.11f x PI x 0.055f attacking) folded down the five-part
 * chain with a -0.35f step and a POSITION follow (each part's y / z is the previous part's plus (cos, sin) of
 * its pitch times the segment length 10 / 10 / 6 / 5 / 7; orig :215-230), written through {@link #moveTo}. The
 * entity is read through {@link BeePose} (the S4 doctrine; the harness poses both sides from a declared state).
 *
 *
...
public final class BeeGeoReplacement extends OreSpawnGeoReplacement<EntityBee> {
    ...
    /** The SPEC's {@code locomotion: flyer} (tools/artist_specs/bee.json): {@code flying} is {@code !onGround()} (contract section 3; the weights slice: every landed species carries its seed's word, the asset audit pins it). */
    @Override

...
/**
 * GeckoLib T-Rex (the hooks, owner 2026-09-14, addendum item 10; landed by the first Tier-1 slice T1a, 2026-09-15, the
 * owner's closing set item 4): {@link ModelTRex#poseFrom} verbatim on the converted rig, ON THE HOOK (no keyframe
 * layer, no transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code walk} ; the
 * geo, the wiring and the proofs landed with T1a). The port's classic model as it is, {@code ANIM_SPEED} 1.0f (orig
 * ClientProxyOreSpawn.java:417 passes {@code new ModelTRex(0.2f)} ; the port's model is what the hook transcribes): the
 * THRESHOLD idiom on the eight leg parts' pitch - above a walking speed of a tenth ({@code limbSwingAmount > 0.1f}, the
 * float compare) {@code cos(age x 1.3) x PI x 0.25 x amount} , 0 at or below it - around the rest pitches -0.174 /
 * 0.506 / -0.401 / 0, the left leg mirrored; the ATTACKING branch on the jaw ({@code getAttacking() != 0}: 0.52 + a
 * 0.45 cosine x PI x 0.18, else 0.1); and the arms' 0.1 cosine x PI x 0.05 around -0.523 (the classic's {@code leftArm}
 * / {@code rightArm} fields are the rig's {@code shape17} / {@code shape11} parts). The entity is read through
 * {@link TRexPose} (the Slice 4b doctrine).
 *
 * <p>Scale and shadow follow {@link TRexRenderer} : 1.2 render scale ({@link TRexRenderer#SCALE}, applied around
 * {@code super.render} - made public by the landing slice T1a, the hook lanes' equal literal dropped; the T2d form) and
 * a 1.0 x 1.2 shadow (ENT-S-092). No zero-thickness cube.</p>
 */

 ...
 * delivers {@code idle} and {@code walk}). The port's classic model as it is: the wings about Z at 1.3 rad/tick
 * around +-2.0 rad, the jaw's {@code |cos|} idiom at 0.3, and four leg parts about X at 1.25 around +-0.349 rad
 * (orig ModelTerribleTerror.java:171-177, :179, :181, :183 for the lines the port keeps - the register line of this
 * slice records where the port's pose departs from the 1.7.10 one). The horns, the wings and the tail tip are
 * zero-thickness cubes: the seam draws every cube with its true transformed normal (ENT-S-161) and its two
 * coplanar faces in the classic order (below).
 *

 ...
 * delivers {@code idle} and {@code walk}). Wingspeed 0.22f (orig ModelCaterKiller.java:14,48 /
 * ClientProxyOreSpawn.java:499) and the ATTACKING-branch idiom (orig :248, :251, :289 {@code getAttacking() != 0})
 * on the jaws (Z, 1.7f x PI x 0.07f attacking / 1.3f x PI x 0.025f at rest), the head bob ({@code headoff}: 8 units at
 * 1.7f attacking / 2 units at 0.3f at rest, a POSITION write carried down the head, false head, tusks and jaws and,
 * divided by the segment index plus one, the first three segments' parts) and those segments' legs (X, 2.91f
 * x PI x 0.15f attacking / 0.35f x PI x 0.04f at rest, each a sixteenth-turn behind the last); the second tusks yaw at
 * 2.11f / 2.3f; the first segments' side spikes roll at 0.91f; the six rear segments' z FOLLOWS the walking
 * speed ({@code 39 + (16 + cos(age * 1.7f * ws + i * PI / 4) * 1.5f * limbSwingAmount) * i}, the gait-scaled idiom
 * on a position) with their side spikes rolling at 0.4f; the last segment sits 16 units past the sixth, its back
 * spikes pitching at 0.81f / 0.87f and yawing at 1.11f / 1.3f. The entity is read through {@link

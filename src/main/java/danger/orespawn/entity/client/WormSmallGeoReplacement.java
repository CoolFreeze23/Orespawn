 ...
 * idle} and {@code walk}). orig ModelWormSmall.java:44-63: a two-link chain of 5-unit links - the tail pitches
 * (0.55f) and rolls (0.35f), the body's pivot FOLLOWS the tail ({@code z = tail.z - sin(pitch) * 5}, {@code x =
 * tail.x + sin(roll) * cos(pitch) * 5}, {@code y = tail.y - 5 + (5 - cos(roll) * cos(pitch) * 5)}) and pitches
 * (0.45f) / rolls (0.25f) itself, the head follows the body the same way and pitches on {@code 0.62f + cos(age *
 * 0.65f) * PI * 0.15f} / rolls on a 0.3f cosine. A POSITION-write idiom through {@link #moveTo}; the
 * tail's pivot is never written (the bind), read through {@link #classicPosition}.
 *

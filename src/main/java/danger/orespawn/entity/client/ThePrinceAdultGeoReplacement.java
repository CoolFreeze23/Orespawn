...
/**
 * GeckoLib The Prince (adult) (the hooks): {@link ModelThePrinceAdult#poseFrom} verbatim on the converted rig (the
 * King's part set under the Prince's own model), ON THE HOOK (no keyframe layer, no transcription - the self-gate
 * stays closed until an artist delivers {@code idle} and {@code walk}; the landing slice adds the geo, the wiring and
 * the proofs). Wingspeed 0.65f (the port's model field): the wings' roll by ATTACKING / ACTIVITY / SIT (0.75 ws x PI
 * x 0.21 attacking; 0.35 ws x 0.15 gliding, scaled by the walking speed while the activity is 0; 0 while
 * ordered to sit) on the ten parts of each wing - the third at five thirds and the fifth at seven thirds of it,
 ...
 * mirrored; the thirteen claw parts flexing on a 0.75 ws cosine x PI x 0.25 while attacking (else 0) around -0.925 /
 * 0.384 / 0.645; the legs' swing (0.6 ws x PI x 0.45 attacking; a 0.3 ws x PI x 0.25 x amount gait while walking
 * and not sitting; else 0) - a quarter of it on the thigh and upper leg around 0.785, half on the lower leg and foot
 * around -0.628, the lower leg FOLLOWING the upper by 50 units and the seven claws of each foot the lower leg by 66
 * (at -0.1 / +0.15 rad); the tail's yaw chain at 0.56 / 0.19 attacking over 0.26 / 0.08, both 0 while sitting,

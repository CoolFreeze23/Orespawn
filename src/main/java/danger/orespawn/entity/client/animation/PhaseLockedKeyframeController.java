 ...
 *
 * <p>Returned from the salvaged G3 branch (commit 0d238ba, {@code entity/animation/}) with the
 * first Tier-2 slice, under the Phase G decisions (scope). What it does, in order:</p>
 *
 * <ol>
 ...
 *       DECLARED length ({@code animation_length} seconds x 20, read at prime
 ...
 * gait bones' rotation points by a per-frame amplitude ({@code limbSwingAmount}: - the clip is
 * authored at full amplitude, the controller scales the delta) and collapses each to a
 * constant point, which {@code AnimationProcessor.tickAnimation} then writes as {@code value +
 * initialSnapshot} (offsets 323-341): the scaled quantity is the delta from the BIND pose;</li>
 *
 ...
 * clip-clock cycle change is left open (recorded OPEN; nothing here is changed).</p>
 *
 *
 * <p>One controller per frequency group, registered in the order the species' layers declare them
 * ({@link KeyframeLayer}); a bone belongs to exactly one locomotion group. The controller never
 * resets GeckoLib's loop: {@link #adjustTick} always returns a time inside the clip, so the ordinary
 * loop reset - which cannot rebase an absolute clock - never fires. Instrumentation ({@link
 * #declaredClipTicks()}, {@link #lastClipTick()}, {@link #lastCosineIndex()}, {@link
 * #clipTicksPerSourceTick()}) exists for the harness and the records; nothing in-game reads
 * it.</p>
 */
public final class PhaseLockedKeyframeController<T extends GeoAnimatable>
     ...
     * phase clock takes effect. Reads the clip's declared length and refuses a clip that does not
     * loop in both the JSON and the controller stage (the contract pins the loop type per role; the
     * artist may not change it).
     */
    ...
    protected double adjustTick(double exactFloatPhaseCarrier) {
        ...
        // clip of DECLARED length; the cosine is never evaluated here.

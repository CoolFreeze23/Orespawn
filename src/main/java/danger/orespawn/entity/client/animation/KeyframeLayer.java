 ...
 * ({@code phase_g_reports/animation_contract/contract_design.md} sections 1, 2.1 and 5.1): the
 * group's name, the clip that animates it, its natural angular frequency in radians per age
 * tick (the tempo the SPEC states; the artist never sets it), the bones it owns (a partition
 * across a species' groups; should two layers ever key one bone, only the LATER layer's
 * additive flag decides: an additive layer adds its scaled delta to whatever earlier layers wrote
 * the bone this frame, a non-additive layer replaces it - and the additive base is read only for
 * the layer's amplitude-scaled bones ({@code amplitudeScaledRotationBones}), so a gait clip that
 * keys a bone outside its group writes that bone non-additively, last-registered-wins) and
 * whether it is the gait group, whose delta the controller scales by {@code
 * limbSwingAmount}. The gait group's clip carries the bare name {@link #WALK}; every other
 * group's is {@link #walkClip} ({@code walk_<group>}). A species WITHOUT a gait group names its
 * primary locomotion group in its SPEC ({@code primary_group} in the clip manifest and the
 * seed; the FIRST group when absent) and THAT group's clip carries the bare {@link #WALK} -
 * a label, not a semantic: the contract's fly - walk fallback is what plays a flyer's walk in flight
 * (contract section 2.1 amended) - so a species with ONE frequency group has only the bare names
 * whether or not its group is gait-scaled (the first Tier-2 slice), and a multi-group one without a
 * gait group has exactly one bare clip (the second). The harness's twin ({@code KeyframeLeg}) and
 * the generator ({@code tools/keyframe_clip.py}) refuse a clip manifest that breaks the rule.
 *
 *
...
public record KeyframeLayer(String group, String clip, float angularFrequencyRadiansPerTick, float wingspeed,
    ...
    /** The contract's standing loop: no target, not moving. Its presence TOGETHER with {@link #WALK}'s makes a species an artist species (one without the other stays classic). */
    public static final String IDLE = "idle";
    /** The contract's ground-locomotion loop on the PRIMARY group: the gait group, else the SPEC's {@code primary_group}, else the first group. */

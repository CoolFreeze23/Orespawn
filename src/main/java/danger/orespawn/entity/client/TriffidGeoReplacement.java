...
/**
 * GeckoLib Triffid (the hooks, owner 2026-09-14, addendum item 10; landed by the sixth Tier-2 slice T2f, 2026-09-15,
 * the owner's closing set item 4): {@link TriffidModel#poseFrom} verbatim on the converted rig, ON THE HOOK (Amendment
 * 2 to Amendment 1: no keyframe layer, no transcription - the self-gate stays closed until an artist delivers
 * {@code idle} and {@code walk} ). Wingspeed 1.0f (orig ModelTriffid.java:15,196 / ClientProxyOreSpawn.java:459): the
 * OPEN / CLOSED branch (orig :1275 {@code getOpenClosed() == 0} : the fold 0.122522116 rad per link closed,
 * {@code cos(age x 0.25 x ws) x PI x 0.039} open) driving four leaf chains through the four helpers transcribed below
 * (the same names, each a link's rotation as its parent's plus the angle and its pivot FOLLOWING the parent {@code j}
 * units along (sin, cos) of the parent's rotation - the POSITION-write idiom, two coordinates per chain through
 * {@link #moveXY} / {@link #moveYZ} ): l1-l15 (Z, about -0.95, the root's pivot from the angle), l31-l43 (Z, about
 * 0.95, the negative angle), l16-l30 (X, about -0.75, the negative angle) and l44-l57 (X, about 0.75; l44's pivot is
 * never written - the bind, read through {@link #classicPosition} ); the ATTACKING branch on the fifteen-link tentacle
 * (orig :1342 {@code getAttacking() != 0} : {@code |cos(age x 0.25 x ws) x PI x 0.5|} lashing, else a right angle
 * held), the links about Z alternating {@code -+angle - 0.6} from t15 down to t3, each pivot following the link above 6
 * / 3 units along (cos, sin) of that link's roll (t15's pivot is never written - the bind), t1 and t2 riding t3, and
 * the thirteen links' yaw zeroed; every value the classic reads back from a part it just wrote is held in a local. The
 * entity is read through {@link TriffidPose} (the Slice 4b form). Eleven leaf tips (c1-c11, never written by the hook)
 * are zero-thickness cubes (0 x 5 x 2; ENT-S-161; the classic face order required below; the shipped geo's count, T2f).
 *

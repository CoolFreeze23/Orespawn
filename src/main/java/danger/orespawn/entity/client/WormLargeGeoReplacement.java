 ...
 * rig, ON THE HOOK (no keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code
 * idle} and {@code walk}). orig ModelWormLarge.java:185-274: the five neck parts pitch on {@code cos(age * 0.25f) * PI
 * * 0.08f - 0.698f} and yaw on {@code cos(age * 0.15f) * PI * 0.07f}; the five head parts' pivot FOLLOWS the neck 32
 * units along (cos, sin) of those angles (a POSITION-write idiom, through {@link #moveTo}) and they pitch / yaw on
 * 0.35f / 0.45f cosines; the eight teeth sit 19 units on from the head with fixed offsets, shifted by {@code sin} of
 * the head's angles, and open on a 0.57f cosine (X for the top / bottom pair, Y for the side pair, both for the four
 * corners); the tail tip pitches on a 0.63f cosine around 0.35 rad and yaws on the same cosine a quarter turn ahead
 * ({@code + 1.57075}). The neck's pivot is never written (the bind), read through {@link #classicPosition}; every
 * value the classic reads back from a part it just wrote is held in a local.
 *

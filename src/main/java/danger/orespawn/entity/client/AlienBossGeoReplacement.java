...
/**
 * GeckoLib Alien Boss (the hook survey, 2026-09-14): the second registry of the Alien rig. orig
 * ClientProxyOreSpawn.java: 435 drew both the Alien and the Alien Boss with the one {@code RenderAlien(new
 * ModelAlien(0.22f), 0.35f, 1.1f)} (the port's {@link AlienRenderer} serves both), so this descriptor shares {@link
 * AlienGeoReplacement}'s geo, clip file, texture, scale, shadow and hook ({@link AlienGeoReplacement#poseRig}) under its
 * own registry path ({@code alien_boss}; one profile per registry path even for a shared rig - the Ant precedent).

...
/**
 * GeckoLib Alien Boss (the hook survey, 2026-09-14; landed by the FK slice, 2026-09-15, on the Alien's
 * parent-child hierarchy, in-game behind the dev switch): the second registry of the Alien rig. orig
 * ClientProxyOreSpawn.java:435 drew both the Alien and the Alien Boss with the one {@code RenderAlien(new
 * ModelAlien(0.22f), 0.35f, 1.1f)} (the port's {@link AlienRenderer} serves both), so this descriptor names {@link
 * AlienGeoReplacement}'s geo, clip file and texture (the literals repeated, as the Ant's consumers do: the asset audit
 * and the package tool attribute the rig by them) and shares its scale, shadow and hook ({@link
 * AlienGeoReplacement#poseRig} on a {@link FlatRig} bound here) under its own registry path ({@code
 * alien_boss}; one profile per registry path even for a shared rig - the Ant precedent). {@link AlienBoss} extends {@code Alien} and so already implements {@code AlienPose}.
 */

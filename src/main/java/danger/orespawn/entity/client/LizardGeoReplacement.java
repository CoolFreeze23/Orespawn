 ...
 * above a walking speed of a tenth, the front pair positive, the rear pair the negative, the lower legs about X and the
 * rest about Y); the ATTACKING-branch lower jaw ({@code 0.52 + cos(age * 0.45) * 0.35} attacking, 0.25 at rest, copied to
 * its six teeth) and the ATTACKING-branch tail rhythm ({@code 0.25 ws x PI x 0.05} at rest, {@code 1.25 ws x PI x 0.35}
 * attacking) folded down five rings as 0.25 / 0.5 / 0.75 / 1.0 / 1.25 of the yaw with a POSITION follow through
 * {@link #moveTo} (12 / 9 / 7 / 7 units along cos / sin); and the HEAD-LOOK idiom: the neck yaws a quarter of
 * {@code toRadians(netHeadYaw)}, the upper jaw and its eighteen followers (noses, eyes at +-0.78, teeth, hat) a half,
 * their pivots FOLLOWING the neck by 2 units, the lower jaw and its six teeth a half following by 3 units. The entity is
 * read through {@link LizardPose}. The eight fins are zero-thickness cubes (the seam draws every cube with its true

 ...
 * speed of a tenth {@code cos(age * 2.3f) * PI * 0.25f * limbSwingAmount}, 0 at or below it (orig :204-211); the HEAD-LOOK
 * idiom about Y at 0.45 of {@code toRadians(netHeadYaw)} on the nine head parts (orig :160-167; no clamp, no pitch); the
 * SITTING branch (orig :168-173, read through {@link ChipmunkPose}): while not sitting the tail pitches on a 0.25 cosine
 * around 0.306 rad plus a gait-scaled 1.3 cosine, the second segment 0.306 rad further; and the two hat parts hidden every
 * frame (through {@link #setVisible}). While sitting the classic leaves the tail's pitch where the last frame left it (a
 * singleton-model latch); the hook leaves the two bones at bind.
 *

...
/**
 * GeckoLib Vampire Butterfly (the hook lanes, 2026-09-14): a consumer of the Butterfly rig. The port-only registration
 * ({@link VampireButterflyRenderer}: no 1.7.10 {@code RenderButterfly} line for it) draws the shared
 * {@link ButterflyModel} at the default wingspeed 1.0f (orig ModelButterfly.java:23) over {@code vampire_butterfly.png},
 * so this descriptor shares {@link ButterflyGeoReplacement}'s geo, clip file and hook under its own registry path
 * ({@code vampire_butterfly}; one profile per registry path even for a shared rig).
 *

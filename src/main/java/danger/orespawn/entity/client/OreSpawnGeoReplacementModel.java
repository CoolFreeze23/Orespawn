...
public final class OreSpawnGeoReplacementModel<E extends Entity, A extends OreSpawnGeoReplacement<E>>
    ...
    public void setCustomAnimations(A animatable, long instanceId, AnimationState<A> animationState) {
        // The remainder slice (2026-09-15; owner's closing set item 3): the frame's inputs as the classic hook receives them,
        // kept EVERY FRAME, before the gate below, for the vanilla-layer adapter that draws the classic biped layers after the
        // bones (the same six floats and the partial tick vanilla's LivingEntityRenderer.render hands its RenderLayer.render
        // calls, 21.1.223 bytecode 632-684): the adapter reads the bones in classic terms whether the hook or the artist layers
        // posed them, so a species whose artist gate opens needs the inputs too (refuter B: kept after the gate, the adapter threw
        // on that species' first frame). One frame at a time: defaultRender runs actuallyRender and applyRenderLayers back to back.

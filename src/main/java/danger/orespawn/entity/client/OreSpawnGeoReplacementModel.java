...
public final class OreSpawnGeoReplacementModel<E extends Entity, A extends OreSpawnGeoReplacement<E>>
     ...
     *
     * <p>The missing-key policy: this call runs inside {@code
     * GeoRenderer.defaultRender}, under {@code EntityRenderDispatcher.render}, so a throw
     * here is a client crash on the mob's first frame. A rig WITHOUT the key is what a
     * resource pack produces (Blockbench's exporter rewrites {@code description} and
    ...
    public BakedGeoModel getBakedModel(ResourceLocation location) {
        ...
        if (baked != this.ordered) {
            ...
            // ENT-S-146: the geo resource is parsed ONCE per bake and the document handed to both keys - not
            // one parse per key for every seam rig on every bake and reload.
            JsonObject geoJson;
            try {
    ...
    /**
     * The Q9 repair (scope, SCOPED to this seam): every clip a replacement's controller asks for
     * is served from a copy of the loaded animation file whose {@code catmullrom} keyframes carry
     * the textbook spline arguments ({@link SplineRepair}: GeckoLib 4.8.4 feeds
     * each segment its own start value as P0 - a kinked tangent at every key; the bytecode is cited
     * there). GeckoLib 4.8.4 {@code GeoModel.getAnimation} (bytecode) fetches {@code
     * GeckoLibCache.getBakedAnimations().get(getAnimationResource(animatable))} (offsets 0-18) and
     * returns {@code BakedAnimations.getAnimation(name)} when the file is there (20-44; {@code
     * null} for a clip name the file lacks), and only then walks the fallback resources and throws for
     * a missing file (45-152); {@code AnimationController} asks through it on every stage poll
     * ({@code AnimationProcessor.buildAnimationQueue}). The repair is made ONCE per loaded file, by
     * identity: {@code GeckoLibCache.reload} bakes every animation file again into a fresh map ({@code
     * loadAnimations} -> {@code FileLoader.loadAnimationsFile} -> new {@code BakedAnimations}
     ...
     * semantics until her own decision; a file that is absent takes GeckoLib's own resolution and
     * error, untouched. A file that is present but lacks the clip answers {@code null} here without
     * walking {@code getAnimationResourceFallbacks} - GeckoLib's own order today (the walk starts at
     * 45, only after a present file has answered; no replacement overrides the fallbacks), so a future
     * fallback declaration must make this override walk it too. What this model can retain is one
     * pair ({@link #repairedSource} / {@link #repaired}, replaced, never accumulated), and the model
     * instance itself is replaced on F3+T (D6).
     */
    @Override

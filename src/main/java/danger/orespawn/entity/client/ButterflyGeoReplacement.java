...
/**
 * GeckoLib Butterfly (the hook lanes, 2026-09-14, addendum item 10): {@link ButterflyModel#setupAnim} verbatim on the
 * converted rig, ON THE HOOK (no keyframe layer, no transcription - the self-gate stays closed until an artist delivers
 * {@code idle} and {@code walk}). The parameterised rig's one rhythm: the eight wing parts about Z at
 * {@code cos(age * 1.3f * wingspeed) * PI * 0.25f}, the four right wings positive and the four left wings the negative
 * (orig ModelButterfly.java:23,28 {@code wingspeed}; the port's ButterflyModel.setupAnim). The WINGSPEED is the

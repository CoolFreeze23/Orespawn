...
public class ModelPeacock extends EntityModel<Peacock> {
    ...
    /** The classic pose over what the model reads from its entity (the port's {@code entity.getBlink() > 0}; orig ModelPeacock.java setRotationAngles). */
    public void poseFrom(PeacockPose entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        float newangle = 0.0f;

 ...
 * idle} and {@code walk}). Four two-part tentacles (orig ModelIrukandji.java:90-133): each root part
 * pitches and rolls on its own cosines, and its tip's pivot FOLLOWS the root - {@code z = root.z + sin(pitch) * 7},
 * {@code x = root.x - sin(roll) * cos(pitch) * 7}, {@code y = root.y + cos(roll) * cos(pitch)
 * * 7} - a POSITION-write idiom carried through {@link #moveTo}; the tip then pitches and rolls on two more
 * cosines. The roots' pivots are never written (the bind), read through {@link #classicPosition}.
 *
 *
...
public final class IrukandjiGeoReplacement extends OreSpawnGeoReplacement<Irukandji> {
    ...
    /** The SPEC's {@code locomotion: swimmer} (tools/artist_specs/irukandji.json): {@code inWater} is {@code isInWater()} (contract section 3; the weights slice: every landed species carries its seed's word, the asset audit pins it). */
    @Override

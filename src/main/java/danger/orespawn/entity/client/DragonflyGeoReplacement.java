 ...
 * plays it in flight) - and {@code walk_jaws} on the other, so the shipped {@code dragonfly.animation.json}
 * ({@code idle} keying no bone + {@code walk} + {@code walk_jaws}) OPENS the gate on the GeckoLib candidate (the
 * second Tier-2 slice; the first slice shipped it CLOSED as {@code walk_wings}).</p>
 *

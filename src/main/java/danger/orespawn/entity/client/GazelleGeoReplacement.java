...
/**
 * GeckoLib Gazelle (the hooks, owner 2026-09-14, addendum item 10; landed by the sixth Tier-2 slice T2f, 2026-09-15, the
 * owner's closing set item 4): {@link ModelGazelle#poseFrom} verbatim on the converted rig, ON THE HOOK (Amendment 2 to
 * Amendment 1: no keyframe layer, no transcription - the self-gate stays closed until an artist delivers {@code idle} and
 * {@code walk} ). Wingspeed 0.65f (orig ModelGazelle.java:14,51 / ClientProxyOreSpawn.java:449): the THRESHOLD idiom on the
 * eighteen leg parts about X - above a walking speed of a tenth {@code cos(age * 1.1f * ws) * PI * 0.12f * limbSwingAmount}
 * around the 0.297 / -0.074 / -0.409 / 0 / 0.185 rad rests, the diagonal pairs opposed, 0 at or below it (orig :342-363);
 * the HEAD-LOOK idiom about Y at 0.45 of {@code toRadians(netHeadYaw)} on the head, nose, mouth and the six antler parts,
 * the ears at 1.57 rad plus it plus a 0.5 cosine x 0.02 (orig :364-375; no clamp, no pitch); and the CROUCH branch (orig
 * :297 {@code func_70906_o()} , the port's {@code isCrouching()} , read through {@link GazellePose} ): while not crouching
 * the tail pitches on a 0.1 cosine x 0.06 around 1.0 rad. Crouching, the classic leaves the tail's pitch where the last
 * frame left it (a singleton-model latch); the hook leaves the bone at bind.
 ...
 * the landing slice T2f; the hook lanes passed an equal literal while it was private - the T2d form).</p>
 */

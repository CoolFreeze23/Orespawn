...
/**
 * GeckoLib Stinky (the hooks): {@link StinkyModel#poseFrom} verbatim on the rig the landing slice converts, ON THE HOOK
 ...
 * itself (StinkyModel.poseFrom {@code float ws = limbSwingAmount}; orig ModelStinky.java ran on a wingspeed
 * of 0.65f, ClientProxyOreSpawn.java:490 - the register line of the landing slice records where the port's pose
 * departs from the 1.7.10 one) - the THRESHOLD idiom on the wings about Z ({@code cos(age x 2.3 x ws) x PI x 0.4 x
 * limbSwingAmount} above a walking speed of a tenth about -+0.4 rad) and on the four legs about X ({@code
 * cos(age x 2.0 x ws) x PI x 0.25 x limbSwingAmount}, alternating; the ACTIVITY branch folds them at -1 / +1 rad
 * at 2); the tail about Y on {@code cos(age x 1.0 x ws) x PI x 0.2}, stilled by the SITTING check, the second and
 * third links at 1.6 and 2.6 of it, each link's pivot FOLLOWING its parent 4 / 3 units along (sin, cos) of the
 * parent's yaw, less 0.5 in x (the POSITION-write idiom, x and z through {@link #moveXZ}; the first link's pivot is
 * never written - the bind); and the HEAD-LOOK idiom on seven head parts, the yaw in radians (the neck at half), the
 * pitch in radians at a third. The entity is read through {@link StinkyPose} (the Slice 4b form).
 * The two wings are zero-thickness cubes (ENT-S-161; the classic face order required below).
 *
 *

...
/**
 * GeckoLib Jeffery (the hooks, landed by the first Tier-1 slice T1a): a consumer of the Giant Robot rig. The port
 * registers {@code GiantRobotRenderer::new} for the {@code jeffery} registry ({@code
 * OreSpawnClient.java:66}; {@code Jeffery extends GiantRobot} - a port-side registry with no 1.7.10
 * registration of its own), so this descriptor shares {@link GiantRobotGeoReplacement} 's geo, clip file and hook
 * ({@link GiantRobotGeoReplacement#poseRig}) under its own registry path (the Ant precedent: one profile per registry
 * path even for a shared rig), with the Giant Robot's texture, shadow and scale: {@link
 * GiantRobotRenderer#SHADOW} (0.99 x 1.0, ENT-S-092), {@code SCALE} 1.0 (identity, no scale hook). The entity is
 * read through {@code GiantRobotPose} , which the Jeffery satisfies through its parent.
 *
 */

...
/**
 * GeckoLib Jeffery (the hooks): a consumer of the Giant Robot rig. The port registers {@code GiantRobotRenderer::new} for
 * the {@code jeffery} registry ({@code OreSpawnClient.java:66}; {@code Jeffery extends GiantRobot} - a port-side
 * registry with no 1.7.10 registration of its own), so this descriptor shares {@link
 * GiantRobotGeoReplacement}'s geo, clip file and hook ({@link GiantRobotGeoReplacement#poseRig}) under its own

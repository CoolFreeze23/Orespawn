 ...
 * the classic renderer to within float rounding (). No keyframe clip is
 * involved in it, so no approximation tolerance applies.</p>
 *
 ...
 * an in-game look accepts the regenerated clip, so the registration is inert
 * in-game and the classic hook below stays the shipped path. The harness's
 * keyframe reference leg proves the layers against this hook on a scratch clip
 * at the decided 2.5e-3 rad (the density stated beside it).</p>
 *
...
public final class BeaverGeoReplacement extends OreSpawnGeoReplacement<Beaver> {
     ...
     * {@code ANIM_SPEED} 1.0F), one clip each: {@code walk} scaled by {@code
     * limbSwingAmount}, {@code walk_teeth} and {@code walk_tail} free-running, as the teeth chew
     * and the tail wags at rest in 1.7.10.
     */

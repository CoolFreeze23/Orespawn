...
/**
 * GeckoLib Ostrich (the hooks, owner 2026-09-14, addendum item 10): {@link OstrichModel#poseFrom} verbatim on the rig
 ...
 * ModelOstrich.java:16,57 / ClientProxyOreSpawn.java:450), the entity read through the EXISTING {@link OstrichPose}
 * (ENT-S-093, the Slice 4b form; nothing added to it): the legs' gait on the entity's own per-tick SPEED
 * ({@code sqrt(dx^2 + dz^2)} of {@code xOld - getX}, {@code zOld - getZ}) - {@code cos(age x 1.25 x ws) x PI x speed x
 * 0.4}, CLAMPED to +-0.75 past +-0.5 - over the twenty-two leg parts about X (the left positive about -0.297 / 0.483 /
 * -0.437, the right the negative); the three tail feathers about X on a bare 0.05 cosine at 0.06 x PI about -0.594 and
 * about Y on 0.061 / 0.072 cosines at 0.08 x PI about -+0.334; the head yaw from the RenderInfo ACCUMULATION when
 * ridden (orig :335-337: the entity's own yaw delta x 20, negated, eased into {@code rf1} by a sixtieth and clamped to
 * +-50; the Rotator's per-frame RenderInfo precedent) else halved; the SITTING-and-not-activated branch (orig :349)
 * that inverts the six head parts to 3.1415 rad about X and zeroes the yaw; the HEAD-LOOK idiom
 * {@code toRadians(netHeadYaw) x 0.65} on five head parts; the wings' {@code |cos|} flap on 1.0 x ws at 0.15 x PI
 * (Z, mirrored; Y at half) gated by the {@code ri1} LATCH rolled from the entity's random on the frame the rhythm

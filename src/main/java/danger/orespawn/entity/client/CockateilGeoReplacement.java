 ...
 * hook ({@link #pose}) under its own descriptor - one profile per registry path (design Q9) - and both are
 * proven by the harness.</p>
 *
 ...
 * group - {@code primary_group} in the clip manifest and the seed, the FIRST group ({@code wings}) here
 * (contract section 2.1 amended; the bare name is a label, not a semantic: the contract's fly - walk fallback
 * plays it in flight) - and {@code walk_tail}, {@code walk_feather1..3} on the others, so the shipped {@code
 * cockateil.animation.json} ({@code idle} keying no bone + {@code walk} + four {@code walk_<group>}) OPENS the gate on the
 * GeckoLib candidate, for both consumers of the rig (the second Tier-2 slice; the first slice shipped it CLOSED as
 * {@code walk_wings}).</p>
 *
...
public final class CockateilGeoReplacement extends OreSpawnGeoReplacement<Cockateil> {
    ...
    /** The SPEC's {@code locomotion: flyer} (tools/artist_specs/cockateil.json): {@code flying} is {@code !onGround()} (contract section 3; the weights slice). */
    @Override

...
/**
 * GeckoLib Baby Dragon (the hooks, Tier 1; landed by the first Tier-1 slice T1a): a consumer of the Dragon rig. The
 * port's {@link BabyDragonRenderer} draws the distinct {@link BabyDragon} entity with {@link ModelDragon} at 0.45
 * scale over the same two type textures, so this descriptor shares {@link DragonGeoReplacement} 's geo, clip file and
 * hook ({@link DragonGeoReplacement#poseDragon}) under its own registry path ({@code
 * baby_dragon}; design Q9, one profile per registry path even for a shared rig - the Ant precedent), and the
 * harness proves it on its own manifest entry. The entity reads {@link danger.orespawn.entity.pose.DragonPose}
 * through its parent.
 *
...
public final class BabyDragonGeoReplacement extends OreSpawnGeoReplacement<BabyDragon> {
    ...
    /** The SPEC's {@code locomotion: flyer} (tools/artist_specs/baby_dragon.json): {@code flying} is {@code !onGround()} (contract section 3; the weights slice: every landed species carries its seed's word, the asset audit pins it). */
    @Override

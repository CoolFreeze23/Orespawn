...
/**
 * GeckoLib Baby Dragon (the hooks, Tier 1): a consumer of the Dragon rig. The port's {@link BabyDragonRenderer} draws
 * the distinct {@link BabyDragon} entity with {@link ModelDragon} at 0.45 scale over the same two type textures, so this
 * descriptor shares {@link DragonGeoReplacement}'s geo, clip file and hook ({@link
 * DragonGeoReplacement#poseDragon}) under its own registry path ({@code baby_dragon}; design Q9, one profile per registry path
 * even for a shared rig - the Ant precedent), and the harness proves it on its own manifest entry. The entity reads

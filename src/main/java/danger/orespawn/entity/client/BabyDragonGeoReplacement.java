...
/**
 * GeckoLib Baby Dragon (the hooks, owner 2026-09-14, addendum item 10; Tier 1): a consumer of the Dragon rig. The port's
 * {@link BabyDragonRenderer} draws the distinct {@link BabyDragon} entity with {@link ModelDragon} at 0.45 scale over the
 * same two type textures, so this descriptor shares {@link DragonGeoReplacement}'s geo, clip file and hook
 * ({@link DragonGeoReplacement#poseDragon}) under its own registry path ({@code baby_dragon}; design Q9, one profile per registry
 * path even for a shared rig - the Ant precedent), and the harness proves it on its own manifest entry. The entity reads
 * {@link danger.orespawn.entity.pose.DragonPose} through its parent.
 *
 * <p>Scale and shadow follow {@link BabyDragonRenderer}: 0.45 render scale and a 0.6 shadow (its constructor literal and
 * private SCALE - no 1.7.10 pair pins them - so the equal literals here); the texture by {@code getDragonType()} as
 * {@link BabyDragonRenderer#getTextureLocation} resolves it. The rig's zero-thickness cube (tail5) is the Dragon's, so the
 * seam expects the classic face order in the shared geo.</p>
 */
public final class BabyDragonGeoReplacement extends OreSpawnGeoReplacement<BabyDragon> {
    /** BabyDragonRenderer.SCALE (private) = 0.45f: the port's half-size variant. */
    ...
    private static final GeoReplacementDescriptor<BabyDragon> DESCRIPTOR = new GeoReplacementDescriptor<>(
        ...
        public void applyScale(BabyDragon entity, PoseStack poseStack, float partialTick) {
            // BabyDragonRenderer.render: poseStack.scale(SCALE, SCALE, SCALE), 0.45 unconditionally

...
/**
 * GeckoLib The Princess (the remainder slice, with the bipeds - held since the hooks' landing on the orbs' per-frame
 * state): {@link ModelThePrincess#poseFrom} verbatim on the converted rig, ON THE HOOK (no keyframe layer, no
 * transcription - the self-gate stays closed until an artist delivers {@code idle} and {@code walk}). Her pose is the
 * Prince's (ModelThePrince.poseFrom, {@link ThePrinceGeoReplacement}): wingspeed 0.65f (orig
 * ModelThePrincess.java:16,56 / ClientProxyOreSpawn.java:511) - the THRESHOLD-OR-ATTACKING idiom on the six wing

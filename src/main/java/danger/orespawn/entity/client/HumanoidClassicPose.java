...
/**
 * THE CLASSIC SIDE OF THE BIPEDS (the remainder slice, 2026-09-15; owner 2026-09-15, closing set, item 3 - TEST-010 (b)):
 * vanilla {@code net.minecraft.client.model.HumanoidModel.setupAnim} (NeoForge 21.1.223, HumanoidModel.java:137-286 by the
 * jar's LineNumberTable; the repository's sources jar is a stub, so the body was read from the bytecode with javap, every
 * float literal as the constant pool holds it) transcribed STATEMENT BY STATEMENT over a {@link HumanoidModel}'s own parts
 * and fields, the entity read through {@link HumanoidPose} - the {@code poseFrom} entry of {@code ModelBoyfriend} /

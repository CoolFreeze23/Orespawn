...
/**
 * GeckoLib Girlfriend (drafted by the fourth Tier-2 slice T2d, TEST-010 (b); landed by the remainder slice, under with
 * the Boyfriend): the classic pose verbatim on the converted rig, ON THE HOOK (no keyframe layer, no transcription - the
 * self-gate stays closed until an artist delivers {@code idle} and {@code walk}). {@code ModelGirlfriend} declares
 * no {@code setupAnim} either: the pose its classic {@code HumanoidMobRenderer} draws is vanilla {@code
 * HumanoidModel.setupAnim} over the same seven bones (the arms 3 wide where the Boyfriend's are 4: geometry, not pose),
 * so this hook delegates to {@link BoyfriendGeoReplacement#poseRig} (the Butterfly / Leon / Baby Dragon form: one
 * hook, its own registry, texture, shadow and scale), the two per-frame lerps at the seam's partial tick as the classic
 * renderer's ({@code PoseInputs.partialTick()}); she swings (Girlfriend.java:441 the held-weapon melee, :521 after every
 * shot or throw), so the swing is a reachable per-frame read the seam now carries.
 *
...
public final class GirlfriendGeoReplacement extends OreSpawnGeoReplacement<Girlfriend> {
    ...
    public static final class Renderer extends OreSpawnGeoReplacedEntityRenderer<Girlfriend, GirlfriendGeoReplacement> {
        public Renderer(EntityRendererProvider.Context context) {
            ...
            // THE CLASSIC LAYERS: GirlfriendRenderer's, in its order - the HumanoidMobRenderer constructor's CustomHeadLayer,
            // ElytraLayer and ItemInHandLayer (21.1.223 bytecode 7-67), then its HumanoidArmorLayer over the player armor layers
            // (:20-23) - through the seam's adapter over a classic ModelGirlfriend posed from the bake.

...
/**
 * GeckoLib Ender Reaper (the hooks, owner 2026-09-14, addendum item 10; landed by the sixth Tier-2 slice T2f,
 * 2026-09-15, the owner's closing set item 4): {@link ModelEnderReaper#poseFrom} verbatim on the converted rig, ON THE
 * HOOK (Amendment 2 to Amendment 1: no keyframe layer, no transcription - the self-gate stays closed until an artist
 * delivers {@code idle} and {@code walk} ). Wingspeed 0.23f (orig ModelEnderReaper.java:80,83 /
 * ClientProxyOreSpawn.java:474): the THRESHOLD idiom feeding a {@code |cos|} - above a walking speed of a tenth the
 * three scythe parts roll on {@code 1.0f - |cos(age * 1.3f * ws) * PI * 0.25f * limbSwingAmount|} , 1.0 at or below it
 * (orig :589-593); the SCREAMING branch (orig :492-507, read through {@link EnderReaperPose} ): screaming, the scythe
 * swings on a 1.9 ws cosine x 0.25 around 1.0 rad, the left arm rises to -0.436 / -0.488 and the six wing parts beat on
 * a 2.7 ws cosine x 0.3; else the left arm hangs at -2.436 / 1.0 and the wings sway on a 0.7 ws cosine x 0.06, both
 * about Y around +-0.785 rad; and the HEAD-LOOK idiom (orig :508-514): yaw = {@code toRadians(netHeadYaw) * 0.45f}
 * clamped to +-0.45 rad (no pitch).

 ...
 * OPPOSITE of the {@link DamselInDistressGenerator} Girlfriend decision:</b>
 ...
 * empty NBT overrides and added no {@code removeWhenFarAway} override), so
 * persistence must come from the spawn call &mdash; a naked {@code
 * spawnEntity} here would produce a robot that despawns (pattern doc trap
 * #9). The doubles in the original are casts of INTS, so the robot stands
 * on the block CORNER at (+10, +1, +10), not the +0.5 centre &mdash;
 * transcribed exactly, not "fixed" (spec S9, the damsel precedent). Yaw is
 * drawn unconditionally by this caller (stitching contract).</li>
 *
...
final class SpiderHangoutGenerator {
    ...
    static void generate(LegacyDungeonPiece piece, BlockPos origin, RandomSource random) {
        ...
        // Girlfriend decision). Mob mapping: "Robot Spider"
        // (orig OreSpawnMain.java:4481) → orespawn:spider_robot
        // (spec §4). Silent variant: the original spawn is bare
        // createEntity + setLocationAndAngles + spawnEntityInWorld with NO
        // playLivingSound (orig :7038-7042 — unlike the spawnCreature
        // pattern), so the live DSB path must not play the ambient one-shot (batch-4 verify fix).

 ...
 * origin.
 *
 * <p>Layout (2026-09-06 -- replaces the 10x10 grid whose far corners lay 260-269 blocks out, past
 * the 256-block tracking cap and the 192-block simulation distance): rows along the look axis from
 * {@link BenchScene#baseDistance()} out at the scene's pitch {@link BenchScene#spacing()}; each row
 * holds the columns at multiples of the pitch whose lateral offset stays within {@code d
 * * tan(35 degrees)} of the axis ({@value #WEDGE_HALF_ANGLE_DEGREES} degrees is half the default
 * 70-degree FOV -- the vertical one; the horizontal FOV is wider on a 16:9 screen, so the wedge
 * is conservative) and whose Euclidean x/z distance from the origin is below

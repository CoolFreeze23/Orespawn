 ...
 * pitch is below its 22-block box on purpose: a hundred Queens at a 24-block pitch cannot
 * fit inside the tracking and simulation ranges, the Queen is {@code noPhysics},
 * and overlapping Queens are still drawn in full -- every one of them costs the collector its
 * whole bone walk, which is the cost the scene measures. What each scene fixes:
 * <ul>
 ...
 * not the protocol's fixed state and is offered for in-game looks only.
 */

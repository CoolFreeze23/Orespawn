...
public class OreSpawnConfig {
    ...
    // [modern].enabled is the ONE master switch that modern-mode
    // features hang off, each behind its own sub-key that only takes
    // effect while the master is on. The earlier per-feature 2.0 keys
    // (spiderMovement, mountCamera, phase14ContentEnable) predate the master and stay
    // independent of it; the MOD-024 items are unimplemented proposals, not keys.
    /**
     ...
     * animations will hang off this same master. Hitbox sub-keys are
     * snapshotted at entity construction (BOSS-017 pattern), so a
     * flip reaches newly constructed/ loaded entities, not live ones.
     *
     *
     */
    ...
    static {
        ...
        // 2.0 spider overhaul (design): default MODERN; one config line back
        // to full 1.7.10 parity. Server-authoritative: the server's value
        // at entity construction wins and is synced.

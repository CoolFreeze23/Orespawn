...
public class OreSpawnConfig {
    ...
    // 2.0 modern mode. Classic 1.7.10 parity is the default experience;
    // [modern].enabled is the ONE master switch. The earlier per-feature 2.0
    // keys (spiderMovement, mountCamera, phase14ContentEnable) keep their
    // names and their [tweaks] section but are effective only while the
    ...
    static {
        ...
        // [modern] enabled is true (default false), so a default config
        // runs CLASSIC until the master is switched on. Server-
        ...
        // per-feature keys, which keep their names. New features register under [modern]."
        BUILDER.push("modern");

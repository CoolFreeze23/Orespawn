...
public class OreSpawnConfig {
     ...
     * {@link OreSpawnConfig#spiderMovement()}: CLASSIC whenever
     * {@code [modern] enabled} is off, this key's value while it is on.
     *
     */
    ...
    // 2.0 modern mode. [modern].enabled is the ONE master switch:
    ...
    // modern features register under [modern]. The earlier per-feature
    // 2.0 keys (spiderMovement, mountCamera, phase14ContentEnable) keep
    // their names and their [tweaks] section and are effective only while the
    // master is on -- every read goes through the effective-value helpers
    // at the bottom of this class (spiderMovement(), mountCamera(),
    // phase14ContentEnable(), mothraWideRootHitbox(),
    // fireRespectsMobGriefing(), godzillaSparesBossPeers(),
    // petsDefendOwner(), pointysaurusStareAggro(),
    // cryolophosaurusRevengeChase(), chainsawSweepVanillaSight(),
    // artistAnimations(type)), never through the keys directly. The MOD-024
     ...
     * (Introduced 2026-09-03 with default false; flipped to true on
     * 2026-09-04.) A default config therefore runs each modern feature at
     * its own key's default -- the modern robots with the riding camera.
     * Effective
     * values: {@link #spiderMovement()}, {@link #mountCamera()}, {@link
     * #phase14ContentEnable()}, {@link #mothraWideRootHitbox()}, {@link
     * #fireRespectsMobGriefing()}, {@link #godzillaSparesBossPeers()}, {@link
     * #petsDefendOwner()}, {@link #pointysaurusStareAggro()}, {@link
     * #cryolophosaurusRevengeChase()}, {@link
     * #chainsawSweepVanillaSight()}, {@link
     * #artistAnimations(EntityType)} -- a feature's key is never
     * consulted without this master. Phase G artist animations hang off this
     * same master (the keys landed 2026-09-06, MOD-038).
     * Construction-snapshotted features (the robot gait mode, the hitbox
     * sub-keys, the T9 goal keys {@link #petsDefendOwner()}, {@link
     * #pointysaurusStareAggro()} and {@link
     * #cryolophosaurusRevengeChase()}; the per-entity animation
     * manager's layers, {@link #artistAnimations(EntityType)};
     * BOSS-017 pattern) see a flip on newly constructed/loaded entities
     * only, not live ones.
     *
     */
    ...
    /**
     * MOD-031 (ACCEPTED 2026-09-04: "MOD-031: accepted as a modern option,
     * default on; classic stays 1.7.10."): OreSpawn fireballs respect the
     * mobGriefing gamerule for the fire they place -- the face fire beside a
     * struck block and the impact blast's fire flag -- as vanilla ghast and
    ...
    /**
     * MOD-032 (targeting ledger batch T9, A1; decision of 2026-09-04: "documented
     ...
     * comment at the site; commit c4cde458). 1.7.10 refused only Godzilla,
     ...
     * Teen / Adult): tamed companions carry the vanilla owner-defence target
     * goals -- {@code OwnerHurtByTargetGoal} / {@code
     * OwnerHurtTargetGoal} on Leon, the Gamma Metroid, Spyro, Stinky, the
     ...
     * Documented only by the Phase 4E commit message (b687062f, "defense AI").
    ...
    /**
     * MOD-034 (targeting ledger batch T9, A3): the Pointysaurus's
     * Enderman-style eye-contact aggression ({@code
     * PointysaurusStareGoal}: the nearest survival player within 32 blocks looking
     * straight at it becomes its target after five ticks; creative and
     * spectator players ignored). Documented three times (the goal's javadoc, the
     * registration comment, commit dd32cbcb -- "Phase 10"). 1.7.10 had no such
    ...
    /**
     * MOD-035 (targeting ledger batch T9, A4): the Cryolophosaurus chases
     * and bites its revenge target (the {@code
     * DinosaurMeleeAttackGoal} acting on the slot only {@code
     * HurtByTargetGoal} fills, with the timid cryolophosaurus dice).
     * Documented by commit 6a5d4ba3 (Phase 4C's goal-based mapping) and the
    ...
    /**
     * MOD-037 (ITEM-070 -- B2): the Chainsaw's left-click sweep tests its sight on
     * vanilla's collision ray ({@code Player.hasLineOfSight}, eye to eye, the COLLIDER
     * clip that ignores fluids and collision-less blocks) instead of 1.7.10's
     * air-only ten-sample walk from the player's feet + 1.4 to the target's
     * mid-body (orig UltimateSword.java:198-247 {@code MyCanSee}, {@code
     * Chainsaw.myCanSee}). Takes effect only while {@link #MODERN_ENABLED} is on
     * -- read through {@link #chainsawSweepVanillaSight()}, never directly, live
     * at every swing (the MOD-031 read shape; no BOSS-017 concern); false is the
     * exact 1.7.10 walk, the classic behaviour.
     */
    public static final ModConfigSpec.BooleanValue MODERN_CHAINSAW_SWEEP_VANILLA_SIGHT;
    /**
     * MOD-038 (Phase G, the standard animation contract; scope): artist keyframe
     * animations on the GeckoLib candidate renderers -- a species whose {@code
     * .animation.json} carries the contract's clips ({@code idle} / {@code walk}
     * and the group clips) registers the contract's phase-locked layers and its
     * code-driven classic pose hook stands down; a species without those clips
     * (every shipped clip file today: the Beaver's is empty until in-game looks)
     * keeps the classic pose whatever this says (self-gated by clip presence).
     * Takes effect only while {@link #MODERN_ENABLED} is on -- read
     * through {@link
     * #artistAnimations(EntityType)}, never directly, ONCE per per-entity
     ...
     * master through {@link #artistAnimations(EntityType)}, never directly.
     */
    ...
    static {
        ...
        // 2.0 spider overhaul (design): key default MODERN. Master-override
        // decision of 2026-09-04: both keys below are effective only while
        // [modern] enabled is true -- its default, so a default config
        // runs each key's own default (MODERN, camera on); the master forces
        // CLASSIC / camera off when set false. Server-
        ...
        // Phase E decision (2026-08-11): wiki-documented mobs with no
        // 1.7.10 source counterpart (Vampire Butterfly, Apple Cow,
        // Golden Apple Cow) stay registered but ship disabled — no
        // natural spawns and no creative spawn eggs unless this is flipped on.
        ...
        // Decision 2026-09-03: "MOD-029: accepted as the modern-mode
        // default; classic keeps 5x2" and "Artist animations are a 2.0 feature
        // behind the modern config; classic stays code-driven parity".
        // Decision of 2026-09-04: "modern.enabled: master override only.
        // Off forces all modern features off; on defers to existing
        // per-feature keys, which keep their names. New features register
        // under [modern]." Default decision of 2026-09-04: "modern.enabled
        // defaults to true in code — the master defers to per-feature
        // keys by default and only forces classic when set false." (introduced 2026-09-03 with default false).
        BUILDER.push("modern");
        ...
        // MOD-031, decision of 2026-09-04: "MOD-031: accepted as a modern
        // option, default on; classic stays 1.7.10." (the proposal text had
        // said default false; the decision overrides it).
        MODERN_FIRE_RESPECTS_MOB_GRIEFING = BUILDER.comment(
                "MOD-031: OreSpawn fireballs place no fire beside the block they hit and their blast scatters " +
                        ...
                        "did (orig BetterFireball.java:261-266). On by default; set false " +
        ...
        // Targeting ledger batch T9 (decision of 2026-09-04: "T9: documented
        // reason -> MOD record behind modern; undocumented -> removed
        // from classic."). Defaults ON, the MOD-029 / MOD-031
        ...
        // MOD-037 (ITEM-070, decision of 2026-09-05: "B2 -- the 1.7.10 air-walk
        ...
        MODERN_CHAINSAW_SWEEP_VANILLA_SIGHT = BUILDER.comment(
                "MOD-037: the Chainsaw's left-click sweep (every living thing within 5 blocks, 56 damage) tests its " +
                        ...
                        "; set false to keep the 1.7.10 walk in modern mode too. Read at every " +
                        "swing, so a change applies to the next left-click."
        ).define("chainsawSweepVanillaSight", true);
        // MOD-038 (Phase G animation contract). Read once per per-entity animation
        // manager as it is built (the client's construction snapshot), never on a
        // live manager.
        //
        //
        MODERN_ARTIST_ANIMATIONS = BUILDER.comment(
                "MOD-038: artist keyframe animations on the GeckoLib candidate renderers. A species whose " +
                        ...
                        "file is empty until an in-game look accepts a species' clips, so a default install " +
    ...
    /**
     * Effective robot gait mode: CLASSIC whenever {@link #MODERN_ENABLED} is
     * off, else {@link #SPIDER_MOVEMENT}. The single read behind the
     * SpiderRobot/AntRobot construction snapshot (their one ctor-tail read
     * consumes this exactly once -- BOSS-017 pattern, the S4 ctor-tear
     * rule); never re-read on a live entity.
     */

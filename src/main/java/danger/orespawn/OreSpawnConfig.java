...
public class OreSpawnConfig {
    ...
    // never through the keys directly. The MOD-024 items are unimplemented
    // proposals, not keys.
    //
    //
    //
     ...
     * #chainsawSweepVanillaSight()} -- a feature's key is never
     * consulted without this master. Phase G artist
     * animations will hang off this same master. Construction-snapshotted
     * features (the robot gait mode, the hitbox sub-keys, the T9 goal
     * keys {@link #petsDefendOwner()}, {@link #pointysaurusStareAggro()}
     * and {@link #cryolophosaurusRevengeChase()}; BOSS-017
     * pattern) see a flip on newly constructed/loaded entities only,
     * not live ones.
     *
     */

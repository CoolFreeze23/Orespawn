...
public class MHLibPartEntity<T extends Entity> extends PartEntity<T> {
	...
	public InteractionResult interact(Player player, InteractionHand hand) {
		// ──────────────────────────────────────────────────────────────
		// 2.0 S6a (vendored change; sitting finding F-1): parts forward
		// INTERACTIONS to the parent's full interact chain, the same way
		// S4 forwards damage. Without this, every pickable part is an
		// interaction-dead surface: vanilla routes the click packet to
		// the part (getEntityOrPart), Entity.interact returns PASS, and
		// the click dies — a modern robot had FEWER working
	 ...
	 * taken as full lengths regardless of direction. One asymmetry:
	 * applyInformation scales the pivot only for an

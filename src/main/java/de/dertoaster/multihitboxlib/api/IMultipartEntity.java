...
public interface IMultipartEntity<T extends Entity> {
	...
	public default void setMasterUUID(UUID id) {
		...
		if (this instanceof IMHLibFieldAccessor<?> access) {
			if (entity.level().isClientSide()) {
				...
				// OPT-003: any client-visible mastership change invalidates
				// the change-only streaming cache (see updateSynching).
				// The server's election loop re-rotates every tick until
				// the elected master's first packet arrives
			...
			// ──────────────────────────────────────────────────────────
			if (id == null) {
				// OPT-003: a server-side master reset only happens when the
				// bone stream is dead (the 10-tick timeout in
				// updateSynching, or the master stopped tracking) — the
	...
	public default void processBoneInformation(final Map<String, BoneInformation> boneInformation) {
		if (this instanceof IMHLibFieldAccessor<?> access) {
			// OPT-003: every accepted packet REPLACES the retained sync map
			// wholesale (clear-then-put). The payload is the master's
			// complete synched-bone state for its tick, so a bone absent
			// from it (e.g. the master stopped rendering the entity and
			// now streams empty payloads) must fall back to config
			// offsets — just like the legacy per-tick-cleared map did.
			// Retention between packets lives in mhlibAiStep.
			access._mhlibAccess_getSynchMap().clear();
			// Process the bones...
	...
	/**
	 * 2.0 S5 (vendored change, decision): whether the per-tick {@code
	 * SPacketUpdateMultipart} stream serves this entity's trackers.
	...
	public default <E extends Entity & IMultipartEntity<?>> void mhlibAiStep() {
		if (this instanceof IMHLibFieldAccessor access) {
			...
			if (profile.isPresent() && profile.get().syncToModel()) {
				...
				// could clear it because the master client re-sent an
				// (identical) packet every tick; the master now skips
				// provably unchanged payloads (see updateSynching), so a
				// skipped tick must re-apply the last received bone
	...
	/**
	 * OPT-003: keepalive interval for the master client's change-only bone
	 * streaming (see updateSynching). A provably unchanged payload is
	 * re-sent at least this often so the server's 10-tick master timeout
	 * below can never fire from protocol silence — 8 leaves a 2-tick margin
	 * under that timeout for tick alignment and latency jitter.
	 */
	 ...
	 * OPT-003: the client send is now change-only with a keepalive — see the comment in the client branch.
	 */
	public default <E extends Entity & IMultipartEntity<?>> void updateSynching(E entity) {
		if (!entity.level().isClientSide()) {
			...
			// ──────────────────────────────────────────────────────────
			// OPT-003: change-only bone streaming from the master client,
			// with a keepalive. Legacy built and sent a
	...
	/**
	 * OPT-003: true only when {@code current} would put the server into
	 * exactly the state {@code lastSent} already did — same bone set and
	 * every field of every {@link BoneInformation} identical. Field
	...
	public default void tickParts(final Collection<MHLibPartEntity<T>> parts) {
		...
		if (this instanceof IMHLibFieldAccessor access) {
			...
			// part.tick()); a no-op on the server (/ B). A species that moves its parts later in its own tick
			// re-caches through mhlibCacheCullBox() (the gait-fed robots' client mirror).
	 ...
	 * Client only (/ B): the server never calls getBoundingBoxForCulling, so it caches nothing and the
	 * modifier passes vanilla's box through there. The union of three boxes: the entity's own; the cube {@code
	 * position +- R}, R being the profile's rest-pose reach (the maximum over the parts of {@code
	 * |position| + |pivot| + sqrt(w^2/2 + h^2)}, MHLibPartEntity.mhlibRestReach, computed once at
	 ...
	 * whose legs are on screen while its body is not would have been culled. {@link
	 * #mhlibComputeCullBox()} is the pure computation, public for the suite (a server gametest cannot run the
	 * client path). Design after MoreHitboxes' EntityMixin.changeCullBox /

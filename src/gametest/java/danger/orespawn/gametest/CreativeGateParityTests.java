 ...
 *
 * <p>The decision under test is ENT-S-107's mapping: orig {@code capabilities.isCreativeMode} (and, for the Ender
 * pair, the legacy loop's game-type {@code isCreative}) is 1.21.1 {@code Abilities.instabuild} —
 * never {@code invulnerable} / {@code canBeSeenAsEnemy}, which also refuses spectators and hand-toggled
 * invulnerability. Per site the three player states of that decision: a creative player refused (orig's own test), a
 * SURVIVAL player with {@code Abilities.invulnerable} flipped on by hand still taken (the discriminating row —
 * vanilla's {@code forCombat} conditions and {@code canAttack} refuse it; every row of this kind fails with its port
 * line reverted), and a plain survival player taken (the control). Ten sites reach a private filter by reflection
 * (the eight ENT-S-108 filters, present at HEAD and pinned here — CreativeMappingParityTests' shape — and, since
 * ENT-S-135 replaced their T8 goals with orig's scans, the CaterKiller's and SeaViper's {@code
 * isSuitableTarget}); one asks the vanilla {@code NearestAttackableTargetGoal<Player>} rebuilt non-combat with orig's
 * creative selector (Pointysaurus) through {@code canUse()} (PlayNicelyGateParityTests' GoalProbe shape, the acquisition roll pinned — bound 3 since ENT-S-136's interval 6, bound 5
 * chained); the Ender pair's goal rows add the shadowing (a creative starer nearer than a survival starer leaves
 * nothing picked; alone, the survival starer is taken), the hold ({@code holdsLegacyTarget} through both target
 * goals' {@code canContinueToUse}), the Knight's stare gate (pumpkin refused, look-away refused, a stare through a
 * stone wall refused by the :92 player-side ray with the cone asserted satisfied, a clear stare taken — each refusal
 * with its within-row control), and the revenge pick (the anonymous {@code HurtByTargetGoal}'s {@code canUse}, its
 * {@code canAttack} overridden to the same mapping: a creative attacker refused with the slot empty, an
 * invulnerable survival attacker taken and stored, where vanilla's
 * {@code canAttack} refused it). The Brutalfly and Mothra AI steps are driven once by reflection under scripted
 * rolls (the ENT-S-109 strafe shape with a per-call script, since the mob-hunt gate and the fire roll share the
 * bound 3): a creative nearest falls through to the mob hunt (the zombie's mark), a survival nearest is strafed and
 * shadows the hunt (the mob-hunt roll never drawn), and for Mothra an unseen survival nearest is neither strafed nor stepped past (the stage-1 sight step).</p>
 *
...
public class CreativeGateParityTests {
    ...
    /** The three player states of the decision, each with its generated-test suffix and the 1.7.10 answer. */
     ...
     * revenge goal's pick ({@code revenge}: the anonymous HurtByTargetGoal's canAttack override).
     */
     ...
     * pair's 2 revenge-pick rows each (4) — 60 rows in the {@code creativeGateParity} batch.
     */
    @GameTestGenerator
    ...
    // ------------------------------------------------------------------
    // E2 — the Ender pair's revenge pick: the anonymous HurtByTargetGoal's canAttack
    // ------------------------------------------------------------------
     ...
     * → {@code Player.canBeSeenAsEnemy} = {@code !abilities.invulnerable}), which the port overrides to the
     * mapping. The attacker is primed through {@code setLastHurtByMob} with the hunter's {@code tickCount} raised
     * first (TargetReleaseParityTests' idiom: vanilla's {@code canUse} ignores a record whose timestamp equals the
     * goal's initial 0) and the goal is driven as the selector drives it — {@code canUse}, then {@code start} if
    ...
    private static String revengeWhy(EnderSite site, PlayerCase playerCase) {
        return switch (playerCase) {
            ...
            case SURVIVAL_INVULNERABLE -> "a SURVIVAL attacker with Abilities.invulnerable set is not creative (EntityMob.attackEntityFrom"
                    ...
                    + " forCombat) -> LivingEntity.canAttack -> Player.canBeSeenAsEnemy (= !abilities.invulnerable) refuses it";

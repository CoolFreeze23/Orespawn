 ...
 * line reverted), and a plain survival player taken (the control). Eight rows reach the ENT-S-108 private filters by
 * reflection (present at HEAD, pinned here — CreativeMappingParityTests' shape); three ask the vanilla {@code
 * NearestAttackableTargetGoal<Player>} rebuilt non-combat with orig's creative selector (CaterKiller, SeaViper,
 * Pointysaurus) through {@code canUse()} (PlayNicelyGateParityTests' GoalProbe shape, the 1-in-5 acquisition roll
 * pinned); the Ender pair's goal rows add the shadowing (a creative starer nearer than a survival starer leaves
...
public class CreativeGateParityTests {
     ...
     * the Brutalfly's 2 and Mothra's 3, then the Ender pair's 2 revenge-pick rows each (4) — 60 rows in the {@code
     * creativeGateParity} batch.
     */
    @GameTestGenerator

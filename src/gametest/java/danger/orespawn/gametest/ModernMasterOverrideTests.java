...
/**
 * Master-override decision (2026-09-04): "modern.enabled: master
 ...
 *
 * <p>Default decision of the same day: "modern.enabled defaults to true in
 * code -- the master defers to per-feature keys by default and only forces
 * classic when set false" (the master was introduced 2026-09-03 with default
 * false). {@code master_defaults_true_and_default_config_reads_modern} pins
 * the code default and the default experience: every value at its
 * default reads {@code spiderMovement()} MODERN and {@code
 * mountCamera()} true -- the modern robots with the riding camera.</p>
 *
    ...
    public void master_defaults_true_and_default_config_reads_modern(GameTestHelper helper) {
        ...
        try {
            helper.assertTrue(OreSpawnConfig.MODERN_ENABLED.getDefault(),
                    "modern.enabled must default to true in code");
            OreSpawnConfig.MODERN_ENABLED.set(true);

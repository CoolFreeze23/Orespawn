...
public class HitboxPartTests {
    ...
    /**
     * Spawns a MODERN spider. Master-override decision of
     * 2026-09-04: the spiderMovement key is inert while {@code [modern]
     * enabled} is off, so the master is raised with the key and both are
     * restored together once the construction snapshot is taken.
     */
    ...
    public void s4_part_counts_and_classic_zero(GameTestHelper helper) {
        ...
        OreSpawnConfig.SpiderMovement prior = OreSpawnConfig.SPIDER_MOVEMENT.get();
        try {
            // Classic with the master off; modern below with the master
            // on.
        ...
        helper.onEachTick(() -> {
            try {
                ...
                for (int leg = 0; leg < SpiderRigProfile.LEG_COUNT; ++leg) {
                    ...
                    // Restated tolerance (design decision, Option A):
                    // PLANTED legs and SWINGING legs alike assert
                    // against the SERVER-TRUE trajectory
                    // (currentFootPos) — never a rendered position. Expected midpoint mirrors the feed.
     ...
     * (/ B, 2026-09-06: nothing calls getBoundingBoxForCulling there), which this server-side row pins
     * directly; the union itself is read through the public, side-effect-free
     ...
     * cover it).
     *
     * <p>Queen (frozen with setNoGravity: spawnWithNoFreeWill removes goals and brain behaviours, not gravity,
     * and she is noPhysics): R from the_queen.json by hand is Lwing1's {@code |(5, 18.15, 6.25)| +
     ...
     * union's own contract, and the reason a box cached before the feed -- a later review's blocker --
     * held the previous tick's legs. Classic spider (no profile): radius 0, no box, {@code
     * getBoundingBoxForCulling()} is the vanilla body box -- vanilla culling untouched.
     */
    ...
    public void s4_cull_box_bounds_the_rest_pose_parts(GameTestHelper helper) {
        ...
        // wave 5: spawnWithNoFreeWill leaves gravity on and the noPhysics Queen would sink 0.047 a tick; the containment pins below
        // are relative and would hold either way -- frozen for hygiene. The spiders stay on the floor under gravity.
        ...
        helper.runAfterDelay(2, () -> {
            try {
                ...
                helper.assertTrue(anyLegBeyondCube, "harvest 3: the gait plants at least one fed leg beyond the 0.735-block R cube (the rig's"
                        + " 99/16-block segments) -- the union's reason, and the post-mirror re-cache's; actual leg0 "

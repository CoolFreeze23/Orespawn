...
/**
 * Phase G, the keyframe leg's return (scope): what the game-test server can pin of the controller, the Q9
 * repair and the Beaver's self-gated registration. Every GeckoLib class used here is free of
 * {@code @OnlyIn} ({@code javap -v} over the pinned 4.8.4 jar: {@code GeoModel}, {@code
 * AnimationController}, {@code AnimatableManager} and its {@code ControllerRegistrar}, {@code
 * AnimationProcessor}, {@code AnimationState}, {@code Animation}, {@code EasingType}, {@code
 * RawAnimation}, the keyframe records, {@code BakedAnimations}, {@code KeyFramesAdapter},
 * {@code GeckoLibCache} - whose class initialiser installs empty maps and touches no client class - and {@code
 ...
 * opens on idle AND walk together); the production {@code registerControllers} path registers nothing on
 * this server (GeckoLib's animation cache holds no bake here).</li>
 ...
 * amount, the reference clip repaired by the production {@link SplineRepair}, driven through GeckoLib's own
 * {@code AnimationProcessor.tickAnimation}) pose the six animated bones within the decided 2.5e-3 rad of
 * the shipped classic hook ({@code OreSpawnGeoReplacementModel.setCustomAnimations} on a manager
 ...
 * <li>{@code kf_007} (the Tier-2 slices, 2026-09-13 / 2026-09-14 / 2026-09-15): the presented state - every one of the one hundred and
 * fifteen shipped replacements, constructed registry-free on this server, declares no keyframe layer but the Beaver (its three)
 * and the fourteen Tier-2 descriptors with transcriptions (pinned group by group in {@link T2SeamTests}; the third
 * slice's fifteen, the fourth slice's thirteen, the fifth slice's fourteen, the sixth slice's twelve, the FK slice's three, the first Tier-1 slice's seventeen, the second Tier-1 slice's two, the remainder's 7 and the Butterfly rig's four hook rigs declare none); {@code registerControllers} is final on the base and no replacement declares its own, so the base's self-gated registration is the single path, and it
 * registers nothing here for every species.</li>
 ...
 * OreSpawnGeoReplacementModel} constructed registry-free: {@code getAnimation} serves a repaired copy
 * (never the loaded object), the same instance on every call while the bake is the same (identity), a
 * fresh repaired copy once the map holds a new {@code BakedAnimations} (a resource reload) or the map itself
 * is swapped ({@code GeckoLibCache.reload}), and {@code null} for a clip the present file lacks
 * (GeckoLib's own answer, no fallback walk).</li>
 * </ul>
...
public class KeyframeLegTests {
    ...
    /** the revised form point 4, ratified: the animation-leg tolerance. */
    ...
    public static void kf_002_reference_clip_registers_the_contract_layers_under_the_key(GameTestHelper helper) {
        ...
        try {
            ...
            helper.assertTrue(replacement.registerKeyframeLayers(registrar(), walkOnly) == 0,
                    "walk alone registers none: the gate opens on idle AND walk together");
    ...
    // ------------------------------------------------------------------ row 7: the presented state - only the Beaver declares layers
    ...
    private static List<OreSpawnGeoReplacement<?>> everyReplacement() {
        return List.of(new BeaverGeoReplacement(), new CoinGeoReplacement(), new ElevatorGeoReplacement(),
                ...
                // the third Tier-2 slice (2026-09-13): the fifteen rigs ON THE HOOK - no layer, the classic source
                new CloudSharkGeoReplacement(), new BeeGeoReplacement(), new FairyGeoReplacement(), new GammaMetroidGeoReplacement(),
                new IrukandjiGeoReplacement(), new SkateGeoReplacement(), new RubberDuckyGeoReplacement(), new TerribleTerrorGeoReplacement(),
                new WormLargeGeoReplacement(), new WormMediumGeoReplacement(), new WormSmallGeoReplacement(), new CannonFodderGeoReplacement(),
                new CaterKillerGeoReplacement(), new CricketGeoReplacement(), new HerculesBeetleGeoReplacement(),
                // the fourth Tier-2 slice (T2d, 2026-09-14): the thirteen rigs landed on the hooks already written - no layer, the classic source
                new CrabGeoReplacement(), new KyuubiGeoReplacement(), new LeafMonsterGeoReplacement(), new AlosaurusGeoReplacement(),
                new AttackSquidGeoReplacement(), new BandPGeoReplacement(), new BaryonyxGeoReplacement(), new CamarasaurusGeoReplacement(),
                new CassowaryGeoReplacement(), new CreepingHorrorGeoReplacement(), new CryolophosaurusGeoReplacement(),
                new EasterBunnyGeoReplacement(), new FlounderGeoReplacement(),
                // the fifth Tier-2 slice (T2e, 2026-09-15): the fourteen rigs landed on the hooks already written (the Lurking Terror held) - no layer, the classic source
                new PointysaurusGeoReplacement(), new WhaleGeoReplacement(), new MolenoidGeoReplacement(), new RatGeoReplacement(),
                new SpitBugGeoReplacement(), new StinkBugGeoReplacement(), new TrooperBugGeoReplacement(), new VelocityRaptorGeoReplacement(),
                new GhostSkellyGeoReplacement(), new HydroliscGeoReplacement(), new LizardGeoReplacement(),
                new MantisGeoReplacement(), new CaveFisherGeoReplacement(), new ChipmunkGeoReplacement(),
                ...
                // the second Tier-1 slice (T1b, 2026-09-15): the Prince Teen and the Water Dragon landed on the hooks already written - no layer, the classic source
                new ThePrinceTeenGeoReplacement(), new WaterDragonGeoReplacement(),
                // the remainder slice (2026-09-15): every remaining hook landed - no layer, the classic source
                new LurkingTerrorGeoReplacement(), new ScorpionGeoReplacement(), new GhostGeoReplacement(), new BoyfriendGeoReplacement(), new GirlfriendGeoReplacement(), new ThePrincessGeoReplacement(), new TheKingGeoReplacement(),
                // the Butterfly rig's slice (2026-09-19, the close, item 1): the four consumers of one rig landed on the hooks already written under the manifest-declared pair-contested cap (TEST-019) - no layer, the classic source
                new ButterflyGeoReplacement(), new LunaMothGeoReplacement(), new MothraGeoReplacement(), new VampireButterflyGeoReplacement());
    }
    ...
    public static void kf_007_every_shipped_replacement_declares_its_layers_and_the_seam_registers_nothing_here(GameTestHelper helper) throws ReflectiveOperationException {
        ...
        helper.assertTrue(replacements.size() == 115, "the one hundred and fifteen shipped replacements (G1, Slice 4, the six Tier-2 slices, the FK slice, the two Tier-1 slices, the remainder and the Butterfly rig)");
        ...
        helper.assertTrue(beavers == 1, "exactly one Beaver among the one hundred and fifteen");
        helper.assertTrue(tier2 == 14, "the fourteen Tier-2 descriptors with layers among the one hundred and fifteen (the third slice's fifteen, the fourth slice's thirteen, the fifth slice's fourteen, the sixth slice's twelve, the FK slice's three, the first Tier-1 slice's seventeen, the second Tier-1 slice's two, the remainder's 7 and the Butterfly rig's four hook rigs declare none)");
        helper.succeed();
    }

    // ------------------------------------------------------------------ row 8: the seam serves the repaired bake, once per loaded file

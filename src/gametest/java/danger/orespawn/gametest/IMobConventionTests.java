...
/**
 * ENT-S-124 — the IMob convention (decision of 2026-09-04: "Convention: {@code Mob} + {@code Enemy}, port-wide,
 ...
 * subclass. The decided form is a {@code Mob}-typed goal with an {@code instanceof Enemy} selector (vanilla's
 * marker interface for hostile mobs: Slime, MagmaCube, Ghast, EnderDragon, Shulker, Phantom, Hoglin implement it
 * alongside every Monster), and'ed ahead of the site's own predicate where it has one (the Leon's tame rule) and
 * under the ENT-S-115 live PlayNicely {@code canUse} gate, kept as it was. The Ghast is still refused —
 * by vanilla {@code Mob.canAttackType} ({@code type != GHAST}, applied in {@code TargetingConditions.test}),
 * as 1.7.10's {@code EntityAITarget} refused it through {@code canAttackClass}; nothing special-cases it.
 *
...
public class IMobConventionTests {
    ...
    private static void run(GameTestHelper helper, Row row) {
        ...
        try {
            ...
            hunter.setOnGround(true); // a frozen mob never lands; the companions' nearbyOnly reach cache (ENT-S-135, TargetGoal.canReach) paths through GroundPathNavigation.canUpdatePath, which needs the ground (precedent)
    ...
    private static NearestAttackableTargetGoal<?> imobGoal(GameTestHelper helper, Mob hunter, Hunter site) {
        ...
        helper.assertTrue(found != null, "precondition: " + site.port() + " must carry a NearestAttackableTargetGoal<Mob> on its"
                + " target selector — the decided shape of orig " + site.orig() + " (" + FINDING + " test setup)");
        return found;
    }

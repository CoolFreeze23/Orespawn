 ...
 * restores them. Decision (2026-09-03): "every change gets a both-modes
...
public class HitboxDimsParityTests {
    ...
    // (ents095_split caveat 3). A later change may re-point or disable these four once
    // the owner confirms the baseline; the 0.1 height delta is the whole finding.
    // ================================================================
    ...
    // ================================================================
    // Decision 2026-09-03 (ENT-S-095 follow-up): Mothra restored to 1.7.10 in classic mode (batch 2, no
    // PlayNicely size branch so both modes agree); the two MOD-021 cows aligned to the cow line;
    ...
    public void s095_mothra_dims_both_modes(GameTestHelper helper) {
        // Classic pin: since the 2026-09-04 decision the [modern] master defaults
        // to true and Mothra's wide 6x3 root box (MOD-029) is the default
        // experience, so the classic 5x2 is reached only with the master off.
        // Snapshot, force classic, restore (MothraModernDimsTests pins the modern box).
        ...
        try {
            ...
            pinBothModes(helper, ModEntities.MOTHRA.get(), 5.0f, 2.0f, "mothra",
                    "orig Mothra.java:65 setSize(5.0f, 2.0f), no PlayNicely size branch in orig :63-70; a 2026-09-03 decision restored the port's 6x3 as the modern-mode box (MOD-029), classic keeps 5x2");
        } finally {
     ...
     * 1.7.10 registered under the display name "Apple Cow" (orig OreSpawnMain.java:3587).
     * Decision 2026-09-03: aligned 0.9x1.4 -> 0.9x1.3.
     */
    @GameTest(template = "empty_large", batch = "hitboxDimsParity")
    public void s095_apple_cow_dims_both_modes(GameTestHelper helper) {
        pinBothModes(helper, ModEntities.APPLE_COW.get(), 0.9f, 1.3f, "apple_cow",
                "no 1.7.10 class (MOD-021), aligned to the cow line = vanilla 1.7.10 EntityCow ctor setSize(0.9f, 1.3f), the RedCow that 1.7.10 registered as \"Apple Cow\" (orig OreSpawnMain.java:3587); the 2026-09-03 decision");
    }
    ...
    public void s095_golden_apple_cow_dims_both_modes(GameTestHelper helper) {
        pinBothModes(helper, ModEntities.GOLDEN_APPLE_COW.get(), 0.9f, 1.3f, "golden_apple_cow",
                "no 1.7.10 class (MOD-021), aligned to the cow line = vanilla 1.7.10 EntityCow ctor setSize(0.9f, 1.3f), the GoldCow that 1.7.10 registered as \"Golden Apple Cow\" (orig OreSpawnMain.java:3595); the 2026-09-03 decision");
    }
     ...
     * target. Decision 2026-09-03: the port's 0.6x0.6 stands and is pinned here so it cannot
    ...
    public void s095_cannon_fodder_port_only_dims_both_modes(GameTestHelper helper) {
        pinBothModes(helper, ModEntities.ENTITY_CANNON_FODDER.get(), 0.6f, 0.6f, "cannon_fodder",
                "port-only 0.6x0.6 with no 1.7.10 parity target: orig EntityCannonFodder.java:32-33 extends EntityTameable with no setSize and orig OreSpawnMain.java never registered it; the 2026-09-03 decision");
    }
     ...
     * setSmall only flipped the flag; the 2026-09-03 decision restores the shrink through

...
/**
 * Targeting ledger batch T9 (ENT-S-125): the port-only targeting additions, split on 2026-09-04
 * ("documented reason → MOD record behind modern; undocumented → removed from classic") into four
 * {@code [modern]} records — MOD-032 {@code godzillaSparesBossPeers} (a filter, read live), MOD-033
 * {@code petsDefendOwner}, MOD-034 {@code pointysaurusStareAggro} and MOD-035 {@code
 * cryolophosaurusRevengeChase} (goal registrations, construction snapshots) — one removal from both
 * modes (the Mantis's two inert target goals) and one deliberate parity exception kept in both modes
...
public class PortOnlyTargetingTests {
    ...
    private enum State {
        ...
        void apply(GameTestHelper helper, String keyName, ModConfigSpec.BooleanValue key) {
            helper.assertTrue(key.getDefault(), "precondition: [modern] " + keyName + " defaults to true in code"
                    + " (the MOD-029 / MOD-031 precedent; T9)");
            switch (this) {
                case MODERN_ON -> {
    ...
    private static void godzillaRow(GameTestHelper helper, State state) {
        ...
        try {
            ...
            helper.assertTrue(!isSuitableTarget(godzilla, peer), "Godzilla.isSuitableTarget with " + State.MODERN_ON.label(key)
                    + ": a PitchBlack (MyUtils.isBigBoss) must be refused — the MOD-032 peer rule (commit c4cde458)");
            peer.discard();
                ...
                try {
                    ...
                    if (state.modern()) {
                        helper.assertTrue(ownerHurtBy == 1 && ownerHurt == 1, where + ": the owner-defence pair"
                                ...
                                + ownerHurtBy + " / " + ownerHurt + " (MOD-033, commit b687062f)");
     ...
     * modern selector is that plus the port-only owner goals (Hydrolisc / VelocityRaptor: Phase 4E, commit b687062f;
     * Boyfriend / Girlfriend: commit 6d43caf, no stated intent), gated on the 2026-09-04 decision.
     */
    ...
    private static final List<Companion> COMPANIONS = List.of(
            ...
            new Companion("Girlfriend", ModEntities.GIRLFRIEND, "orig Girlfriend.java:161-174 (the two MyValentineTarget tasks, the Creeper"
                    ...
                    List.of("1:OwnerHurtByTargetGoal", "2:OwnerHurtTargetGoal"))); // the modern pair @3 / @4 -> @1 / @2 with wave 4: the Leon's and the Boyfriend's slots, strictly ahead of the hunt it tied with at @3 (MOD-033's promise; the wave-4)
            ...
            for (Companion companion : COMPANIONS) {
                ...
                try {
                    ...
                    helper.assertTrue(actual.equals(expected), companion.name() + " spawned with " + state.label(key)
                            ...
                            + (state.modern() ? "the 1.7.10 goals plus the port-only owner goals (MOD-033; Phase 4E b687062f or 6d43caf)"
    ...
    private static void pointysaurusRow(GameTestHelper helper, State state) {
        ...
        try {
            ...
            helper.assertTrue(stare == (state.modern() ? 1 : 0), where + ": the PointysaurusStareGoal must be "
                    + (state.modern() ? "on the target selector (the MOD-034 eye-contact rule, commit dd32cbcb)"
    ...
    private static void cryolophosaurusRow(GameTestHelper helper, State state) {
        ...
        try {
            ...
            helper.assertTrue(chase == (state.modern() ? 1 : 0), where + ": the DinosaurMeleeAttackGoal must be "
                    + (state.modern() ? "on the goal selector (the MOD-035 revenge chase, commit 6a5d4ba3)"
    ...
    private static void girlfriendRow(GameTestHelper helper, boolean master) {
        ...
        try {
            ...
            helper.assertTrue(!conditions.test(girlfriend, creative), "ValentineTargetGoal<Player> with " + mode
                    + ": a creative player must be refused on Feb 14 (Player.canBeSeenAsEnemy — kept in both modes on "
                    + "a safety decision; 1.7.10's MyEntityAITarget.java:96-98 hunted creative players) (MOD-036)");
            ...
            helper.assertTrue(!conditions.test(girlfriend, survival), "ValentineTargetGoal<Player> with " + mode
                    ...
                    + " on a safety decision; 1.7.10 hunted players in Peaceful on Feb 14) (MOD-036)");
        } finally {

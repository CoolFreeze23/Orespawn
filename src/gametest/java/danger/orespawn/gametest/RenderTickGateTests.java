...
/**
 * BUG-044: the once-per-game-tick gate of MHLib's client bone collection. The
 * vendored collector kept ONE {@code currentTick} per render layer (one
 * layer per renderer instance), collected while {@code currentTick ==
 * entity.tickCount || currentTick < 0} and advanced it to
 * {@code tickCount + 1} only on equality. A client frame during which the
 * entity ticked twice left the stamp one behind for good (equality never held
 * again and {@code tickCount} only grows), and two Queens drawn by the same
 * renderer shared the stamp, so it followed whichever entity last matched and the
 * other never collected. The fix (design after MoreHitboxes' {@code
 * GeckoLibMobMixin}, MIT): the stamp is a field on the ENTITY ({@code
 * IMHLibFieldAccessor#_mhlibAccess_getRenderTickStamp},
 * -1 until the first collecting pass) and the rule is {@link
 * RenderTickGate#shouldCollect} = {@code stamp < tickCount}, advanced to the
 * collected tick.
 *

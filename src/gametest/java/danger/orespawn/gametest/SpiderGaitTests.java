 ...
 * decision (2026-09-04) the key is inert while {@code [modern] enabled}
 * is off, so every MODERN spawn raises the master with the key and
 * restores both together.</p>
 */
        ...
        helper.onEachTick(() -> {
            try {
                ...
                for (int leg = 0; leg < SpiderRigProfile.LEG_COUNT; ++leg) {
                    ...
                    // Invariant 3a (amended S6b, reference behavior):
                    // comfort-invalidated and stranded lifts BYPASS the
                    // inhibitors (canMoveLeg's unconditional first line
                    // in the reference), so co-swing is legal iff at
                    // least one of the two swings was FORCED.

 ...
 * sequencing decision of 2026-09-05, scope (a); raised at Slice 2). GeckoLib 4.8.4's {@code
 * SingletonAnimatableInstanceCache.getManagerForId(long)} creates one {@code AnimatableManager} per id and nothing in
 * GeckoLib ever removes one; a replaced renderer keys them by entity id ({@code
 * GeoReplacedEntityRenderer.getInstanceId} = {@code currentEntity.getId()}), so every entity ever drawn kept its manager
 * for the client session. The fix: {@link OreSpawnAnimatableInstanceCache} (the same class GeckoLib picks, with {@code
 * evict} / {@code evictAll} / {@code size}), registered per entity type in {@link GeoReplacementCaches} by the

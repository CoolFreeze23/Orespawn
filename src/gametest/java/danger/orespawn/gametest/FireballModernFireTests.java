...
/**
 * MOD-031, decision of 2026-09-04: "MOD-031: accepted as a modern option, default on; classic
 * stays 1.7.10."
 *
 ...
 * {@link OreSpawnConfig#fireRespectsMobGriefing()} = master AND key (master-override: new
 * features register under [modern]; the master off forces classic). While effective, {@code
 * BetterFireball.onHitBlock} places orig BetterFireball.java:261-263's face fire only if {@code
 * EventHooks.canEntityGrief(level, owner)} (vanilla SmallFireball.onHitBlock's gate: the
...
public class FireballModernFireTests {
    ...
    /**
     * The owner-decided pin, three scenarios in sequence (see the class comment): (a) key on + master on
     * + mobGriefing off -> no face fire, fire flag false, KEEP; (b) key on + master on + mobGriefing on
     * -> face fire, fire flag true, DESTROY (the classic result); (c) master off + mobGriefing off ->
     * face fire, fire flag true, KEEP (classic stays 1.7.10). The key's code default is pinned first:
     * true, per the decision "default on". Master, key and rule are restored, and the shared shooter
     * discarded, once, on every path.
     */
    ...
    public void mod031_fire_respects_mob_griefing_three_scenarios_in_sequence(GameTestHelper helper) {
        ...
        guarded(restore, () -> {
            helper.assertTrue(OreSpawnConfig.MODERN_FIRE_RESPECTS_MOB_GRIEFING.getDefault(),
                    "modern.fireRespectsMobGriefing must default to true in code ("
                            + "MOD-031)");

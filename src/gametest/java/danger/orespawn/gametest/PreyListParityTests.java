...
/**
 * ENT-S-128 (targeting ledger batch T6, wave 2 — owner: "Targeting ledger, decided by wave … Generated pins
 * wherever the pattern allows … Next: T5 and T6"): the exclusion / prey lists. 1.7.10 decided a hunter's prey
 * by name in fixed filter chains; the port had drifted at fifteen of them — the shared {@code
 * MyUtils.isAttackableNonMob} membership (orig MyUtils.java:77-115, reached by the Crab :417, Mantis :391,
 * Molenoid :274, TheKing :981, TheQueen :929 and WaterDragon :679 fallthroughs), the Dragonfly's whitelist (orig
 * Dragonfly.java:213-228), the Lizard's AttackSquid grant (:316), the Purple Power's tamed-pet and royalty
 * steps (:261-264), the Rat's five species steps (:201-224), the Terrible Terror's nine dropped kinds
 * (:229-285), the Triffid's named seven for a blanket {@code !Monster} (:291-311), the
 * Boyfriend / Girlfriend monster task's own rules (MyEntityAITarget.java:88-128), and the three ENT-S-108
 * residuals (DungeonBeast :216-239, EmperorScorpion :476-493, HerculesBeetle :401-406 — present at HEAD since ENT-S-108, pinned here).
 *
...
public class PreyListParityTests {
    ...
    private static final class GoalDriver implements Driver {
        ...
        GoalDriver(GameTestHelper helper, Hunter kind) {
            ...
            this.hunter.setOnGround(true); // a frozen mob never lands; the companions' nearbyOnly reach cache (ENT-S-135, TargetGoal.canReach) paths through GroundPathNavigation.canUpdatePath, which needs the ground (precedent)

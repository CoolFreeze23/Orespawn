...
public class PlayNicelyGateParityTests {
    ...
    private static final class GoalProbe extends HunterProbe {
        ...
        public void setUp(GameTestHelper helper) {
            ...
            this.hunter.setOnGround(true); // a frozen mob never lands (LivingEntity.travel is gated on isEffectiveAi); the companions' nearbyOnly reach cache (ENT-S-135, TargetGoal.canReach) paths through GroundPathNavigation.canUpdatePath, which needs the ground (precedent)
     ...
     * runs it on to the gated scan and clears the slot, as at HEAD (an unconditional blanking made the pick stick,
     * {@code null != null} never clearing it). Nothing is reset between the two drives — the flag-up pass must
     * start from the slot the control filled.
     */
        ...
        public boolean drive(GameTestHelper helper, boolean playNicely) {
            ...
            if (playNicely) {
                helper.assertTrue(after == null && scanPick == null, "with playNicely on the scan's own pick must be cleared by"
                        ...
                        + " stood down) — saw " + this.trace + " (" + FINDING + ", review)");
            }
     ...
     * consulted nothing and set attacking 0 (:219-221), so that read is gated with the pass. A pig 5 blocks off
     * is stored through {@code setTarget}, no revenge target: with the flag down the fallback engages it (attacking
     * 1); with the flag up the pass consults nothing, attacking stays 0 and the slot still holds the pig.
     */

 ...
 * Cater Killer's {@code MyCanSee} walk against vanilla's eye ray both ways; the T3b review' rows (41-43): orig's grant of a
 ...
 * set on the ground ({@code setOnGround(true)}: a frozen mob never lands, precedent); every flip is restored in a
 * finally, every spawn discarded, every player removed; PlayNicely and the difficulty are asserted as preconditions
 * and never flipped. No row pins a hit on a mock player, so no spawn shield is cleared (the Stinky's bite lands on a
 * Zombie).</p>
 */
...
public class ScanSetParityTests {
    ...
    /** One test per row: 43 TestFunctions in the {@code scanSetParity} batch (40 from the T3b batch, 3 added on review). */
    @GameTestGenerator
     ...
     * 4.5 retargeted at 3 and 4 too (D4 / B-Q4).
     */
     ...
     * the Creeper task's pick — vanilla's {@code canAttack} reach-tested it and refused it; the IMob task grants it the same
     * way. Row 36 keeps the reach-refused Zombie as the control.
     */
     ...
     * him); a Boyfriend on the same spot behind the same wall is refused by the Boyfriend task — his class reaches :108 —
     * and taken once the wall is razed. The clock is restored in the finally.
     */
     ...
     * the goals' nearbyOnly reach cache paths through GroundPathNavigation.canUpdatePath, which needs it; precedent), its
     * FOLLOW_RANGE raised to 40 so the path search behind that test reaches every probe of these rows (orig's own attribute
     * bounded the search, 16 by default; the rows isolate the box).
     */

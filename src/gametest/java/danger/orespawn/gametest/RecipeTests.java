...
public class RecipeTests {
     ...
     * piston; the port's oak_door_from_crystal_planks.json was faithful all
     * along and must keep working.</li>
     * </ul>
     */
    @GameTest(template = "empty")
    ...
    /**
     * Checklist i133-item-062 / finding ITEM-062 (D5 closure) — data-driven loop
     * over ALL 116 water-bucket conversions: for every row of the documented
     * master table (spawn_ores_spec.md §2, orig OSM:2665-3021) the grid
     * {@code water bucket + spawn block} must resolve, assemble exactly one of
     * the expected spawn egg, and hand back exactly one EMPTY BUCKET via the
     * crafting remainder (the modern equivalent of the 1.7.10
    ...
    public void item062_all_116_water_bucket_egg_conversions(GameTestHelper helper) {
        helper.assertTrue(WATER_CONVERSIONS.size() == 116,
                "master table must carry 116 rows, has " + WATER_CONVERSIONS.size());

 ...
 * <li>{@code t2_003}: the gate as decided (contract section 2.1): under the modern keys a ONE-group species (its
 * clip the bare {@code walk}) registers its one phase-locked layer on the declared controller with the
 * declared omega and wingspeed; a MULTI-group species without a gait group (no bare {@code walk}: {@code
 * walk_<group>} only) registers NOTHING - the classic hook stays its path until it is decided which group carries
 * the bare name (the presented question's pin).</li>
            ...
            for (Species species : species()) {
                ...
                if (species.bareWalk()) {
                    ...
                    helper.assertTrue(registered == 0 && registrar.controllers().isEmpty(),
                            species.name() + ": a multi-group species without a bare walk registers nothing (the gate as decided: "

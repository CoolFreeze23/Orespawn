 ...
 *
 * <li>{@code t2_013} (the second Tier-1 slice T1b, on the hooks already written, hitbox profiles excluded): the last
 * two Tier-1 HOOK registries landed on their hooks - The Prince Teen and the Water Dragon - each pinned exactly as {@code
 * t2_005} pins the third slice's (no layer, an empty file, nothing registered, the classic shadow, the face order on the
...
public class T2SeamTests {
    ...
    public static void t2_013_second_tier1_slice_hook_registries_declare_no_layer_register_nothing_and_pose_through_their_hooks(GameTestHelper helper) {
        ...
        try {
            ...
            helper.assertTrue(all.size() == 2, "the two hook registries landed by the second Tier-1 slice");
            for (HookSpecies species : all) {
                assertHookSpecies(helper, species);
            }
        } finally {

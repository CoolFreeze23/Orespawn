 ...
 * <li>{@code t2_007} (the fifth Tier-2 slice T2e, 2026-09-15; owner's closing set item 4 on the hooks already written,
 *     addendum items 10 and 14): the fourteen HOOK species landed on their hooks - the Pointysaurus, Whale, Molenoid, Rat,
 *     Spit Bug, Stink Bug, Trooper Bug, Velocity Raptor, Ghost Skelly, Hydrolisc, Lizard, Mantis, Cave Fisher and Chipmunk
 *     (the Lurking Terror held on the visual leg, its register line) - each pinned exactly as {@code t2_005} pins the third
 *     slice's (no layer, an empty file, nothing registered, the classic shadow, the face order on the Trooper Bug, Velocity
 ...
 * <li>{@code t2_009} (the constant render transform, owner 2026-09-15, closing set continued, item 2; TEST-013): the
 *     descriptor's {@code renderTransform()} - the identity by default; the Dungeon Beast's classic YP 90 and the
 *     Kraken's XP 90 declared in the classic renderer's own terms - and its SLOT form, what
 *     {@code OreSpawnGeoReplacedEntityRenderer.applyRotations} multiplies onto the pose stack: the rotation conjugated
 ...
 *     subject).</li>
 * <li>{@code t2_014} (the remainder slice, 2026-09-15; owner's closing set items 3 and 4): the seven remaining HOOK registries
 *     landed on their hooks - the Lurking Terror, the Scorpion, the Ghost, the Boyfriend, the Girlfriend, the Princess and the
 *     King (the Butterfly rig's four REPORTED, not landed, TEST-019, are not pinned here) - each pinned exactly as {@code t2_005}
 *     pins the third slice's, the entity-reading hooks on one declared rest subject, the Ghost's on none; PLUS the seam change's
 *     pin: {@code PoseInputs} carries the frame's partial tick and the Boyfriend hook reads it verbatim through {@code getAttackAnim(partialTick)}
 *     (a swinging subject: the attack arm at partial tick 0.5 differs from partial tick 0, and at 0 equals oAttackAnim's pose).</li>
...
public class T2SeamTests {
    ...
    public static void t2_007_fifth_slice_hook_species_declare_no_layer_register_nothing_and_pose_through_their_hooks(GameTestHelper helper) {
        ...
        try {
            ...
            helper.assertTrue(all.size() == 14, "the fourteen hook descriptors landed by the fifth Tier-2 slice (owner 2026-09-15, closing set item 4; the Lurking Terror held on the visual leg)");
            for (HookSpecies species : all) {
                assertHookSpecies(helper, species);
            }
        } finally {
    ...
    /**
     * The constant render transform's conjugation, measured on matrices (owner 2026-09-15, closing set continued,
     * item 2; TEST-013; the seam's {@code OreSpawnGeoReplacedEntityRenderer.applyRotations} path:
     * {@code descriptor.renderTransform().applySlot(poseStack)} after the entity yaw and the descriptor's own rotations,
     * then the height compensation). RE-PINNED UNDER TEST-015 (owner 2026-09-15, closing set continued, second, item 35
     * (2)): the classic path rotates INSIDE {@code renderToBuffer}, after {@code LivingEntityRenderer.render}'s
     * {@code scale(-1, -1, 1)} flip (21.1.223 offsets 395-400) and {@code translate(0, -1.501, 0)} lift (413-417) - M;
     * the slot runs in entity space before the seam's chain, which after the slot is {@code translate(0, comp, 0)
     * translate(0, 0.01, 0) B} with B the bake map {@code (x, y, z) -> (-x, 1.5 - y, z)} (the converter's Bedrock
     * convention and the baker's x negation, {@code BakedModelFactory$Builtin.constructCube} 98-139) - and that product
     * IS M, so the seam frame F is M itself and the slot carries {@code M C M^-1}: {@code S R_y(90) S = R_y(-90)} for the
     * Dungeon Beast's YP 90 (S = diag(-1, -1, 1) is a half turn about Z, which reverses the sense of a rotation about Y;
     * before TEST-015, with F the y flip alone, the slot was YP +90 and the seam drew the rig facing the other way in the
     * entity frame the classic mirrors), and translate(0, 1.501, 1.501) R_x(-90) for the Kraken's XP 90 (unchanged: the
     * mirror in x commutes with a rotation about X, the mirror in y reverses it; the rotation about the classic origin,
     * 1.501 up). Asserted two ways: the slot matrix against the closed form, and the two chains on sample points - M, C
     * against slot, M (the frame is the bytecode's; t2_010 measures that the seam's chain after the slot is M).
     */
    @GameTest(template = "empty", batch = BATCH)
    ...
    /**
     * THE MIRROR FIXED AT THE SOURCE AND THE SEAM'S HEIGHT COMPENSATION, measured on matrices (TEST-015; owner
     * 2026-09-15, closing set continued, second, item 35 (1)-(2)). From the bytecode: the classic chain carries a
     * ModelPart-space point p to {@code M p}, {@code M = scale(-1, -1, 1) translate(0, -1.501, 0)}
     * ({@code LivingEntityRenderer.render} 395-400 / 413-417, 21.1.223); the seam's chain after the descriptor slot is the
     * height compensation ({@code OreSpawnGeoReplacedEntityRenderer.applyRotations}), GeckoLib's
     * {@code translate(0, 0.01, 0)} ({@code GeoReplacedEntityRenderer.actuallyRender} 722-727, 4.8.4) and the bake, which
     * places a converted cube's corner at {@code B p = (-x, 1.5 - y, z)} (the converter's Bedrock convention: the cube
     * keeps its ModelPart x and its y is measured up from the 24-unit datum; the baker negates x,
     * {@code BakedModelFactory$Builtin.constructCube} 98-139). Pinned: (1) the frame constant the record holds is M
     * (a rig point mirrors in x and y and rises 1.501); (2) with the compensation the seam's chain equals M on sample
     * points to float precision - the two renderers draw every rig in the same place; (3) WITHOUT it the two chains
     * differ by exactly the 0.009 blocks of height every seam rig carried before this landing (and by nothing else),
     * the measured residual the compensation closes; (4) the bake map itself mirrors x: a point at ModelPart x = +2
    ...
    public static void t2_014_remainder_slice_hook_registries_declare_no_layer_register_nothing_and_the_seam_carries_the_partial_tick(GameTestHelper helper) {
        ...
        try {
            ...
            helper.assertTrue(all.size() == 7, "the 7 hook registries landed by the remainder slice (owner 2026-09-15, closing set item 4)");
            for (HookSpecies species : all) {
                assertHookSpecies(helper, species);
            }
            // THE SEAM CHANGE (owner 2026-09-15, closing set item 3): PoseInputs carries the frame's partial tick and the biped hook
            // reads getAttackAnim(partialTick) verbatim - the attack arm's pitch on a swinging subject differs between the partial
            // ticks 0 and 0.5 (vanilla's swing advancing a sixth per tick), and at 0 equals the pose of a subject frozen at oAttackAnim.

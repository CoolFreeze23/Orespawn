...
final class G1AnimationRuntime {
     ...
     * courtesy), but the harness - the probe's geo dumps and the benchmark both bake through this
     * evaluator - proves SHIPPED rigs, every one of which must carry the key, so an empty order is a
     * harness failure here, never a fallback.
     */
     ...
     * WARN and fall back to GeckoLib's own face order, and the harness takes no fallback. This evaluator
     * bakes the bind sample of EVERY probed rig, translucent ones included.
     */

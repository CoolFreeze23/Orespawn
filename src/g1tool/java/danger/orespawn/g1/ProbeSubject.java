 ...
 *
 * <p>THE GETTERS' VALUES (owner 2026-09-14, second set revised, item 32 (2)): every int and boolean pose-interface
 * getter answers its REST value (0 / false: the defaults every landed dump was pinned against) unless the state
 * JSON's {@code getters} object declares another - {@code {"getters": {"getActivity": 1, "isInSittingPose": true}}} -
 * the reference-clip sampler's way of raising one value alone; the Slice 4b preset fields ({@code attacking},
 * {@code rock_type}, {@code ri1}, {@code rf1}, {@code seed}) keep their meaning and are read first. Every getter
 * records that it was read ({@link #readGetters()}; {@link #attackingRead()} kept for the attacking cross-check), so the
 * sampler can report which reads a hook reaches at the sampled inputs.</p>
 */
final class ProbeSubject implements Robot2Pose, Robot3Pose, Robot4Pose, RockBasePose, RotatorPose, PurplePowerPose,
     ...
     * attacking VALUE itself is declared through the state ({@code attacking}, the Slice 4b presets' field, or the
     * {@code getters} object's {@code getAttacking}): the sampler's attack state declares 1, its walk and idle states 0.
     */
    ...
    // seam's partial tick (owner's closing set item 3) over the declared previous-tick / current-tick values, vanilla's own
    // forms (LivingEntity.getAttackAnim :3057-3062, getSwimAmount :387).

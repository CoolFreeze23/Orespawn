...
/**
 * ENT-S-146. What the headless probe can record of the render state each side actually requests, in the
 * un-bootstrapped JVM (OPT-029 R0), written beside every dump as {@code <id>.render-state.json} so the
 * existing dumps stay byte-identical (the compiled dump's sha256 is the conversion's provenance).
 *
 *
 * <ul>
...
final class RenderStateProbe {
     ...
     * every run of the probe write a different sidecar: the address is dropped, so the name says "a lambda of
     * this owner" and two runs write the same bytes. Any other class is its own name.
     */

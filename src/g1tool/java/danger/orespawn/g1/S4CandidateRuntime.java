...
final class S4CandidateRuntime {
    ...
    /**
     * ENT-S-146: whether the shipped descriptor of {@code candidateClass} requires the within-cube
     * face-order key ({@code GeoReplacementDescriptor.cubeFaceOrderRequired}) - the probe hands it to the
     * {@link G1AnimationRuntime} evaluator, whose bind bake must be exactly as strict as this one.
     */
     ...
     * GeckoLib's own order ({@link DrawOrder#applyOrFallback}, the resource-pack courtesy), but
     * the harness proves SHIPPED rigs, every one of which must carry the key - a generated geo
     * without it is a harness failure, never a proof.</p>
     *
     ...
     * harness failure as an absent draw order: the harness takes no fallback.</p>
     */

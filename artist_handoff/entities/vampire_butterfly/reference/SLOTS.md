# Reference capture slots — vampire_butterfly

Empty placeholders the owner fills from the dev client (F3+B off, fixed camera, day light, `-Dorespawn.dev.geckolibRenderers=candidate` where the species has a candidate). One file per slot, named exactly as below.

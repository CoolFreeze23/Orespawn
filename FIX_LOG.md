...
## BUG-041 — THE MIRROR DROP (2026-09-13)

RULING. Owner 2026-09-13, second set, item 9: "the mirror drop lands now on its proofs (the reference leg and law 11),
the scope as ruled on 2026-09-05, one refuter, one gate; my Section B item 1 look follows it; a failure there reverts
the drop by commit." The scope of 2026-09-05 (addendum item 23 (4)): the 89 manifest models pinned in
`tools/reference_model_proofs.json`, `entity/client/ButterflyModel` and the seven item models under `client/model`, in
the same commit, geometry-only. Law 11 (the AUDIT BUG-041 entry, from Mojang's 1.7.10 client jar): 1.7.10's `ModelBox`
reads the part's mirror flag in its constructor, so a `mirror = true` stored AFTER `addBox` — the Techne export order
every one of the 3,542 reference stores follows — never reached its box; the port's `.mirror()` preceded `addBox` and
flipped every face's U. The drop removes exactly those calls; the geometry (origins, sizes, pivots, rotations, texture
offsets) is untouched, and the reference-geometry leg proves it.

COUNTS, CLAIMED -> MEASURED -> DROPPED (measured at 9c20d3e before anything was touched, `count_mirror.py`; every
number of the 2026-09-05 reconciliation reproduced, no difference to state):
- the 89 pinned models: 101 manifest entries, 89 with a pin, every pin `{MIRROR: n}`, the pins sum to 3,122 and equal,
  model for model, the live `.mirror()` call sites in the 89 sources (comments and strings stripped; every call the
  no-arg form; 0 mismatches); 12 exact (cliffracer, coin, enderreaper, ghost, island, islandtoo, kyuubi, mosquito,
  seaviper, skate, stinkbug, tshirt), none with a live call. Reference side: 3,542 `field_78809_i = true` stores in 108
  of 109 `Model*.java`, 0 before an addBox (live), 44 `= false`; 3,124 inert stores in the 89 pinned models' references
  (ModelGazelle 34 vs pin 33, ModelRobot4 56 vs pin 55: one part each the port never mirrored, nothing to drop).
  DROPPED: 89 models / 3,122 calls; 0 live calls remain in the 89.
- `ButterflyModel`: 10 calls -> 10 (orig ModelButterfly.java 10 inert stores) -> DROPPED 10; 0 remain.
- the seven item models (`client/model`): BattleAxe 15, Bertha 12, Chainsaw 7, Hammy 33, QueenBattleAxe 9, Slice 14,
  SquidZooka 12 = 102 -> 102 (references 15/12/8/33/9/14/12 = 103 inert stores, the Chainsaw's eighth on the `tooth`
  part the port lacks: a note, not a drop) -> DROPPED 102; 0 remain.
- port-wide: 97 files / 3,234 calls dropped (3,234 lines changed by the removal of `.mirror()` and nothing else;
  `git diff -U0`: 3,234 removed lines all containing `.mirror(`, 3,234 counterpart lines, 675 javadoc lines). KEPT, not
  BUG-041 (7 calls in 5 files): ModelGirlfriend 2 + ModelBoyfriend 2 (HumanoidModel arm/leg mirrors; 1.7.10 drew both
  with vanilla ModelBiped), ModelGodzillaHead / KingHead / QueenHead 1 each (port-only rigs). entity/client: 95 files /
  3,139 live calls before, 5 files / 7 after.
- the form: the precedent's (ModelEnderReaper at 5354420): the `.mirror()` removed from each builder chain, the file's
  line endings kept, and one javadoc note per file naming the orig file, its inert store count and the port's call count
  (appended to the class javadoc where one exists, a new block otherwise); the 97 raw `.mirror(` hits left in the
  sources are those notes, as EnderReaper's line 15 was.

THE MANIFEST AND THE HARNESS:
- `tools/reference_model_proofs.json` regenerated by tool, never by hand (`scratch mir/drop/update_reference_manifest.py`):
  `--add-outside` appended the eight outside models (reference_butterfly, reference_battleaxe, reference_bertha,
  reference_chainsaw, reference_hammy, reference_queenbattleaxe, reference_slice, reference_squidzooka: class, the
  renderer's texture, the 1.7.10 source of the same name, no pin); `--pins-from <the leg's reports>` then rewrote every
  entry's pin from the leg's OBSERVED categories: 89 MIRROR pins cleared (sum 3,122; each gains the precedent's `fixed`
  line "BUG-041 stage 2 (2026-09-13): mirror flags dropped; exact match required"; reference_elevator keeps its
  ENT-S-091 `fixed` line), 0 kept, one NEW pin only where named on the command line — reference_chainsaw
  `{MISSING_IN_PORT: 2, EXTRA_IN_PORT: 1}` with its pin_note (below). 109 models, `reference_source` declared for all,
  108 exact, 1 pinned.
- `G1ModelProbe` (g1tool, vanilla mode only; +90/-15 lines): three optional manifest keys so the standing leg can dump
  what the default path cannot construct — `constructor_arguments` (ButterflyModel(root, wingspeed): the shared rig of
  Butterfly / LunaMoth / Mothra / VampireButterfly, dumped at 1.0; the value only scales the hook's phase, the compiled
  layer is one), `layer_factory` (the item models expose `createLayerDefinition`, not `createBodyLayer`) and
  `geometry_only` (a static LayerDefinition holder with no model class, no setupAnim, no render type: the dump is the
  compiled tree, the texture and the bone names; no samples, no draw order, no render state). An entry without the keys
  takes the unchanged path — checked before the drop: the 101 reference dumps and their render-state sidecars produced
  by the patched probe are byte-identical to gradle's `build/reference/vanilla` at 9c20d3e, and the leg read them as
  12 PASS / 89 pinned / 0 drift.
- `tools/t2_model_proofs.json` (`update_t2_manifest.py`): the 11 entries whose `reference_note` deferred to the MIRROR
  pins (brutalfly, dragonfly, cockateil, ruby_bird, firefly, goldfish, ant, rainbow_ant, red_ant, termite, unstable_ant)
  gain `reference_source` after `texture` (ModelBrutalfly / ModelDragonfly / ModelCockateil x2 / ModelFirefly /
  ModelGoldFish / ModelAnt x5) and lose the note; the t2 chain's reference leg now proves all 14 rigs (14 `G1 REFERENCE
  PASS`; the s4 precedent was model_coin only). The g1 and s4 manifests carry no such note and were not touched.
- `tools/reference_survey_manifest.py` (the survey generator) untouched: the rule it would need for ButterflyModel is
  the class regex (`public class X extends EntityModel` does not match `ButterflyModel<T extends Mob> extends
  EntityModel<T>`) plus the probe's `constructor_arguments`; for the item models a `client/model` scan with
  `createLayerDefinition` and `geometry_only`. It is a survey tool, not the gate; the manifest is the gate's population.

THE REFERENCE-LEG RESULT: 0 MIRROR divergences on all 109 models. 108 exact (the 12 exact before, the 89 dropped, ButterflyModel,
six item models: BattleAxe 15 parts, Bertha 12, Hammy 33, QueenBattleAxe 9, Slice 14, SquidZooka 12 matched); 1 pinned —
reference_chainsaw `{MISSING_IN_PORT: 2, EXTRA_IN_PORT: 1}`: NOT a mirror divergence but a geometry finding the drop
uncovered (the item models had never been surveyed): orig ModelChainsaw.java draws a `tooth` part (lines 78-81, animated
along the blade by renderTooth*) the port lacks — the known note — AND the port's `blade2` box sits at x -2.5 where the
original's is at x 0.0 (origin (-2.5, -2.5, -2.5) vs (0, -2.5, -2.5), the same 1x5x5 box at the same pivot (0, 0, -28);
the leg reports the pair as one MISSING and one EXTRA). Pinned with its note so the count can only shrink; the owner's
call whether the blade2 offset is a parity bug to fix (the tooth is a missing animated part, a separate item).

THE CONVERTED RIGS (the seam): 17 shipped geos regenerated from the three headless chains, the converter's output byte
for byte (`copy_geos_all.py`, the t2b copy step's form; the g1 and s4 Elevator outputs identical, every shared consumer's
generated geo equal to its owner's except the identifier): beaver (g1), elevator (g1 = s4), purplepower, robot1, robot2,
robot3, robot4, robot5, rockbase, rotator, vortex (s4), ant, brutalfly, cockateil, dragonfly, firefly, goldfish (t2);
unchanged: cliffracer, coin, island, islandtoo, mosquito, tshirt (no mirrored cube) and the_queen (native). In every
regenerated geo exactly the formerly mirrored cubes' per-face UV rectangles moved (`modelpart_face_uv` bakes the flag:
Beaver 9/9 cubes, Elevator 5/5, Vortex 1/1, PurplePower 3 parts = 18 clone cubes, Robot1 27/27, Robot2 15/15, Robot3 19/19, Robot4 55/56,
Robot5 11/11, RockBase 22/22, Rotator 3 parts = 24 clone cubes, Ant 20/20, Brutalfly 14/14, Cockateil 16/16, Dragonfly 26/26, Firefly 12/12,
Gold Fish 16/16), the `modelpart_mirror` provenance key is gone (no cube is mirrored), no origin, size, pivot or
rotation moved, no bone field moved; the `orespawn:cube_face_order` key of Vortex (1 bone), Firefly (12), Gold Fish (16)
and PurplePower (18, its Shape__i clones) is rewritten from the flags — every cube's order
`[up, down, east, north, west, south]` -> `[up, down, west, north, east, south]` (the classic order; a mirrored cube
swapped its WEST/EAST polygons). The proof numbers (this lane's `--validate-only` evidence against the checked-in
`phase_g_reports/<tree>_proof/evidence/report.json`, per rig): geometry, surface mapping, animation, draw order and
keyframe-leg numbers moved on NO rig (all exact / PASS as before; provenance sha256s moved on every regenerated rig);
what moved is (a) the face-order objects above (Vortex, PurplePower, Firefly, Gold Fish), (b) visual numbers where the
now-unflipped texels change which pixels the two renderers disagree on — still PASS under the thresholds (changed
fraction 0.001, MAE 0.25): Vortex changed fraction 1.37e-4 -> 2.29e-4 and MAE 0.0042 -> 0.0077 on all 6 samples (the two
foreground fractions 0.05408 / 0.05406 swapped sides); Robot3 contested fraction 0.0025 -> 0.00076; Robot4 sample 2
contested 0 -> 3.05e-5; RockBase foreground fraction 0.1740 -> 0.1709 on samples 8-11 (both sides equally: the cutout
silhouette follows the texels); PurplePower 5 and Gold Fish 6 visual fields of the same kind; (c) t2's
`reference_geometry` rows NOT_APPLICABLE -> PASS for the 11 rigs above and `deterministic_text_outputs` 101 -> 112
files (the 11 new leg reports). Beaver, Elevator, Robot1, Robot2, Robot5, Rotator, Ant (x5), Brutalfly, Dragonfly,
Cockateil, Ruby Bird: provenance only. The three chains ran twice on the final state, diff-clean (vanilla, generated,
geo, reference, evidence identical between the runs), and run 1's generated geos equal run 2's.

CLIPS AND DESCRIPTORS, BYTE-IDENTICAL: the rotation basis is independent of the mirror flag. The 15 transcription clips
regenerated from their clip manifests with `tools/keyframe_clip.py` into scratch are byte-identical to the shipped
`.animation.json` (beaver, tshirt, mosquito, cliffracer, brutalfly, dragonfly, cockateil = ruby_bird, firefly, goldfish,
ant = rainbow_ant = red_ant = termite = unstable_ant); the 28 reference clips and their index re-sampled headlessly by
`ReferenceClipSampler` over the dropped classes are byte-identical to the checked-in `tools/reference_clips/` (29/29);
the clip manifests (`tools/keyframe_clips/`), the shipped animations, the `*GeoReplacement` descriptors and the pose
classes are untouched (`git status` empty for them).

ITEM MODELS: proven by the same leg, not by census — the harness dumps them through their own static factory
(`layer_factory` + `geometry_only`); six exact, the Chainsaw pinned as above. Per-file calls before/after: BattleAxe
15 -> 0, Bertha 12 -> 0, Chainsaw 7 -> 0, Hammy 33 -> 0, QueenBattleAxe 9 -> 0, Slice 14 -> 0, SquidZooka 12 -> 0.

GAMETESTS: no per-model mirror pin under `src/gametest` (59 raw "mirror" hits, all prose or gait/bench symmetry
language; 6 lines with cube UVs, all synthetic fixtures in DrawOrderFallbackTests / KeyframeLegTests); the pinned models'
classes referenced only by ModelAntRobot (AntGaitTests), ModelPurplePower (PurplePowerPoseTests) and ModelSpiderRobot
(SpiderGaitTests) — pose and gait, no UV assertion. Nothing to re-pin.

CHECKS (headless, no gradle): javac main / g1tool / gametest exit 0 / 0 / 0 (961 / 65 / 244 classes) on the harness
change and again on the drop; the standing reference chain on the final manifest 108 PASS / 1 pinned / 0 drift; the g1,
s4 and t2 chains exit 0 three times each (2 / 13 / 14 models, `G1 PARITY STAGING PASS`); `reference_renderer_pins.py`
exit 0 (133 registrations: PASS 120, NOT_APPLICABLE 13, DIVERGES 0, MANIFEST_DRIFT 0; the pins JSON unchanged — no
renderer or descriptor moved); `test_artist_package.py` 30 tests OK; the package dry run into scratch exit 0 (29 species
packaged, the round trip EQUAL with the order kept for all; the only row change: `dropped_keys` N -> 0 on the 22 registries
of the regenerated rigs, the `modelpart_mirror` key no longer present to drop; 34 warnings, the same set as the sampler
step's); `asset_audit.py` exit 1 with exactly 18 GECKO_GEO_PROOF_DRIFT (the 17 regenerated rigs, Elevator counted under
g1 and s4) and nothing else — the shipped geos now differ from the checked-in proof copies, which are the orchestrator's
to regenerate with `--write-proof`; green after that staging, as the shipped files ARE the chains' output.

PRESENTED BY THE ORCHESTRATOR (nothing here is a new ruling; each is reversible):
- THE HARNESS EXTENSION, with its before/after as the cost rules require of a harness change before its gate:
  `G1ModelProbe` gained three OPTIONAL vanilla-mode manifest keys (`constructor_arguments`, `layer_factory`,
  `geometry_only`) so the standing reference leg can dump the eight models the default path cannot construct — the
  parameterised `ButterflyModel(root, wingspeed)` and the seven static item-model holders. BEFORE (HEAD 9c20d3e): the leg
  proved 101 models — 12 exact, 89 pinned MIRROR; the eight outside models had no proof (the brief allowed a law-11
  census alone). AFTER: the same 101 models dump through the patched probe such that the leg, run over those dumps with
  the HEAD manifest, reproduces HEAD's 101 checked-in reports BYTE FOR BYTE (12 PASS / 89 pinned / 0 drift — the
  refuter's own re-run, its note N1: the lane's "dumps and sidecars identical to gradle's" sentence had no cmp log
  behind it and the gate's regeneration has since overwritten the HEAD-time output, so the reproduced reports are the
  evidence), plus the eight — no comparison semantics, threshold or category changed; the leg's verdict on every
  pre-existing row is the same function of the same inputs. This is a dump-side extension of the REFERENCE leg (not a
  transcription extension: Amendment 2's "no harness or controller extension is built to transcribe" is untouched).
- THE CHAINSAW PIN (decided under doctrine, reversible): the leg's first look at the item models found a geometry
  divergence in `ChainsawItemModel` (the missing animated `tooth`, and `blade2`'s box 2.5 units off in x). Parity lanes
  are frozen (2026-09-12 cost rules): it gets a register line — ITEM-071 — and stops; the entry is PINNED
  (`reference_chainsaw {MISSING_IN_PORT: 2, EXTRA_IN_PORT: 1}`, a `pin_note`) under the standing rule that a model
  carrying a ruled or reported divergence pins its exact count until the fix lands, so the gate stays green and the
  count can only shrink. Not BUG-041: the drop moved no box.
- THE PROOF RULE'S ORDER, this time honoured: every tree verified BEFORE any rewrite; the verify passes are fail-fast
  (each stops at the first drifted file) and their first hits are recorded in the GATE line; the per-rig account of what
  moved is the lane's `--validate-only` evidence above (geometry / surface / animation / draw order / keyframe numbers
  moved on no rig; provenance hashes, the four face-order objects and the visual disagreement fractions moved, all
  still PASS).
- THE REFERENCE CLIPS: the lane's headless identity claim (29/29) confirmed under `gradle referenceClips` at the gate
  (the rotation basis is independent of the mirror flag, as the sampler's javadoc states).
- THE LOOK: the owner's Section B item 1 look follows this landing (the look sheet carries the dated note; a failure
  reverts the drop by commit — `git revert` of the two commits, the proof trees returning with them).
- RECORDS TOUCHED: this section; AUDIT_FINDINGS BUG-041 (the header's stage list, the Resolution's status, a dated
  bullet) and the new ITEM-071 entry; the look sheet's Section B note; KNOWN_ISSUES' count unchanged (1277).
  `phase_g_reports/ADVISOR_HANDOFF.md:74` ("the port-wide drop of the other 81 models waits on the owner's A/B look")
  is the advisor's file — tracked, not edited: superseded by this landing (97 models / 3,234 calls on the reference leg
  and law 11; the look follows). `tools/reference_survey_manifest.py` (the survey generator) does not know the three new
  keys; it is a survey tool, not the gate — noted, not changed.

REFUTER NOTES: one refuter (2026-09-13), beside the gate. NO MUST-FIX; all eight claims CONFIRMED by independent
measurement: its own reference-side classifier (each `mirror = true` store checked against the boxes before AND after
it: 109 files, 3,584 boxes, 3,542 stores -> 3,542 inert / 0 live / 0 orphan, 44 `= false`, no part with more than one
box) and a source diff (the HEAD blob with its `.mirror()` calls stripped against the working file: the residual is 675
javadoc lines and nothing else in all 97 files; `git ls-files --eol`: every file still `i/lf`, the per-file working
endings preserved); the manifest tool replayed into scratch byte-identical to the checked-in manifest; the leg re-run on
the lane's dumps 108 PASS / 1 pinned / 0 drift with identical reports; 24/24 shipped geos `cmp`-equal to the chains'
third-run output; a semantic geo diff over the 17 (only `uv`, `uv_size`, the removed `modelpart_mirror` and
`orespawn:cube_face_order` moved; the face order rewritten on exactly Vortex 1, Firefly 12, Gold Fish 16, Purple Power
18 cubes); the clips 29/29 and 15/15; the gametest greps (PurplePowerPoseTests bakes an inline one-bone rig and never
loads the shipped geo); the t2 manifest tool replayed byte-identical. Its notes, folded in: N1 the harness-inertness
evidence (the presented item above); N2 the Rotator / Purple Power counts (24 / 18 clone cubes, corrected above); N3
the Elevator's manifest entry now carries no BUG-041 trace (its `fixed` line stays ENT-S-091 under the tool's
only-if-absent rule; its old BUG-041 pin_note is gone - acceptable, recorded here); N4 the lane echoed its javac / chain
/ dry-run exit codes to stdout without persisting them (the logs carry 0 error lines and the artefacts - class counts,
PASS lines, 29 package rows - instead), and the "dropped_keys N -> 0 on 22 registries" delta's baseline is the sampler
...
## RULINGS 2026-09-13 (FOURTH SET) RECORDED (docs-only) — push; TEST-008 (c) the rule and open; ANIM-021 to 024 deferred with "the original moved more" on their sheets; the checker's rule and the Cannon Fodder ratified; THE FULL FOLDER (every artist-tier species packaged, seeds drafted by lanes, the bosses' provisional locks, the full priority table)

Recorded in `phase_g_reports/phase_g_scope_addendum_2026-09-03.md` item 29 (the rulings in full) and `PHASE_G_PROMPT.md`
§6b (the bullet); the register carries dated lines under TEST-008 (ruled (c): bind excluded from the visual samples of a
rig whose hook writes the flat bone every frame, with the note; each renderer's own diagonal kept; the line open) and
under ANIM-021 to 024 (deferred with the parity lanes; the sheets gain the paragraph). Executed here, docs-only: the push
(origin/master 5db339d, the six commits after ba4d2c0, before anything else landed). The order of work from here: items
3 and 4 as one tooling commit (the four sheets' "the original moved more" paragraph from the register entries; the
Terrible Terror's sampler-row wording) → the full folder (items 5 to 8: the generator's path for a species without a
shipped rig — the reference leg's converter output, the "not yet in-game / bone names final" statements, "sampled when
the rig lands" for the reference clip; the seeds drafted by lanes, DRAFT-marked, one refuter over the twenty boss seeds;
the bosses' provisional locked bones from design §6, the Queen's profile the template; the full priority table, bosses

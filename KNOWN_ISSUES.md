...
- **Classic is the default** since the master-override decision: a default config
  already gives the exact 1.0/1.7.10 spider (visual-only legs, body-only hitbox,
  unsteerable); `modern.enabled = true` (with `spiderMovement = "MODERN"`, the key's
  default) is the opt-in to the modern robots.
